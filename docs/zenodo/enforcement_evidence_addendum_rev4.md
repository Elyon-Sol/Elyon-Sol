# Elyon-Sol v0.9.8.4 — Enforcement Evidence Addendum (Revision 4)
...
addendum publishes (a) an updated enforcement-evidence snapshot of the implementation and
(b) a change in how the implementation is distributed.
...
**Licensing change (the material update in this revision).** The canonical model remains
published for citation. The **core implementation is now released as open source** under the
GNU Affero General Public License v3.0 (AGPL-3.0) at
`https://github.com/Elyon-Sol/Elyon-Sol` — an open-core model: the admission gate, the
admissibility envelope, the target-side verifier, and the ext-authz sidecar are AGPL; a
commercial license is available for uses that cannot accept AGPL's network-copyleft terms;
and a separate administration / tooling SDK is proprietary and distributed separately. This
**supersedes the "proprietary, all rights reserved, not distributed" posture of Revision 3**:
the core is now public and inspectable. This document is licensed CC BY 4.0.

The full repository test suite is **419 of 419 passing, 0 xfailed**, up from 211 at
Revision 2's successor snapshot (Revision 3) and 84 at Revision 2. The growth is real work,
not re-counting: it adds an OPA/Envoy ext-authz sidecar and its adversarial suite, a
request-body-derived interaction binding, a multi-instance fail-closed replay guard, signed
published-record freshness wiring, clock-skew tolerance tests, an envelope inspector, an
executor SDK, an issuance log, and an MCP server surface (inventory in Section 1).
...
record transport (loopback model), and a reference enforcing target. Since that snapshot the
following were built and exercised in code:
...
| Repository | `https://github.com/Elyon-Sol/Elyon-Sol` (public, AGPL-3.0) |
...
new external interception run was performed for Revision 4. The enforcement code path it
exercised is unchanged at this snapshot.
...
1. Clone `https://github.com/Elyon-Sol/Elyon-Sol` and check out commit
...
canonical model (G(I) = AC³ ∧ T²⁶ ∧ CCS); the additions in this revision are (a) updated,
referent-bound enforcement evidence at the named commit and (b) the open-source release of
the core under AGPL-3.0, superseding the proprietary posture of Revision 3.
...
| Supersedes | Revision 3 (DOI `10.5281/zenodo.20600051`, snapshot `c756f8f`, published 2026-06-08) |
| Repository | `https://github.com/Elyon-Sol/Elyon-Sol` |
