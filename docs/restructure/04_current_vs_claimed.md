...
- **Replay within the window / exactly-once: CLOSED (VL-066).** A time-window bounds how LONG a captured decision is usable but cannot distinguish a first use from an in-window replay; that needs the executor to REMEMBER. `sign_envelope(..., decision_id=...)` adds a per-issuance signed `decision_id` (also `_HASH_EXCLUDED_KEYS`); `pep.py` stamps `uuid4().hex`; the reference target keeps a TTL-bounded seen-set (`app.state.seen`, pruned at `not_after`) and refuses an already-honored `decision_id` with `REF_VERIFY_REPLAY` (verify_envelope stays pure  -  anti-replay is the acting party's stateful concern). The falsifiable wedge claim now holds in-process end-to-end, demonstrated on an MCP-shaped tool-call surface (`EVIDENCE/proofs/wedge_agent_toolcall_001_runner.py`, 7/7, tool fired exactly once). **Honest open:** the seen-set is PER-INSTANCE; multi-instance exactly-once needs a SHARED cache (the genuinely hard distributed-dedup part)  -  named, not built (Phase-B item B3).
- **Sub-case (b)  -  record freshness: OPEN.** A stale-but-anchor-matching published record can still be honored cross-host; `reassert(record_source=...)` checks the record's hashes, not its liveness. The next continuity increment (Phase-B item B1): migrate the published-hashes record from the byte-anchor model (B-prime-1) to a signed-record model mirroring `key_record_source.py` (publisher signature + `not_after` + monotonic serial; reader enforces `REF_VERIFY_*_STALE`). Cross-host clock-skew tolerance (B2) and the 300s policy placeholder are the adjacent named-open items.
