...
Last updated: 2026-06-10 (VL-098: SEMANTIC RE-EVALUATION added to the envelope inspector - reevaluate_envelope() + the `reevaluate` CLI subcommand (spec docs/restructure/27_envelope_reevaluation_spec.md; suite 343 -> 362). Two judgments the repository previously could not make by tool: (1) internal consistency - the recorded decision vs the recorded condition_results under evaluate()'s own short-circuit logic (ELIGIBLE requires all-True, REFUSE requires at least one False; malformed/unknown fail-closed; condition_results.ccs None not consulted per VL-029 Decision A); (2) live re-evaluation - the envelope's recorded request_context re-run through the PRODUCTION evaluate() + ac3_valid/t26_valid/manifest_integrity_valid against the live manifest, answering 'would this be admitted TODAY' - the tool that PERFORMS the re-evaluation reassert()'s RE-EVALUATE-REQUIRED outcome demands (before VL-098 that outcome named work no tool did). The inspector's evaluation ladder is complete: shape (inspect) -> provenance (verify_issuer) -> currency (reassert) -> semantics (reevaluate) -> log completeness (reconcile); rungs orthogonal and composable. Live-state semantics are inherent (manifest_integrity_valid's G11 guard). Process finding: the live re-run immediately caught the VL-097 test fixture's invented AP/OP not satisfying the real manifest AR/R - the tool found a defect in its own test data on first use. No production module touched. Specs 26+27 are SINGLE-SOURCE (not yet cross-model-verified). G5 remains the only open road item. Does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-10 (VL-097: the envelope inspector / reconciler is BUILT per the Next-open-action directive - IMPLEMENTATION/envelope_inspector.py + spec docs/restructure/26_envelope_inspector_spec.md + 33 tests TESTS/adversarial/test_envelope_inspector.py (suite 310 -> 343, all green). Four capabilities: inspect (decode the exact bound scope - AP/OP/context/target_url + manifest version+sha pins + issuance metadata; fail-closed on shape), verify_issuer (issuer signature + validity window ALONE, verifier-step-1.5/1.5b semantics with the identical REF_VERIFY_SIGNATURE_* codes and VL-075 clock_skew parity), currency via envelope.reassert() unwrapped, and reconcile (a log of EXECUTED actions vs a log of ISSUED envelopes: MATCHED / OUT_OF_SCOPE / DUPLICATE_CONSUMPTION per action, UNUSED / INVALID_ENVELOPE per envelope; single-use consumption keyed on the VL-066 decision_id; a forged issuance-log entry cannot legitimize an executed action; currency deliberately NOT in the matching predicate - audit-time transitions must not retro-invalidate a then-current authorization) + a CLI whose exit code is the audit verdict. Build-then-wire: no production module touched, no default-path caller; the signed region and binding predicate are imported from envelope.py / verifier.py, not re-derived. GR-3: audit / red-team ergonomics, NOT a G5 closer; the reconciler audits a LOG (the trustworthy-log assumption is the explicit bound). Spec 26 is SINGLE-SOURCE (not yet cross-model-verified). G5 remains the only open road item. Does not cite its own hash (VL-012).)
...
**NEXT (chosen at VL-097 close, 2026-06-10) - the gate-side issuance log.** The VL-097 reconciler consumes "a log of ISSUED envelopes", but the gate (pep.py) does not yet persist one. Build-then-wire an issuance-log seam: append each signed ELIGIBLE envelope as one JSONL line through an injectable sink (default None - the default path stays byte-unchanged), spec'd as a new VL increment, so a live deployment produces the issued.jsonl that `python -m IMPLEMENTATION.envelope_inspector reconcile` audits against the target's action log. PRIMARY SOURCES: IMPLEMENTATION/pep.py + docs/restructure/26_envelope_inspector_spec.md (re-read them; do not work from a prior session's summary). Alternative next if preferred: cross-model verification of specs 26+27 (both SINGLE-SOURCE, VL-014/VL-015 precedent). Scope honestly: audit infrastructure and ergonomics, NOT a G5 closer. G5 (a real EXTERNAL attacker on a real PUBLIC surface) remains the only open road item; that track is deployment-and-people and is the author's.
