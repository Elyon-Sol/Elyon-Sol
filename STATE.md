...
PREVIOUS: 2026-06-09 (VL-086 deploy runbook (operational; no code/claim change): `deploy/host_setup_virtualbox.md` - the full self-contained VirtualBox sibling of the Hyper-V checklist (VL-085), for the author's cross-host TLS run on two Oracle VirtualBox Ubuntu VMs (VM-A = gate :8000 at 192.168.56.101; VM-B = target :9000 + publisher :9100 at .102). The VirtualBox-specific piece is networking: each VM gets TWO adapters - NAT (internet for apt/clone) + Host-only (vboxnet0, stable VM<->VM<->host IPs on 192.168.56.0/24) - with a netplan static-IP snippet for the second NIC. Otherwise identical to VL-085: bootstrap_config + gen_certs (SANs = both IPs), the docker-compose.hosts.yml IP override (committed compose is single-box), the per-host compose split, the attack_suite_live_runner invocation, the C4 flip-REAL_TRANSPORT-green step, curl smoke test, and troubleshooting. HONEST SCOPE unchanged: greens REAL_TRANSPORT (the cross-host referent), NOT the public-internet / external-attacker referent (G5/GR-3). UNVALIDATED in-sandbox (no VirtualBox/docker); the author's stand-up. No code/canon/test change (suite 298). Does not cite its own hash (VL-012).)
