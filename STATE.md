...
Last updated: 2026-05-31 (VL-039 T-G5-transport: cross-host transport of the published record landed. A target on a separate process fetches the published record over loopback, anchor-verifies it against a single pinned root (Decision B-prime-1: the sha256 of `EVIDENCE/published_hashes.json`, held out-of-band), and checks envelope currency against the FETCHED record rather than its own local disk (Decision D-b: `reassert()` / `verify_envelope()` gained an optional `record_source`; local-disk default preserves prior behavior and the 131 baseline). It honors a valid envelope despite a genuinely divergent local tree, and refuses tampered / anchor-failing / un-attested calls. New `IMPLEMENTATION/published_source.py`; new `TESTS/adversarial/test_cross_host.py` (8); evidence at `EVIDENCE/proofs/g5_cross_host_001.{runner.py,log,md}` (runner exits 0); pytest 131 -> 139 passed + 0 xfailed (real env and in-container). G5 is transport-built, NOT blanket RESOLVED  -  record freshness/revocation, signing/PKI (B-prime-2/3), secure anchor distribution, and multi-machine/TLS remain the G5 floor, named not built. The VL-038 cross-model adversarial audit verdict-of-record is logged in VL-039 (G4's scoped non-bypassability not overturned; F1-F5 pinned as `test_findings_001.py` tripwires). Prior substantive entry VL-038 at `33d0f5c`, audit disposition at `15c53cb`; parent `b48f9ac`. Next trajectory action: record freshness/revocation (the next hardening after transport), then signing/PKI and A1 target-side policy. VL-039 follow-up 2: a three-model forgery probe (Grok/OpenAI/Gemini, clean, blind, 3-0) found decision_sha256 is an unkeyed hash over public fields - the envelope is TAMPER-EVIDENT, not forgery-resistant; the 'forged' claim is retracted and the G5 proof corrected. Issuer signing (B-prime-2) is now the next increment, ahead of A1 and freshness, since an attestation that does not authenticate its issuer cannot be the floor under either)
