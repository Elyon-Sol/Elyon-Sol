...
Last updated: 2026-06-02 (VL-044 T-root-recovery: planned root rotation + per-root status built (B-prime-3), opt-in, build-then-wire. A current root signs its successor's designation, so a target pinning only R1 comes to trust a designated R2 in-band without a flag-day re-pin; per-root status (active/retired/revoked) is honored at the record-validation layer. `EVIDENCE/published_roots_gen.py` (live signer; the record is a runtime artifact, never committed) + `IMPLEMENTATION/root_record_source.py` (the B-prime-3 sibling reader: pinned-root signature -> freshness -> bootstrap downgrade -> within-record consistency -> per-root status view) + a `root_status_view` gate added to `key_record_source.py` (revoked -> REF_VERIFY_ROOT_REVOKED; a retired root's NEW record -> REF_VERIFY_ROOT_RETIRED via issued_at<retired_at while past records age out via freshness; root_status_view=None is exact VL-042 byte-behavior). `verify_envelope` logic UNCHANGED; verifier.py gains four REF_VERIFY_ROOT_* constants only. The honest ceiling: root-key COMPROMISE recovery stays irreducibly out-of-band (artifact 11 section 2); only PLANNED rotation is built. Spec `7cfc699` (artifact 11) + conservative-frame clarification `9e5181b` + build `aec58ee`. pytest 178 -> 196 + 3 xfailed (real env; the new ROOT_RECOVERY anchor joins the two). The readiness gate stays 0 of 3 green by design (see `EVIDENCE/readiness.json`): root_rotation is a built-but-unwired capability and ROOT_RECOVERY stays RED - green-on-built would be the CAPABILITY-masking-WIRING theater the gate exists to catch (Decision F); it goes green only when rotation is on `pep.py`'s default path and transported. Classification (VL-017a): a capability/trajectory move (the rotation primitive is a real capability advance). Decision H locked (ii): transitive root designation is a new trust assertion and owes a framework-level evaluate; the prompt is drafted (`~/elyon-sol-offrecord/vl044_root_designation_evaluate_prompt.md`), run off-framework AFTER this build, folds as the VL-044 follow-up - 'forgery-resistant' does not move regardless (constraint l). Prior substantive entry: VL-043 build `753e978`; VL-043 follow-up 2 `7a0b2b7`. Next trajectory action: run the transitive-designation evaluate and fold it (VL-044 follow-up); the doc-freshness commit (artifact 04 VL-041..044, 00_README owes 08/09/10/11); then the reds in order - DEFAULT_SECURE at the signing cutover, END_TO_END_NO_SHORTCUT at G5 transport, ROOT_RECOVERY when rotation is wired+transported; A1 target-side policy; caller-carry / proxy-removal; T-bookkeeping and T-prose-drift remain open)
