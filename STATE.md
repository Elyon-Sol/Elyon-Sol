...
Last updated: 2026-06-09 (VL-083 C3-live + C4 staging (artifact 13, Phase C - the last in-house step): the AUTHOR's real-transport run is staged so their part is run-it, not write-it. New `EVIDENCE/proofs/attack_suite_live_runner.py`: reads the live gate/target URLs + CA bundle from env, builds an HttpSurface over real `requests` (RequestsClient, now with TLS `verify`), and runs the VL-079 attack suite over real transport; exits 0 iff every attack is defeated, 2 (loud) if unconfigured. EXCLUDED from the CI runner loop (a documented skip in ci.yml, parallel to the multi-process-TLS / external-webhook skips - no live surface exists in CI). Two backward-compatible harness additions: `RequestsClient(verify=)` and `run_suite(include_stale=)` (the live HTTP adapter cannot control the gate's decision window, so the live run sets it False; stale/drift stay covered by the in-process suite). C4: `EVIDENCE/readiness.json` gains a `REAL_TRANSPORT` deployment predicate, RED by design, GR-2-honest (named blocker = the author's real-transport run via the live runner); validate_manifest stays clean (0 errors), the canonical summary stays 3/3 (REAL_TRANSPORT is the uncounted new tier until the author greens it). Spec `docs/restructure/22_live_attack_run_and_real_transport_predicate_spec.md`. Acceptance: `TESTS/adversarial/test_attack_harness.py` (4) - the live-suite subset (7 attacks + positive control, no stale/drift) defeated over an in-process HttpSurface + the live runner exits 2 unconfigured. pytest 296 -> 298 + 0 xfailed. Build-then-wire: IMPLEMENTATION / canon / MANIFEST / published_hashes byte-unchanged -> no evaluator_sha256 roll. **Every in-house-authorable item on the artifact-13 road is now done (A1-A6, B1-B5, C1-C4 scaffolding); the remainder is pure AUTHOR execution on real hosts + a real external attacker (G5/GR-3).** Does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-09 (VL-082 C2 (artifact 13, Phase C): real TLS/cert + trust bootstrap artifacts authored (locus AUTHOR for a real cross-host TLS run). New `deploy/tls/gen_certs.py` generates a private dev CA + per-service leaf certs (gate/target/publisher, EC P-256, SANs for the compose names + localhost + any real hostnames) using the `cryptography` lib only (no openssl-binary dependency). `deploy/docker-compose.tls.yml` is a compose OVERLAY that serves each service under `uvicorn --ssl-*`, flips the URLs to https, and points the gate/target clients at the CA bundle via the existing `transport.py` `ELYON_TLS_CA_BUNDLE` / `ELYON_TLS_CLIENT_CERT` hooks (no code change). `deploy/tls/trust_bootstrap.md` documents the out-of-band distribution (CA bundle + pinned anchor + gate public key on channels SEPARATE from the record) and the dev-CA vs real/Let's-Encrypt paths. Spec `docs/restructure/21_real_tls_and_trust_bootstrap_spec.md`. HONEST LOCUS SPLIT: the container/TLS stand-up is UNVALIDATED (no docker), but the cert MATERIAL is validated in-sandbox by a REAL TLS handshake - `TESTS/deploy/test_tls_certs.py` (6) drives a server (leaf) + a CA-trusting client to a completed, verified handshake over `ssl.MemoryBIO`, and a wrong-CA client is REFUSED (SSLCertVerificationError, fail-closed); plus chain validation (CA self-signed + CA:TRUE; leaf CA-signed, in-window, SAN) and the overlay structural check (dependency-free, no pyyaml). `deploy/tls/certs/` is git-ignored. pytest 290 -> 296 + 0 xfailed. Build-then-wire: new deploy/tls/ + overlay + test only; IMPLEMENTATION / canon / MANIFEST / published_hashes byte-unchanged -> no evaluator_sha256 roll. Next: C3's LIVE run + C4 (AUTHOR - real hosts). Does not cite its own hash (VL-012).)
