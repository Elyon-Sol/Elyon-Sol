...
Last updated: 2026-06-09 (VL-089 attack-harness fix, the THIRD (deepest) real-surface finding - push vs caller-carry: on the live run all 6 adversarial attacks DEFEATED over real cross-host TLS, but the positive control came back REF_VERIFY_REPLAY. Cause: the production gate uses PUSH delivery (VL-038) - on ELIGIBLE it forwards the signed envelope to the target itself, so the honor happens AT ADMIT; the harness then re-presents the same envelope (a caller-carry assumption) and the target correctly refuses the duplicate decision_id. In-process this was invisible (the gate forward is mocked = no push). This is the caller-carry/push architecture fork the docs flag, made concrete by the real surface. Fix (`EVIDENCE/proofs/attack_harness.py`): a `push_delivery` mode where the positive control OBSERVES the target acting (its read-only `/received` count via new `HttpSurface.acted_count` + `RequestsClient.get`) instead of re-presenting; the live runner passes `push_delivery=True`. In-process (mocked / caller-carry) is unchanged (default False, present-then-honor). Tests 5/5 (incl. an acted_count plumbing test); full suite 298 -> 299. Same hermeticity class as VL-087/088 (the harness's model held only in-process). ACTION: client-side only - re-run `attack_suite_live_runner.py` (the laptop repo has the fix on disk); no cert/service change. Does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-09 (VL-088 attack-harness bug fix, the SECOND real-surface finding: the `target_url_swap` attack in `EVIDENCE/proofs/attack_harness.py` admitted an envelope bound to a literal unreachable URL (`mcp://elyon-sol/OTHER-surface`). In-process the gate's ELIGIBLE forward is mocked, so it worked; against the REAL gate the forward is live (push delivery), the `mcp://` scheme is unreachable, the gate fails closed, and the admit returns no envelope -> `KeyError: 'envelope'` on the live run (after 6 attacks ran cleanly over real cross-host TLS, confirming the VL-087 cert fix). Fix: derive a REACHABLE different-identity swap URL from the surface's real target (`real_target + '-SWAP'`, a different path on the same reachable host) so the gate can mint the envelope and the executor still refuses it on the binding check; falls back to the literal for surfaces with no target attribute. In-process tests + the HTTP-subset test 4/4; suite 298/0. Same hermeticity class as VL-087 / Lesson 12 (the harness assumed admit does not forward - true only in-process). ACTION: the client (laptop) repo already has this fix on disk; just re-run `attack_suite_live_runner.py` (no cert regen / no service restart needed - this is client-side only). Does not cite its own hash (VL-012).)
