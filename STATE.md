...
Last updated: 2026-06-10 (VL-096 live: the three-domain POC (medical/legal/finance) reproduced LIVE over real cross-host TLS on the VirtualBox deployment - gate VM-A .101; SIGNED-mode reference target + publisher VM-B .102; dev-CA TLS; Windows client. Each domain ran the 13-case suite against the REAL gate and REAL enforcing target: 12/13 passed LIVE each (36/36 substantive cases, 0 failures); only the timing-only stale_decision was skipped (gate window 300s; proven deterministically in-process). Per domain: 3 admitted+honored via real PUSH delivery (target /received incremented), 3 gate REFUSALs (AC3 / T26 / manifest-integrity each isolated), 6 executor REFUSALs over the wire (ENVELOPE_ABSENT / SIGNATURE_INVALID / REPLAY / BINDING_MISMATCH x3). Proof of record EVIDENCE/proofs/three_domain_poc/live_run_001.log + the three reports regenerated Mode: live. The runner's live-mode fix (target client-base vs bound-identity split --target-url/--target-id + push-aware positive controls) is pushed at 51d5ff3. GR-3: characterization over real transport (two VMs / one host / private net / dev CA / the author's own calls), NOT certification; the gate/envelope/verifier/target are byte-unchanged; G5 (a real external attacker on a public surface) remains the only open road item. Does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-10 (VL-096 THREE-DOMAIN POC: a synthetic CHARACTERIZATION of the UNCHANGED production admission chain across three unrelated domains - medical (e-prescribing), legal (court e-filing), finance (trade execution). Each domain = its own manifest (domain-realistic AR/R + version) + 13 reviewer-legible synthetic cases: 3 admitted (incl. minimal-authority) + 3 gate-refusals that report WHICH of AC3/T26/manifest-integrity failed + 7 executor-refusals (unattested/forged/replay/rebind-operation/rebind-context/target-swap/stale). New proof `EVIDENCE/proofs/three_domain_poc/` (domains.py synthetic content; poc_runner.py dual-mode inproc+live; 3 committed manifests; 3 generated reviewer reports; RUNBOOK_live.md) + spec `docs/restructure/25_three_domain_poc_spec.md`. In-process self-verify: 39/39 cases (13x3) pass against the pep gate -> signed envelope -> verify_envelope + ReplayCache executor; gate/envelope/verifier byte-unchanged; MANIFEST/manifest.json is swapped-per-domain-then-restored (canonical sha pristine); full suite 310 unchanged. The runner's live mode drives the real gate+target over TLS - the author's to run per the RUNBOOK; it reproduces the committed reports over real transport. GR-3: in-loop runs are characterization (breadth of domain CONTENT over one decision chain whose security properties are unchanged), NOT certification; G5 untouched and remains the only open road item. Does not cite its own hash (VL-012).)
...
**NEXT BUILD (chosen 2026-06-10) - the envelope inspector / reconciler.** A local tool that takes a signed admissibility envelope and: (1) decodes the exact scope it binds (AP / OP / context / target_url + manifest version+sha pins); (2) verifies the issuer signature against a pinned key; (3) runs `reassert()` for currency against live / a published record; and (4) reconciles a log of EXECUTED actions against ISSUED envelopes - an action with no matching bound, single-use envelope is an out-of-scope action (the auditability property worked out in the VL-096 follow-on discussion). Build from `IMPLEMENTATION/verifier.py` + `IMPLEMENTATION/envelope.py` as PRIMARY SOURCES (re-read them; do not work from a prior session's summary), and spec it as a new VL increment per SESSION_PROTOCOL. Scope honestly: this is enabling / audit infrastructure and red-team ergonomics, NOT a G5 closer. G5 (a real EXTERNAL attacker on a real PUBLIC surface) remains the only open road item; the public-surface + rules-of-engagement track is deployment-and-people, runs in parallel, and is the author's.
