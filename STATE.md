...
**Ledger discipline (GR-4, 2026-07-03):** the verification ledger is APPEND-ONLY and scoped to verification events (verified/corrected/retracted/disputed a claim vs a primary source). Authoring, recruiting, site copy, refactors, and bookkeeping are commit + STATE only, NOT ledger entries. The ledger is never pruned/renumbered; readability is served additively by `EVIDENCE/verification_ledger_index.md`. This GR-4 adoption is itself recorded here + in the commit, with NO VL entry - the rule obeying its own clause 1. **Archiving (GR-5, 2026-07-03):** the ledger is archived in immutable, hash-anchored, byte-preserving volumes under `EVIDENCE/ledger_archive/` (manifest `INDEX.md` with a reconstruction check). Initial split: VL-001..VL-107 -> vol_001 (sha 381008b5...); VL-108 onward stay active (30 entries). Commit + STATE only, no VL entry.

Last updated: 2026-07-16 (VL-150: REFUSAL REASON CODES — the canon-evolution audit's top auditability increment, scoped as a CODE enhancement; CANON UNTOUCHED (not a canon-version event). `evaluator.decide(ctx, manifest) -> (state, reason)` is the single source of truth; `evaluate()` is now a byte-behavior-identical back-compat projection `decide()[0]`, `refusal_reason()` the diagnostic `decide()[1]`; six `G_` codes (G_MANIFEST_MALFORMED / _REQUIRED_SETS_UNRESOLVED / _AC3 / _T26 / _MANIFEST_INTEGRITY / _INTERNAL), one per real refusal point in short-circuit order, in a namespace DISJOINT from the boundary REF_* vocabulary; `pep.governed_call` surfaces `refusal_reason_code` in the 403 (additive — the REF_PEP_FAIL_CLOSED path unchanged). No admissibility-decision change, no new invariant/gate (canon §13/§14). evaluator_sha256 e307fab2 -> ca7c922c; EVIDENCE/published_hashes.json regenerated via its generator (VL-115 discipline); canon_sha256 / evaluator_version (0.9.8.4, tracks the canon version implemented, not the code revision) / manifest_sha256 UNCHANGED. +15 tests (TESTS/adversarial/test_refusal_reason.py) incl. an END-TO-END pep-403-carries-G_AC3 check (upstream never reached); suite 630 -> 645 green. Commit 732f1d2 (code) + this close. NOT deployed — the author deploys; a partial rollout self-refuses via reassert() Row 3 (evaluator_sha256 mismatch). NOT the v0.9.8.5 canon increment: the canon-text pieces (§6 naming note, §8.2 PoE rationale) remain the author's to ratify and author the PDF; on honest assessment the canon is not a compelling standalone increment, so only the code half — the true, executable advancement — was landed. ALSO THIS SESSION since VL-148: VL-149 corrected VL-147's live-site-stale claim as a browser-CACHE artifact (the live site was CURRENT); repo-hygiene (no VL, GR-4 clause 1) backfilled the ledger index VL-132..146, added scripts/repo_health.py to automate the close-protocol bookkeeping, and GR-5 Amendment A1 broadened the archive cut-anchor to `^#{2,3} VL-N`. NOT G5 — adds observability, not a live-surface behavior change.)

PREVIOUS: 2026-07-16 (VL-148: SES-9b (GL-01-refine, VL-124) RESOLVED — the LAST deferred governance-review item. pep now resolves approver trust IN-PROCESS from the pinned-root SIGNED key-record chain (the ELYON_APPROVER_KEY_RECORD_PATH + ELYON_PINNED_ROOT_* trio, via the existing key_record_source + approver_trust.resolve_approver_keys), and the startup wiring guard gates G-01 on signed-chain PROVENANCE, not mere injectedness — closing the residual where a gate-controlled INJECTED map under a different key_id passed the guard and verify_grant. The deploy shim approver_trust_bootstrap retires its injection step to a thin pep.app re-export (same env contract, so compose/docs keep working). The analog, for the APPROVER key, of what SES-9a/K-01 did for the ISSUER key. Suite 616 → 630; NEW test_approver_signed_chain.py drives pep's own resolver over a REAL signed record (approver-role resolves SIGNED_CHAIN; no-role/wrong-root/tampered/missing fail closed to empty-but-still-SIGNED_CHAIN → G-06; the real startup hook starts a signed-chain high-impact gate and fails closed on the static pin); the injected-refused G-01 catcher is REVERT-PROVEN RED. DEFAULT-OFF / inert on the live flat manifest (HIGH_IMPACT: []) — guard is a NO-OP, per-request path byte-unchanged; canon/evaluator/manifest/impact/envelope/verifier/approval/approver_trust byte-UNCHANGED, NO pin moved; pure, ABOVE G(I). WHITE-BOX deployment-posture hardening, NOT a live break (the residual was never exploitable on the single-worker HIGH_IMPACT:[] surface); NOT G5. Commit ef91842 (build) + the ledger/STATE close. — PREVIOUS THIS SESSION: VL-147 + follow-up: PUBLIC-SURFACE CURRENCY + the item-(0) security gate closed, and a CONTINUITY repair under a new GR-6. THE REPAIR (no VL — reorganization, GR-4 clause 1): STATE.md had reached 258,290 bytes / ~106k tokens, so SESSION_PROTOCOL resume step 2 ("Read STATE.md in full") was physically unrunnable inside a working context and `## Next open action` — the field the protocol calls the single most important for continuity — sat at byte 225,092 behind all of it; sessions oriented off the newest history blob instead of the ordered list. GR-6 adopts GR-5's archive design wholesale: the 29-block PREVIOUS chain (VL-109..VL-145) MOVED verbatim to STATE_archive/vol_001 (hash-anchored, sidecar, manifest), Next open action hoisted to the head (byte 225,092 -> 3,486), section headings preserved verbatim (scripts/update_state_vl011.sh matches on full-line equality). Executable check `python STATE_archive/reconstruct.py` — the pre-split file (sha256 289fe5b2..., blob at 61ad782) reassembles byte-for-byte from the archive. NEW CLAUDE.md carries the bounded read order + the GR-1..GR-6 invariants into every session automatically. Commits 4890d85, a4b7add. THE VERIFICATION (VL-147): the public site advertised Rev 6 as the current Zenodo record while Rev 7 (21313671) AND Rev 8 (21364720, 14 Jul) were published — a twice-superseded citation target — fixed at the MECHANISM by adopting the CONCEPT DOI 10.5281/zenodo.19367848, which always resolves to the current revision, so the class cannot recur; suite 553 -> 616; the changelog footnote falsely called SES-5/SES-6 open (fixed VL-143/144) and omitted SES-8 + SES-9a. site/index.html corrected (53f35fc); the LIVE site is a separate artifact and still carries all four drifts until applied in its editor (VL-131). ITEM (0) IS DONE, both halves: the repo IS public (GitHub API `visibility: public`; Rev 8 says so), and the history-scrub HARD GATE is MET — a presence-only sweep of the FULL object store (1055 blobs, every ref) found ZERO 64-hex secret assignments and ZERO PEM private blocks. The gate is satisfied vacuously: VL-108/VL-146 both record the keys as "printed to the working chat", never committed, and deploy .env files are gitignored — the exposure channel was the chat, never the repository. Combined with DEAD (rotated VL-122, again VL-146), "dead AND unreachable in history" holds. NAMED-OPEN, recorded not closed: vol_002 (`## Current verified state` is 165,108 bytes — 64% of the pre-split file — and is structurally history: 107 bullets, 100 naming a VL, VL-008..VL-131; deciding which remain TRUE is author judgment); the GR-5 clause-1 heading break (VL-135 onward use `## VL-N`, so the "cut at a primary `### VL-N` header" rule matches no recent entry and the NEXT ledger archiving event would mis-cut); the ledger index gap (VL-132..146 unsummarised). Those last two share a boundary with Current-verified-state's: all three stopped at 2026-07-03 — the close protocol's additive-readability bookkeeping stopped being executed while the ledger itself kept being appended correctly. WHITE-BOX, author-side; no code/canon/manifest/test change, suite 616 unaffected, no pin moved. NOT G5 — this corrects what the project SAYS about itself, not what it does; the external-attacker floor is unmoved.)

PREVIOUS: 2026-07-14 (VL-146: SES-9a ENABLED on the live surface — signed key-record mode turned ON for the target + sidecar; the K-01 gate-key revocation floor is now IN-BAND (flip `revoked` in one publisher-signed record) instead of out-of-band re-pinning N consumers. Publisher serves /published_keys_signed.json (per-request ROOT-signed; a NEW endpoint — the kickoff wrongly assumed it existed — commit 4a40bc9, +5 tests, adversarial suite 538 green); target pins the ROOT public via the ELYON_KEY_RECORD_URL/_ROOT_ID/_ROOT_HEX trio, sidecar via the local-file ELYON_KEY_RECORD_PATH trio kept fresh by elyon-keyrecord-sync.timer (120s, inside the 300s not_after). PROVEN LIVE over real transport: target HONORS via the record + 6/6 attacks defeated, fail-closed REF_VERIFY_KEY_RECORD_INVALID, revocation drill REF_VERIFY_KEY_REVOKED then restore-at-higher-serial, rollback rehearsal (trio unset -> static pin honors); sidecar /authz ALLOW + revoke-drill DENY/REF_VERIFY_KEY_REVOKED + fail-closed DENY/REF_VERIFY_KEY_RECORD_INVALID. PREREQ: all three nodes were 14 commits behind on 7dac7ad (1d6e777 absent); ff-redeployed to 012834b/4a40bc9 and restarted before any trio was set. INCIDENT: the publisher SIGNING key and the key-record ROOT private were each printed to the working chat and were ROTATED/REGENERATED (publisher-2026-07-14 / root bc12137d) — closes the VL-108 open-item (1) publisher-key-regen, but repeated that exposure class. NEW availability dependency: key-record freshness on the critical path — pub outage or stalled sidecar sync fails the surface CLOSED (REF_VERIFY_KEY_RECORD_STALE). D3 (root-status chain) DEFERRED (LATER; anti-drift). Canon/evaluator/manifest pins UNCHANGED; deploy/.env changes live only on the hosts (gitignored). WHITE-BOX, author-side live-ops; NOT G5 — the external-attacker floor is unmoved and remains the finish line. NEXT: monitor elyon-keyrecord-sync + a print-guard for *_SIGNING_KEY_HEX / *_PRIVATE* values (this session's two follow-ups).)

**History:** the `PREVIOUS:` chain is archived under `STATE_archive/`
(GR-6, byte-preserving; manifest `STATE_archive/INDEX.md`). This file carries
current state only - `git log --oneline` is the record of what was done.

---

## Next open action

**At 2026-07-16 (VL-148) — the literal next actions, in order (AUTHOR-locus). This block supersedes the item lists below; they are retained as the dated record of each session's view.**

**GOVERNANCE TRACK — SES-9b (GL-01-refine) RESOLVED at VL-148; the governance-review backlog is now EMPTY.** The last deferred item (VL-124's approver-provenance sharpening) is built and revert-proven: pep resolves approver trust in-process from the signed key-record chain and the wiring guard gates G-01 on provenance. All of Feature-1 (mechanism 1a-1d + R1 + R2), Feature-2 (mTLS 2a), the integration proof, SES-1…9a, and now SES-9b are done in-repo. What remains for a CLAIMABLE oversight guarantee is OPERATOR-LOCUS only (Feature-2 layers 1+3 on real hosts + a shared store), unchanged, and gated on a real deployment decision — no in-repo governance capability is outstanding. Typed-impact turn-on (VL-132/133, item (3) below) is the next governance BUILD only if the HIGH_IMPACT policy decision is taken.

**(0) RETIRED — DONE.** The 2026-07-05 item (0) "HISTORY-SCRUB + OPEN THE CORE REPO" is closed in BOTH halves and should not be re-opened. The repo is public (GitHub API `visibility: public`; Rev 8: "The source repository is public and inspectable"). The history-scrub HARD GATE ("the publisher signing key exposed at VL-108 must be dead AND unreachable in history") is MET: DEAD via VL-122 + VL-146 rotation, and UNREACHABLE vacuously — a presence-only sweep of the full object store (1055 blobs, all refs) found zero secret assignments and zero PEM private blocks, because the exposure channel was the working CHAT, never the repository (VL-108/VL-146 both say so; deploy `.env` is gitignored). Referent: VL-147 follow-up. Honest ceiling: that sweep targeted the class item (0) named (an Ed25519 seed assigned to a signing/private var, plus PEM blocks) — it is NOT a general gitleaks/trufflehog run, which remains a separate, lower-priority question and is NOT closed.

**(1) LIVE-SITE CORRECTIONS — RESOLVED / MOOT (corrected at VL-149); the live site was already CURRENT.** CORRECTION (2026-07-16): VL-147 and an earlier note here claimed the LIVE elyon-sol.io was stale (Rev 6 / 553, "two items remain tracked" footnote). That was an IN-APP-BROWSER CACHE ARTIFACT — every fetch this session served a cached page; the browser was never cache-busted. The author's screenshot shows the live hero reading `616 tests green` (the Rev 8 figure), and per the author the live site matches Rev 8. The genuinely stale artifact was the REPO FILE `site/index.html`, which lagged at 553 / Rev 6 until 53f35fc corrected it — that file was read DIRECTLY (file tool, not the browser), so those corrections STAND and were necessary; VL-147 simply got the drift DIRECTION backwards (the repo was behind the live site, not in sync with it). Consequence: there is NO pending outward posting action — the live site is current. I cannot independently re-verify every element (the in-app browser cache persists); the author is authoritative on the live state, and if any element lags it is the author's to spot. STANDING LESSON: an in-app-browser read is NOT a primary source unless cache-busted; the file tool and out-of-band HTTP (PowerShell `Invoke-WebRequest`) are reliable, the browser cache is not. TEST-COUNT CONVENTION (still valid, documented in an `site/index.html` comment): the page's `tests green` figure tracks the CURRENT PUBLISHED Zenodo revision (Rev 8 = 616), NOT repo HEAD (630 at VL-148) — so every number stays citable to a DOI and advances per-publication, not per-commit. This also retires the old item (5) ("elyon-sol.io says 512 tests").

**(2) Everything below is unchanged in kind: G5 EXTERNAL VALIDATION remains the finish line and is not author-controllable** — it needs a blind external party to engage. The remaining author-side gate is counsel sign-off on `deploy/SAFE_HARBOR_CLAUSE.md` (item (1) of the 2026-07-05 list). Advancement, hardening, and internal red-teaming continue to be valuable on their own terms; GR-3 already labels them correctly, and that labelling is the strength, not a deficiency.

**(3) The GR-6 named-open items.** (a) The **GR-5 clause-1 heading break** — RESOLVED 2026-07-16 via **GR-5 Amendment A1**: the cut anchor now accepts both `## VL-N` and `### VL-N` (`^#{2,3} VL-N`), so the next ledger archive will not mis-cut. NO ledger entry was edited (GR-4 clause 2 — in-place normalisation is forbidden; the rule was made tolerant instead). (b) **vol_002** — STILL OPEN: `## Current verified state` is 165,108 bytes and is structurally history (107 bullets, 100 naming a VL, VL-008..VL-131), stale since VL-131; archiving it needs author judgment on which bullets remain TRUE, not a mechanical cut. (c) **Ledger index gap** — RESOLVED 2026-07-16: VL-132..VL-146 backfilled in `EVIDENCE/verification_ledger_index.md` (VL-134 noted reserved/never-issued). ROOT-CAUSE FIX (new this session): `scripts/repo_health.py` automates the close-protocol bookkeeping — GR-6 reconstruction + ledger-index currency + STATE ordering — so this drift class is caught by one command instead of by memory. That directly answers the pattern behind (a)-(c) and Current-verified-state's staleness (all began 2026-07-03): a protocol step too expensive to run by hand silently stops being run. **Remaining GR-6 open item: (b) vol_002 only.**

**At 2026-07-13 (VL-145) the AUTHOR-locus next action is UNCHANGED IN KIND: G5 EXTERNAL VALIDATION.** The CVP red-team cycle is now FULLY CLOSED: SES-1…4, 7, 8 fixed (VL-135…140), SES-5/6 resolved (VL-143/144), SES-9a resolved (VL-145, commit 1d6e777 — the last open SES item; SES-9b/GL-01-refine deferred to a governance session, inert on the live flat manifest). Remaining author-side items, in order: (1) ENABLE SES-9a on the live surface — the next planned session, scoped in docs/redteam/SES9A_ENABLE_KICKOFF.md: publisher serves the signed key record, out-of-band root pin, REDEPLOY target+sidecar at ≥ 1d6e777 (the live chain predates it), set the ELYON_KEY_RECORD_* trios, then a live revocation drill; enablement is LIVE-OPS and gets its own VL entry. (2) The recruit-a-blind-external-red-teamer action below is unchanged - that is still what moves G5.

**At 2026-07-05 (session close) - the literal next actions, in order (AUTHOR-locus):** (0) HISTORY-SCRUB + OPEN THE CORE REPO. The core is AGPL-3.0 open-core but the GitHub repo is private, contradicting the inspect/adopt/attack thesis and the red-team challenge (which points researchers at repo paths). Before flipping visibility, audit + rewrite git history for secrets - the publisher signing key exposed at VL-108 (rotated VL-122) must be dead AND unreachable in history (gitleaks/trufflehog over full history; git filter-repo/BFG if any live-sensitive material remains). Highest-leverage move + a HARD security gate. (1) Finalize the safe-harbor clause (counsel, deploy/SAFE_HARBOR_CLAUSE.md) + open the red-team challenge - the only path to G5. (2) Reconcile the deployed-commit divergence (ledger records the four nodes at 3343e32; a later operator deploy record puts the gate at bd1159b) and decide the live-surface scope. (3) TYPED-IMPACT DEPLOY: VL-132 (evaluator) + VL-133 (wiring) are built/proven/default-off; the turn-on is the manifest flip + ~85-test migration (propose VL-134) + the coordinated two-hash re-pin per deploy/TYPED_IMPACT_DEPLOY.md; gated on the HIGH_IMPACT policy decision. (4) Re-issue the R1 approver key-record before 2026-08-03 (deploy/governance/make_approver_key_record.py --serial 2; an expired record fail-closes the live gate to refuse-all) + reboot the gate host. (5) Fix site staleness (elyon-sol.io says 512 tests; suite is 541 - derive the count). (6) HIL APPROVAL WORKFLOW (design, raised 2026-07-05): the approval PRIMITIVE is built/proven (202 PENDING_APPROVAL hold -> a human signs a grant with their OWN key via approver_cli -> verify_grant -> forward once, single-use, SoD-enforced), but the human WORKFLOW layer is minimal - it assumes an operator polling the queue; no push alerting, no self-service approver surface, no timeout/escalation, which does not scale to a real deployment (e.g. a hospital gate). Two facts fixed for continuity: (a) the high_impact designation is NOT a runtime agent - it is DECLARATIVE policy in the SHA-pinned MANIFEST/manifest.json HIGH_IMPACT set (per-interaction-type under typed impact), evaluated deterministically + fail-closed by impact.requires_approval/safe_high_impact; the policy owner authors + re-pins it; (b) LOAD-BEARING SoD constraint on any approval UX - the approver signs with their own key on their own device (hardware token / secure enclave / their CLI); NO server-side approve-button that signs (that would let the gate self-approve), and the operator console holds no signing primitive. Next step is a SPEC (alerting on the read-side /pending -> a role-appropriate on-device approver surface -> timeout+escalation, fail-closed -> multi-approver rotation, which the R1 signed key-record already supports on the trust side). This is OPERATOR-TOOLING locus (the alerting/UX half); the core stays default-off. G5 NOT-MET.

**At VL-127 (2026-07-03) - the literal next actions, in order (all AUTHOR-locus):** (0) verify the FIVE sandbox-built commits (685a907, a2c9f82, 56b1ea9 = VL-126; a8ae52b + the VL-127 close) blob-by-blob natively and `git push origin main` - the sandbox has no credentials, so HEAD is ahead of origin until this lands; if a session resumes before the push, pushing IS the first task (at-rest invariant). (1) Pre-exposure item 3: cert-renewal deploy-hooks on ALL FOUR nodes per `deploy/PHASE1_PRELAUNCH_RUNBOOK.md` Gate 2. (2) Item 4: counsel sign-off on `deploy/SAFE_HARBOR_CLAUSE.md` (HARD GATE before any post or invitation). (3) Item 5: set the engagement window + reporting channel in `deploy/AUTHORIZATION_TO_TEST.md` + the program pack. (4) Item 7 / Phase 3.2 (parallel, does not depend on the live surface): commission the rebuild estimator using `deploy/REBUILD_ESTIMATOR_BRIEF.md` - Gate 3 is met by the estimator's shipped report, whatever it concludes. (5) Item 6 publish/solicit - CHANNEL DECISION (VL-127): the public site disclaimer + LinkedIn posts (`docs/outreach/linkedin_redteam_posts.md`; fill [SITE URL]), NO bug-bounty platform; post ONLY when the PHASE1 runbook's five gates are all green. G5 NOT-MET.
...
Known items open but not scheduled (non-blocking; the entire artifact-13 build is done. The RECURRING process-finding patterns (chat-paste-eats-content, typographic drift, source-first, set-exhaustiveness, environment-hermeticity) were captured as Lessons 1-12 in an internal session-mechanics methodology file that was REMOVED from the public repo on 2026-07-05 (AI-session-mechanics process residue, not project substance); the underlying findings remain recorded in their originating verification-ledger entries. What remains below is the residual INFRASTRUCTURAL backlog not covered by a lesson):
...
- VL-012 latent inconsistency on `receipt.py` `canonical_json`.
  RESOLVED at VL-143 (2026-07-11): receipt.py reuses envelope.canonical_json.
...
- VL-020 follow-up process finding: third instance of the
  ...
  items" require assistant-side discipline in *generating* the
...
- **Site built out + truncation repaired (VL-131).** The VL-130 conversion kit is folded into `site/index.html`: a "Break it in 60 seconds" quickstart section (live-`3343e32` curl walkthrough), inspector self-adjudication, a Wall-of-Fame/CTF board, a why-trust-this honesty block, a footer, and nav links. This also REPAIRED a pre-existing truncation (the source had been committed cut-off mid-word since >=`40fb5e6`, missing its submit-list tail, closing tags, and footer) - the document now closes `</body></html>` with every tag balanced. `site/index.html` is the repo-canonical source; the live WordPress site is separate and must be edited there. Recruiting material, NOT external validation; G5 NOT-MET.

- **G5 conversion kit authored (VL-130).** Seven paste-ready recruiting assets to lower the friction + suspicion barriers to a blind external red-teamer: a 60-second live-surface curl quickstart (wire shapes matched to the deployed 3343e32 manifest), the inspector as a one-command self-adjudicator, a wall-of-fame/CTF board, a target list + referral cold-mail, audience-tailored pitches (CTF/AI-safety/early-career), a break-it-week event plan, and an honesty-as-credibility trust block. The attacking surface is public now; the credited engagement stays invite-only + Authorization-to-Test (sign-off in hand). Suite independently re-verified 512 passed. Recruiting material, NOT external validation; G5 NOT-MET until a blind party engages.

- **Live surface VERIFIED current 2026-07-03 (VL-128).** All four public nodes up; Let's Encrypt certs valid to 2026-09-14; signed record FRESH on key `pub-2026-06-18` (~5-min re-sign cycle); VL-083 live attack suite GREEN version-matched at the deployed commit `3343e32` (positive control honored + 6/6 attacks refused over real TLS, exit 0). Author self-test / currency check - NOT external validation, NOT a G5 referent; G5 NOT-MET. Reminder: pre-exposure hard gates (counsel safe-harbor) unmet - no responder on the surface yet.

- **Recruiting channel re-alignment (VL-127).** Author decision: solicitation = the public site's disclaimer/engagement block + LinkedIn posts; NO bug-bounty platform (YesWeHack retired VL-125; HackerOne/Bugcrowd/Intigriti explicitly not used). `deploy/RED_TEAM_OUTREACH.md` rewritten platform-free and cash-free (also repairing its committed mid-word truncation); `deploy/BREAK_IT.md` intake line platform-free; cheatsheet broad-reach leads with LinkedIn. NEW `docs/outreach/linkedin_redteam_posts.md` (three paste-ready drafts, honest-scope line, publishing-GATED banner, no-coaching discipline). Assets + a decision only - nothing posted, hard gates unmet, G5 NOT-MET; suite unaffected.
- **Close-repair + Phase-3.2 commissioning brief (VL-126).** The 2026-07-03 native session (40fb5e6) closed without STATE/ledger and left the tree dirty; repaired this session. The pre-launch pack (`deploy/AUTHORIZATION_TO_TEST.md`, `deploy/SAFE_HARBOR_CLAUSE.md`, `deploy/PHASE1_PRELAUNCH_RUNBOOK.md`) is now COMMITTED with private-invite references and honest STATUS annotations (it was referenced by the committed program pack but untracked); `docs/design/governance_layer_KICKOFF.md` committed; the `docs/COPYRIGHT_HEADER.txt` deletion committed (author decision); 10 session-local files gitignored. NEW `deploy/REBUILD_ESTIMATOR_BRIEF.md` is the Gate-3 / Phase-3.2 commissioning pack (cost question bound to a shipped attempt per VL-057; stake-free, not blind; claim-sheet-bound equivalence target; both verdicts acceptable; verbatim ledgering + recognition). Assets only - Gate 3 NOT MET, G5 NOT-MET; no code/canon/manifest/test change; suite unaffected.
...
- **G0 confirmed (anchor finding).** Canonical CCS (whitepaper sections 12-13)
  ...
  Confirmed by three independent derivations from primary sources: independent models,
...
- **VL-025 G0 build half: canonical CCS implementation via envelope.py (this commit).** `IMPLEMENTATION/envelope.py` lands per
  ...
  instance and first with an AI assistant as executing agent; two-instance
...
- **VL-027 envelope.py import fix (this commit).** One-line fix
  ...
  two test files at `/home/user/work/vl028_archived/`,
...
- **VL-058 follow-up: Lesson 11 promoted - the sandbox-mount file + git mechanics (this entry).** `docs/methodology/session_mechanics_lessons.md` gains Lesson 11: write tracked repo files LF from the Linux side (not the sandbox desktop Write/Edit tools, which emit CRLF on Windows - the VL-058 artifact-12 phantom-modified, resolved by `git checkout --`); and do not drive git over the sandbox mount (it cannot unlink, so locks/temp leak and the index can corrupt - the VL-058 unlink-EPERM, recovered by `git read-tree HEAD`). If git must run over the mount: rename-not-unlink lock clear + read-tree repair + explicit-path staging. Lesson count 10 -> 11. Efficiency move per VL-017a; no code/canon/test/spec change; pytest 203 + 0 xfailed unchanged. Cites VL-058 chain (37f9ab7 / 7894c5d / 31da5ec); does not cite its own hash (VL-012).
...
| Volume | Range | sha256 |
|---|---|---|
| `STATE_archive/vol_001__VL-109_to_VL-145.md` | VL-145 .. VL-109 | `3f42d85bfd187bb00942704225780d411a604b6e75d87fb31512847c300d3345` |

Manifest + reconstruction check: `STATE_archive/INDEX.md`.
