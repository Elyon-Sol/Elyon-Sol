...
Last updated: 2026-06-15 (VL-105: the ext-authz SIDECAR gains a TLS test path - in-sandbox referents + a two-VM manual runbook (extends the VL-104 track to the C2 every-hop-TLS posture). deploy/tls/gen_certs.py now emits an elyon-authz leaf; TESTS/deploy/test_authz_sidecar_tls.py (4) is the hermetic CI-safe referent (a strict in-memory TLS handshake verifies the leaf under the dev CA, refuses a wrong CA); EVIDENCE/proofs/authz_sidecar_tls_001_runner.py serves the PRODUCTION sidecar on a REAL loopback uvicorn+TLS socket and gets ALLOW(200)/DENY(403 signature) over an HTTPS client verifying the CA (exits 0; CI-excluded like g5_multiprocess_tls, skip case added). deploy/docker-compose.authz.tls.yml is the TLS overlay (uvicorn --ssl-* + the Envoy upstream-TLS snippet); deploy/elyon-authz/VM_TLS_TEST.md is the executable two-VM manual test (gen certs -> bootstrap -> stand up the sidecar under TLS on VM-B -> mint from VM-A's gate -> present over HTTPS for ALLOW, with forged/rebind/replay/un-attested DENY). Suite 387 -> 391; build-then-wire (the sidecar decision path + every default path byte-unchanged; gen_certs/ci edits are deploy/CI tooling). Honest scope: real cross-host TLS, NOT an external attacker - G5 unchanged. Does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-15 (VL-104: the OPA ext-authz admissibility SIDECAR is built and verified in-container - a deliberately-inserted in-house DERIVATIVE track per docs/design/opa_sidecar_kickoff.md, NOT a G5 road item. IMPLEMENTATION/authz_sidecar.py is a thin FastAPI ext-authz service ("elyon-authz") wrapping the SHIPPED ExecutorGate (VL-078): it answers ALLOW(200)/DENY(403 + x-elyon-reason: REF_*) over the X-Elyon-Sol-Envelope + X-Elyon-Sol-Interaction header contract - the reference_target consume-path (VL-061) refactored from "a target that ACTS" into "an authorizer that ANSWERS allow/deny" so it can sit as an Envoy ext_authz filter in front of ANY target. REUSE-only: no admissibility logic and no crypto re-implemented (imports verify_envelope + the VL-076 ReplayCache seam), no new refusal code (the two sidecar reasons are the existing REF_TARGET_*; every other DENY is the gate's REF_VERIFY_* surfaced unchanged), no new canonical invariant. TESTS/adversarial/test_authz_sidecar.py (15) covers allow + deny-on-each-REF_*-class + fail-closed-on-bad/absent-config + replay shared across two sidecar instances + the Envoy-forwarded suffix path; suite 372 -> 387. deploy/ carries the Mode A two-filter example (envoy.example.yaml - elyon-authz FIRST then OPA, failure_mode_allow FALSE on both - + docker-compose.authz.yml + opa-policy.example.rego + elyon-authz/README.md); the container stand-up + envoy --mode validate are UNVALIDATED in-sandbox (no docker/envoy) and are the author's locus, parity with C1/C2. Build-then-wire: git diff over tracked files empty, nothing imports the module. The "upstream of OPA" claim is now shipped, not just positioned. G5 remains the only open ROAD item. Does not cite its own hash (VL-012).)
