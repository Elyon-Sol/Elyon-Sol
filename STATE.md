...
Last updated: 2026-06-08 (VL-064 governance/relicense: the repository `LICENSE` changed from MIT to PROPRIETARY (all rights reserved; commit 47926cf) and the README rights section was updated to match. Rationale: the MIT grant authorized anyone to use/modify/SELL the code, defeating the productization goal; this repo and all future releases are now proprietary. Honest-history clause: prior PUBLIC MIT releases remain MIT-licensed for those specific released copies (the grant cannot be retroactively revoked); this file governs this repo and subsequent releases. 'Elyon-Sol' trademark application pending. Per-file copyright headers were deliberately NOT added: `IMPLEMENTATION/evaluator.py` is hashed into `EVIDENCE/published_hashes.json` (evaluator_sha256) and every envelope reassertion, so a header would roll the pin and break the verification chain + suite; a root `LICENSE` covers the repo without touching any hashed file. No code/canon/MANIFEST/SPEC/evaluator/published_*/readiness change; no new invariant; pytest 214 unchanged. Next open action unchanged (artifact 12 step 5). Classification: governance per VL-017a. Cites relicense commit 47926cf; does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-08 (VL-063 T-G5-transport: the multi-process + real-TLS chain lands (artifact 12 steps 2-3, in-env). Gate (`pep`), reference target (`reference_target`), and a new standing publisher (`IMPLEMENTATION/publisher.py`) run as THREE separate OS processes over real TLS with a local test CA; the gate->target forward and the target->publisher fetch are CA-verified real sockets (the VL-058/060 transport seam resolves `verify` from `ELYON_TLS_CA_BUNDLE`). `reference_target` gains a read-only `/received` observability endpoint so the honor verdict is read over the wire (no capture/redeliver). `EVIDENCE/proofs/g5_multiprocess_tls_001_runner.py` generates the CA+certs, starts the three uvicorn TLS processes, drives honor THROUGH the gate (gate->target->publisher; target acts exactly once via /received) and forge / replay / target_url-swap / absent-envelope refusals DIRECT-to-target, exit 0. New pytest `TESTS/adversarial/test_publisher.py` (2) + a `/received` test; suite 211 -> 214. No canon/MANIFEST/SPEC/evaluator/published_*/readiness change; no new invariant (canon section 14: transport is verification I/O). G5 stays OPEN: single host, no docker, no external attacker - docker-compose / two-VM / real-CA are deploy artifacts and finish line (B) is author-arranged (GR-3 bounds in-loop attack to characterization). Next: step 5 (attack harness -> claim sheet). Classification: trajectory move per VL-017a. Cites the VL-063 build files; does not cite its own hash (VL-012).)
...
**The in-env G5 build is complete through step 4: the reference enforcing target (VL-061) and the multi-process + real-TLS chain (VL-063) both land.** Gate, reference target, and publisher now run as three separate OS processes over real TLS (local test CA) with CA-verified gate->target and target->publisher hops; `EVIDENCE/proofs/g5_multiprocess_tls_001_runner.py` proves honor over the real chain (observed via the target's `/received`) plus forge / replay / target_url-swap / absent-envelope refusals direct-to-target, exit 0. This realizes `docs/restructure/12_g5_transport_design.md` steps 2 and 3 at single-host fidelity (separate processes + TLS/cert + out-of-band anchor/key pinning). The remaining G5 work is: **step 5** - the attack harness mapped to `docs/methodology/external_verification_readiness.md` gate 2 (forge / replay / swap / revoked / expired / retired-root / mint-despite-divergent-disk over real transport, plus the honestly-open A3b stale-record attack), the next in-loop buildable; and the **deploy-target artifacts** - a docker-compose (docker is absent from the build sandbox, validated only on real hosts) and a two-real-VM promotion note with real-CA certs. Finish line **(B)** - an EXTERNAL attacker on a real surface - closes G5 and is the author's to arrange; GR-3 bounds any in-loop attack to characterization, never certification. Standing, none blocking: the deposit-readiness audit (VL-059, reserved) under GR-3; the now-safe `target.py` stub removal (the chain runs the real reference target); A1 target-side policy and caller-carry / proxy-removal as the section-14 forks; T-bookkeeping (G1/G8/G9/G11/G14 + `server.py` retirement) and the T-prose-drift on the stale numbered list below.
