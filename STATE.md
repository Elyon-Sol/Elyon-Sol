...
Last updated: 2026-06-09 (VL-073 follow-up A5: the first real CI run made green. The CI run (Python 3.13, GitHub ubuntu) that VL-073 enabled failed at `g5_multiprocess_tls_001_runner.py` (`SERVICES NOT READY`, exit 2): its three uvicorn+TLS subprocesses did not come up inside the ~40s wait window on the slower runner, with stderr discarded. Hardened the runner (time-based 150s readiness budget; early `poll()` dead-process detection; per-service stdout+stderr captured and dumped on failure; tz-aware `datetime.now(UTC)` replacing deprecated `utcnow()`), and changed `.github/workflows/ci.yml` so the runner step runs every hermetic runner and reports all failures in one job (instead of stopping at the first). Verified in-sandbox (Python 3.10): suite 218; all 13 hermetic runners pass under the new loop. The 3.13 CI run is the author's referent (locus AUTHOR); G8's CI residual closes on the first green run. Next open action unchanged: A6 (deposit-readiness audit VL-059). No code/canon/test-logic change. Does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-09 (VL-073 A5 (artifact 13, Phase A): CI wired. `.github/workflows/ci.yml` (GitHub Actions) runs the full pytest suite + every exit-coded EVIDENCE/proofs runner on push and pull_request (PYTHONPATH=repo root; `cryptography==44.0.0` pinned); the one non-hermetic runner (`external_interception_webhook_001`, posts to webhook.site) is excluded with a documented reason. Wiring CI surfaced a real defect it then fixed: `g4_refused_bypass_001_runner.py` crashed standalone (`KeyError: json`) because the gate has fail-closed without a signing key since the VL-047 cutover and standalone runners do not load `TESTS/conftest.py`'s autouse key fixture - an ephemeral gate key is now injected (mirrors `default_secure_cutover`); its committed `.log` was also stale (pre-evaluator-roll) and was regenerated against the current published record. Verified in-sandbox (Python 3.10): suite 218 + 0 xfailed; all 13 hermetic runners exit 0; webhook skipped. The workflow pins 3.13 (author env); the first real CI run is the author's to confirm (locus AUTHOR), which is what closes the G8 CI residual. No code/canon/test-logic change. Next open action advances to A6 (deposit-readiness audit VL-059; locus SANDBOX). Classification: trajectory/infra + an evidence-runner repair per VL-017a. Does not cite its own hash (VL-012).)
