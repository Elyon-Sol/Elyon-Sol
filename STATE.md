...
Last updated: 2026-06-09 (VL-073 A5 (artifact 13, Phase A): CI wired. `.github/workflows/ci.yml` (GitHub Actions) runs the full pytest suite + every exit-coded EVIDENCE/proofs runner on push and pull_request (PYTHONPATH=repo root; `cryptography==44.0.0` pinned); the one non-hermetic runner (`external_interception_webhook_001`, posts to webhook.site) is excluded with a documented reason. Wiring CI surfaced a real defect it then fixed: `g4_refused_bypass_001_runner.py` crashed standalone (`KeyError: json`) because the gate has fail-closed without a signing key since the VL-047 cutover and standalone runners do not load `TESTS/conftest.py`'s autouse key fixture - an ephemeral gate key is now injected (mirrors `default_secure_cutover`); its committed `.log` was also stale (pre-evaluator-roll) and was regenerated against the current published record. Verified in-sandbox (Python 3.10): suite 218 + 0 xfailed; all 13 hermetic runners exit 0; webhook skipped. The workflow pins 3.13 (author env); the first real CI run is the author's to confirm (locus AUTHOR), which is what closes the G8 CI residual. No code/canon/test-logic change. Next open action advances to A6 (deposit-readiness audit VL-059; locus SANDBOX). Classification: trajectory/infra + an evidence-runner repair per VL-017a. Does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-09 (VL-072 A4 (artifact 13, Phase A): STATE.md prose-drift cleared. The ~45-item historical numbered "Next open action" build-track log - every item long-since-done, each trailing stale forward "Next:" guidance and the now-false "T-bookkeeping (G1/G8/G9/G11/G14 ...)" label - is retired and replaced by a pointer to the repository's stated provenance layers (`git log --oneline` + `EVIDENCE/verification_ledger.md`). The stale "bookkeeping batch" bullet in "What is locked vs. open" is corrected (G1 VL-052, G11 VL-053, G14 VL-054 RESOLVED; G8/G9 NEAR-CLOSED). A pre-existing mid-sentence EOF truncation - the committed STATE.md ended at "- **G12** - canon section 11.1 u", losing the rest of the G12 bullet and the entire G13 bullet from the "Known open gaps" summary - is repaired by restoring the full G12/G13 bullets and adding the missing G14 bullet, sourced from artifact 04. Doc-only; STATE.md 1527->1153 lines plus the restored tail; no code/canon/test change; suite 218 unchanged. Next open action advances to A5 (CI; locus AUTHOR). Sandbox note: all STATE.md edits + commits done in-sandbox from the committed blob via plumbing, avoiding the mount-read truncation that bit VL-071. Classification: bookkeeping / hygiene per VL-017a. Does not cite its own hash (VL-012).)
