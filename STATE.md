...
Last updated: 2026-06-03 (VL-048 T-end-to-end: the SIGNED cross-host chain runs over real loopback transport via the production fetch path with NO test-only shortcut; END_TO_END_NO_SHORTCUT flips RED->GREEN, the second of three deployment-readiness reds. A WIRING/trajectory move per VL-017a: it composes the VL-047 mandatory signing cutover (the gate's default forward signs) with the VL-039 cross-host transport (a target on a separate process fetches the published record over a real socket and checks currency against the FETCHED record) into one chain with no shortcut. The proof of record is EVIDENCE/proofs/g5_signed_cross_host_001_runner.py: the gate signs on its default path via the PRODUCTION env-var key path (ELYON_SIGNING_KEY_HEX + ELYON_SIGNING_KEY_ID, NOT the conftest in-process injection - that would be a forbidden shortcut), pushes the envelope, and a target SUBPROCESS with a genuinely byte-divergent evaluator.py fetches the published record over a real http.server socket (the production fetch_published_record), then verifies the issuer signature against an out-of-band-pinned public key AND currency-from-the-fetched-record AND interaction binding; it honors the signed-valid envelope despite its divergent disk, and refuses the VL-039-follow-up-2 keyless forge / a tampered envelope / a record that fails the pinned anchor / an absent envelope (exit 0). IMPLEMENTATION/readiness.py::_consistency is narrowed from quantifying over ALL capabilities to the enumerated END_TO_END_CAPABILITIES = (issuer_signing, enforcement_push) - the capabilities the signed chain exercises end-to-end (validate_manifest's honesty check still quantifies over every capability; only the predicate-green consistency narrows). TESTS/readiness/test_deployment_predicates.py::test_end_to_end_no_shortcut drops xfail and wires ANCHOR 2 as the lighter in-process signed-chain regression gate (it drives the production env-var key path, NOT the conftest fixture). EVIDENCE/readiness.json: issuer_signing + enforcement_push exercised_e2e/transported -> true (proof: the runner); END_TO_END_NO_SHORTCUT.green -> true with an A3b-freshness + cross-host-floor scope note; expiry/revocation/rotation stay false (not on the chain). GR-2 formalized in docs/MAINTENANCE_PROTOCOL.md (readiness is test-derived, never human-attested). NO follow-up evaluate: transport plumbing moves no trust boundary (a network adversary reduces to denial/stale/tamper/forge, all already caught); signing's claim-track gate ran at VL-040 follow-up 2, transport's Decision G evaluate at VL-039 follow-up. Honest frame: 2 of 3 green is real progress, NOT "deployed" - green means the full signed chain runs over real transport with no shortcut; it does NOT mean true multi-machine/TLS (the named G5 floor) and does NOT close the A3b freshness sub-class (a stale-but-anchor-matching, validly signed record is still honored). "forgery-resistant" stays BOUNDED (signed-path-under-uncompromised-root) and out of any deposit. pytest 199+2 -> 200+1 xfailed (real env: the END_TO_END xfail becomes a real pass, +1 ANCHOR-2 test body; the ROOT_RECOVERY xfail remains). Spec commit 2b48336 precedes build commit a89c9b1; this STATE+ledger commit does not cite its own hash (VL-012). Next: the last red - ROOT_RECOVERY (the VL-044 rotation primitive wired to pep.py's default path AND run over this session's transport with no shortcut); then A1 target-side policy; caller-carry/proxy-removal; T-bookkeeping (G1/G8/G9/G11/G14 + server.py retirement) and the remaining T-prose-drift (the envelope.py module-docstring lines 42-43 "NOT imported by pep.py" stale post-VL-029/047; the verifier.py Step 1.5 comment; the stale trailing Next-open-action prose). None blocking)
