...
Last updated: 2026-06-09 (VL-091 WIRE B1 - signed published-record freshness onto the reference target (closes A3b sub-case (b) for a configured deployment; the WIRE half of VL-074, parity VL-039 seam -> VL-060 wire; the first built-but-unwired capability drawn off the shelf). `IMPLEMENTATION/reference_target.py` gains a SIGNED consult MODE: when configured with a pinned publisher key (`ELYON_PUBLISHER_KEY_ID`/`_HEX`, opt `ELYON_SIGNED_RECORD_URL`) it fetches the signed record via `fetch_signed_record` (publisher signature + FRESHNESS + serial), refusing a stale/invalid one with `REF_VERIFY_PUBLISHED_RECORD_STALE`/`_INVALID`; absent a publisher key the byte-anchor path is UNCHANGED (every existing runner/test, g4 exit 0). `IMPLEMENTATION/publisher.py` gains `/published_hashes_signed.json` (signs the live pins under a not_after window when `ELYON_PUBLISHER_SIGNING_KEY_HEX` is set; 503 otherwise). `IMPLEMENTATION/published_record_source.py::fetch_signed_record` now uses the TLS-aware transport seam (`get_published`, ELYON_TLS_CA_BUNDLE) so signed mode works over the real cross-host TLS deployment. Spec `docs/restructure/23_published_record_freshness_wiring_spec.md`; live procedure appended to `deploy/host_setup_virtualbox.md` (the stale-record attack). readiness.json `published_record_freshness` blocked_by updated (wired as a configurable mode; bare default stays byte-anchor by choice). Acceptance: `TESTS/adversarial/test_record_freshness_wiring.py` (5) - signed mode honors fresh / refuses STALE / refuses INVALID; byte-anchor unchanged; publisher serves a reader-valid record + 503 without a key. pytest 299 -> 304 + 0 xfailed. Trust-model shift (honest): signed mode pins a PUBLISHER KEY (a load-bearing floor) vs the byte-anchor's record bytes - a deployment chooses. Does not close G5 (external attacker). Does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-09 (VL-090 C4 MET - REAL_TRANSPORT green: the VL-079 attack suite was DEFEATED over real cross-host TLS on the author's two-VM VirtualBox deployment (gate 192.168.56.101 / target+publisher .102, host-only network, dev CA, client = the author's laptop). Result: positive control HONORED (the gate PUSHED the valid call, the target acted) and all 6 adversarial attacks (un-attested / forged / replay / rebind-tool / rebind-args / target_url-swap) REFUSED over real transport, exit 0 - after the three real-surface fixes VL-087 (cert SKI/AKI), VL-088 (swap-URL reachability), VL-089 (push positive control). Proof of record `EVIDENCE/proofs/real_transport_attack_001.log` (the captured author run, commit 8d40171). `EVIDENCE/readiness.json` REAL_TRANSPORT flipped green naming that log; `IMPLEMENTATION/readiness.py` PREDICATE_NAMES gains REAL_TRANSPORT so it is counted -> readiness now **4 of 4 deployment predicates green** (validate_manifest 0 errors, 0 missing proofs, suite 299/0). Artifact-13 C4 acceptance (a GR-2-honest predicate naming a passing real-transport proof) is met. HONEST BOUND carried in the manifest + log: two VMs on one physical host / private network / dev CA / the author's OWN scripted attack = the CROSS-HOST referent, NOT a public surface or a real EXTERNAL attacker; **G5 (GR-3) - a stranger attacking a public deployment - remains OPEN** and is the residual binding NOT-READY reason for an external-validation / production claim. Does not cite its own hash (VL-012).)
