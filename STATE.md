...
**At VL-126 (2026-07-03) - the literal next actions, in order (all AUTHOR-locus):** (0) verify the three sandbox-built commits (685a907 build, a2c9f82 build, the VL-126 close) blob-by-blob natively and `git push origin main` - the sandbox has no credentials, so HEAD is ahead of origin until this lands; if a session resumes before the push, pushing IS the first task (at-rest invariant). (1) Pre-exposure item 3: cert-renewal deploy-hooks on ALL FOUR nodes per `deploy/PHASE1_PRELAUNCH_RUNBOOK.md` Gate 2. (2) Item 4: counsel sign-off on `deploy/SAFE_HARBOR_CLAUSE.md` (HARD GATE before publish). (3) Item 5: set the engagement window + reporting channel in `deploy/AUTHORIZATION_TO_TEST.md` + the program pack. (4) Item 7 / Phase 3.2 (parallel, does not depend on the live surface): commission the rebuild estimator using `deploy/REBUILD_ESTIMATOR_BRIEF.md` - Gate 3 is met by the estimator's shipped report, whatever it concludes. (5) Item 6: publish + first invitation ONLY when the PHASE1 runbook's five gates are all green. G5 NOT-MET.
