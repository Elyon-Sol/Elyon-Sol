...
Last updated: 2026-06-16 (VL-108: G5 Phase 1 EXECUTED - the four-node public surface is LIVE under a real CA and the author self-test is GREEN over it; the REAL_TRANSPORT referent is upgraded from the VirtualBox/dev-CA tier (VL-090) to the real public surface. Four internet-reachable nodes stood up on Hetzner under Let's Encrypt TLS: gate.elyon-sol.io:8443 (Helsinki; holds the private signing key), target.elyon-sol.io:9443 (Hillsboro US-West; primary attack target), pub.elyon-sol.io:9143 (Helsinki; signed published record), authz.elyon-sol.io:9243 (Ashburn US-East; ext-authz sidecar) - two continents, SINGLE carrier (a deliberate POC/red-team-stage choice; multi-carrier deferred). Target + publisher in SIGNED freshness mode (a fresh publisher Ed25519 key; /published_hashes_signed.json under a 300s not_after). attack_suite_live_runner GREEN over the public surface in signed mode: positive control HONORED + 6/6 adversarial (un-attested / forged / replay / rebind-tool / rebind-args / target-swap) refused, exit 0 - proof EVIDENCE/proofs/attack_suite_live_run_2026-06-16.log; readiness REAL_TRANSPORT.proof re-pointed to it. HONEST CEILING: the self-test is the AUTHOR's OWN scripted attack over the 6 gate-2 cases + positive control ONLY; claims 8-13 (incl. the sidecar live-ALLOW path) are in-house-tested, NOT yet live-adversarially-exercised over the public surface; the publisher private key was exposed in the working chat and MUST be regenerated before non-POC; counsel safe-harbor sign-off + bounty/window/reporting-channel remain pending; the attacker pack is NOT published. G5 (GR-3) - a real EXTERNAL stranger on the public surface - remains NOT-MET and the only open ROAD item. Does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-16 (VL-107: G5 Phase-1 forks LOCKED and the recruiting pack made consistent + publish-ready pending counsel - an EXECUTION session, no code/canon/test change, G5 unchanged. Forks: hosts Hetzner + DigitalOcean (two providers, different networks); recruiting via a private time-boxed bug-bounty listing (HackerOne/Bugcrowd/Intigriti); reward a small bounty pool (per-tier amounts TBD); the BREAK_IT.md safe-harbor / authorization clause a HARD GATE - counsel sign-off before publish. Pack repair (a process finding against 24c197a, whose commit message claimed all four attacker-facing docs were consistent): deploy/RED_TEAM_OUTREACH.md still carried [EXAMPLE.COM] + the old ports 8000/9000/9100 - FALSE consistency - corrected to the canonical four hosts (gate 8443 / target 9443 / authz 9243 / pub 9143 at elyon-sol.io); deploy/LIVE_BRINGUP_RUNBOOK.md port-aligned to 8443/9443/9143 + a SIDECAR GAP note (it predates the VL-104/105 sidecar); both were untracked from the VL-106 close and are committed here. deploy/BREAK_IT.md determinable placeholders filled (contact security@elyon-sol.io; 90-day coordinated disclosure, resolving a 30-vs-90 inconsistency; bounty-pool wording); bounty $ amounts / window / RED_TEAM_BRIEFING reporting-channel / counsel clause left as MARKED TODO, not silently filled. Decontamination re-verified clean (Gate 4: zero canon/ledger/VL/cross-model leak in the four attacker-facing docs); self-test env-var contract (attack_suite_live_runner.py) matches the runbook. Nothing went live: the public-surface bring-up, the author self-test over it, the REAL_TRANSPORT flip, publishing, and recruiting all remain AUTHOR-locus; the sandbox cannot provision real hosts/DNS/CA or push. G5 remains NOT-MET and the only open ROAD item. Does not cite its own hash (VL-012).)
