...
Last updated: 2026-05-28 (commit: see `git log` for STATE.md; VL-035 methodology refinement: `docs/methodology/session_mechanics_lessons.md` Lessons 2 and 3 sharpened from the VL-033/VL-034 source-first + terminal-rendering findings; Refined per the evolves-clause, not a new lesson (lesson count unchanged at 8); Lesson 2 gains the VL-034 fragile-anchor third surface event + a corrective that edit/append anchors come from `cat -A`/disk bytes not pasted display (line-wrap variant); Lesson 3 gains three surface events (VL-033 F3, VL-033 F5, VL-034 F3), failure-mode sharpening to cover file identity not only convention-format, a corrective list bullet for uploaded/governing docs, the precondition-not-disposition corrective with the [unread] flag and checklist-as-hard-gate, and a matching self-check; cross-references embedded inline; +4029 bytes; applied via synthetic-fixture apply-script with fixture cleanup on every exit path per VL-034 carry-forward 2; classification: methodology-artifact update, efficiency move per VL-017a; no code/canon/test/spec/structural-doc change; pytest 106 passed + 0 xfailed unchanged from HEAD `1e6fb01`; prior ledger entry VL-034 at commit `1e6fb01`; parent commit `1e6fb01`; next trajectory action: T-G4-design renumbered VL-036)
