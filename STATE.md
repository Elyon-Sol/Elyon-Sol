...
Last updated: 2026-06-02 (VL-041 T-key-lifecycle: issuer-key EXPIRY built, opt-in - `sign_envelope(..., not_after=...)` stamps a signed-region `not_after`; `verify_envelope` enforces `now < not_after` fail-closed (`REF_VERIFY_SIGNATURE_EXPIRED`, canon section 9); pytest 149 -> 160 + 0 xfailed; spec `abdb9e0` + build `807ccfe`. Time-bounds an UNDETECTED key compromise (the VL-040 follow-up 2 decisive failure) without depending on detecting the leak; the highest-value first increment of the key-lifecycle floor. Prior session VL-040 T-signing: issuer signing landed, opt-in. The gate signs the admissibility envelope with an Ed25519 private key (`IMPLEMENTATION/envelope.py::sign_envelope`); a target verifies `issuer_signature` against a pinned public key selected by `issuer_key_id` (`IMPLEMENTATION/verifier.py::verify_envelope(..., pinned_public_keys=...)`) BEFORE `reassert()`, fail-closed (`REF_VERIFY_SIGNATURE_INVALID` / `REF_VERIFY_SIGNATURE_UNKNOWN_KEY`, canon section 9). Closes the VL-039 follow-up 2 forgery finding on the SIGNED path: the three-model from-scratch forge (correct unkeyed `decision_sha256`, no signature) is now refused; the killer demo reproduces it defeated (`EVIDENCE/proofs/signing_forgery_defeated_001.{runner.py,log,md}`). Opt-in (Checkpoint A decision 5a): `pep.py` default forward stays UNSIGNED, the unsigned path is byte-unchanged, and `decision_sha256`/`reassert()` Row 2 are unchanged because the issuer fields are excluded from the hash region; so forgery is closed only where signing is required and the mandatory cutover is the named follow-on. Spec commit `b9ca90a` (artifact 05 'Issuer signature (opt-in)') precedes this build commit per the spec-defines-the-change practice (GR-2 candidate, not formalized). Canon basis: section 8.2 PoE + 11.9 integrity; no new invariant; no new reassertion row (verification is verifier-layer, orthogonal to CCS currency); section 14 identity-agnostic holds (the key proves the ISSUER, not the actors). New dependency `cryptography==44.0.0` (duck-typed: `envelope.py`/`verifier.py` do not import it; only the signing tests + runner do). New `TESTS/adversarial/test_signing.py` (10); pytest 139 -> 149 passed + 0 xfailed (real env and in-container); runner exits 0. CLAIM-TRACK GATE (VL-040 follow-up 2, evaluate-confirmed): the key-governance cross-model evaluate has RUN (Grok + OpenAI + Gemini, 3-0); 'forgery-resistant' is BOUNDED, not settled - defensible only on the signed path, against an adversary lacking the issuer private key, while that key is uncompromised and the pinned public key is authentically distributed. The decisive failure is private-key compromise with NO revocation/recovery path; revocation/compromise-recovery is the load-bearing floor item (B-prime-2, parallel to the B-prime-1 record anchor). The word enters no Zenodo deposit until that floor exists. Prior substantive entry VL-039 at `c964612`; spec commit `b9ca90a`. Next trajectory action: the published signed key record (B-prime-2) carrying revocation + rotation state (expiry landed at VL-041; revocation - the detected-compromise instant-kill case - and the publisher/root key it introduces remain open; that root owes its own framework-level evaluate before 'forgery-resistant' moves further off its bound), then the mandatory signing cutover; A1 target-side policy; T-bookkeeping (G1/G8/G9/G11/G14 + the now-earned `.gitignore` guard for `apply_vl*.py`) and T-prose-drift remain open)
