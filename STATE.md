...
PREVIOUS: 2026-06-15 (VL-106: the OPA sidecar (VL-104/105) claim set is CROSS-MODEL verified at CONFORMANCE scope. A 19-claim verification request (docs/methodology/verification_request_vl106.md) was run against three independent verifiers (verbatim responses committed at EVIDENCE/verification_runs/vl106_{grok,openai,gemini}_response.md): GROK and OPENAI both procedurally clean and unanimous - 18/19 Supported, CA-9 Under-specified, 0 Contradicted (5 + 2 grounds spot-checked against HEAD, all HIT); GEMINI DISCARDED under VL-008 rule (b) for three fabricated citations (claims VERIFY_X509_STRICT absent when it is at test_authz_sidecar_tls.py lines 62/72/73; cites ssl.create_default_context / ssl.Purpose.CLIENT_AUTH, neither in the file; cites a test test_cross_instance_replay_with_shared_cache that does not exist), its lone CD-2 reframing refuted on the merits - the second consecutive Gemini fabrication in this project (VL-102 was the first). The 18 Supported claims transition VL-104/105 RECORDED single-source -> CONFIRMED at conformance scope (faithful-to-design); CA-9 (no unattached importer) stays a named gap for the cross-model record - both clean runs correctly under-specified it - with the property itself verified by adjudicator grep (zero importers) but NOT cross-model-confirmed. This is INTERNAL CONFORMANCE evidence, FORBIDDEN to show a blind external reviewer as validation (VL-057 / ext-readiness gate 4); G5 remains NOT-MET and the only open ROAD item. Does not cite its own hash (VL-012).)
