...
Last updated: 2026-06-06 (VL-058 T-G5-transport: the G5 real-transport design artifact (docs/restructure/12_g5_transport_design.md) landed plus the first build increment - the transport-config seam IMPLEMENTATION/transport.py (post_to_target / get_published; the TLS verify policy + an optional client cert resolved from args or the ELYON_TLS_* env, fail-closed; the load-bearing property is that with no args/env the default request is BYTE-IDENTICAL to the current direct requests.post / requests.get calls, so a later wiring step changes pep.py / published_source.py without changing default behavior). Build-then-wire (VL-025 / VL-037 precedent): the seam has NO callers; pep.py and published_source.py are byte-unchanged. Proven referent-bound by EVIDENCE/proofs/g5_transport_seam_001_runner.py (real TLS between distinct OS processes, no monkeypatch - contrast the VL-048 runner's fake_post; byte-identical default resolution + push (header+body intact) + fetch (record bytes intact) + fail-closed SSLError on an untrusted self-signed peer; 7/7, exit 0). An env-capability spike confirmed the substrate (real TLS multi-process, fail-closed cert verification) and found docker NOT available in the build sandbox, so artifact 12 step 2's compose is a deploy-target artifact, not greened in-env. G5 stays OPEN (the seam is build-then-wire with no caller; capability unchanged until wired); the design names the (A) buildable / (B) external-attacker split and GR-3 bounds any in-loop attack to characterization, never certification. No canon/MANIFEST/SPEC/published_* change; no evaluator_sha256 roll; no readiness.json change; no new invariant (canon section 14); NO follow-up evaluate (GR-3). The reserved deposit-readiness audit renumbers VL-058 -> VL-059. pytest 203 + 0 xfailed unchanged. Cites design 37f9ab7 + build 7894c5d; does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-06 (VL-057 second follow-up: the convergence demotion completed. VL-057 demoted 4 cross-model convergence verdicts from evidence to framing stress-test; a pre-push grep of the ledger found 5 MORE that used convergence as confirmation and were missed - VL-025 follow-up (envelope.py vs spec/canon), VL-031 (the Q1-Q4 convergent table), VL-036 (P1 convergent on the enforcement design), VL-039 follow-up (G5 trust-reduction PARTIAL), VL-040 follow-up 1 (forgery re-probe convergent NO). All 5 are now demoted, completing the set to 9. The referent for soundness/value comes from OUTSIDE the authored loop (external attack on a live surface; a stake-free rebuild) - never from the IDE, the tests, the canon, or another model reading the framing; execution facts (incl. IDE tests) CHARACTERIZE behavior but do not certify soundness. Two bounding notes recorded: (a) downstream citations of any demoted verdict inherit the demotion (named, not chased); (b) VL-008 is bounded by GR-3 - valid as a procedural-cleanliness check, never sufficient to make an evaluative verdict evidence. The product arc this clarifies: dev + dev-side verification is done; QA = external validation has not started and is blocked on the G5 real-transport build (a stranger cannot attack a loopback simulation). NOT demoted by deliberate scope: VL-039 follow-up 2 (referent-bound forgery finding) and VL-024 (a derivation, not a cross-model run). No code/canon/MANIFEST/SPEC/published_* change; pytest 203 + 0 xfailed unchanged. Cites VL-057 b1330cd; does not cite its own hash (VL-012).)
