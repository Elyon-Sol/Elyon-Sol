...
Last updated: 2026-06-05 (VL-050 T-prose-drift + prose-bookkeeping consolidation: a post-3-of-3 narrative refresh, no capability or trajectory advance, no follow-up evaluate, board unchanged at 3 of 3 deployment predicates green. envelope.py module-docstring lines 42-43 corrected (pep.py DOES import build_envelope per VL-029 and sign_envelope per VL-047; evaluator.py still does not import envelope.py - the one-sided boundary holds). verifier.py Step 1.5 comment corrected (the signature step runs on the pinned-key OR key_record_view path, not 'only when pinned_public_keys is supplied' - the VL-042 record path). STATE.md stale trailing 'Next open action' prose below item 44 removed (the orphaned item-4 fragment + the VL-020-era 'Suggested next move' + 'Decisions parked', all long-resolved; the 'Known items open but not scheduled' subsection LEFT intact per Checkpoint A - a closure-prune is a separate VL-033-style pass). Current-verified-state bullets backfilled for VL-039 (G5 transport + the folded post-VL-038 audit), VL-048 (signed cross-host chain, 2 of 3), and VL-049 (root-recovery wire, 3 of 3). Findings: server.py retirement HALTED out of scope at Checkpoint B - its deletion reverses a documented keep decision (ledger ~10351-10409: 'a parallel un-enveloped gate, named not retired') and forces an analysis rewrite of docs/restructure/01_repository_structure.md (66/152/165), so it is rescheduled as its own increment (delete-vs-deprecate decided there); the non-.py grep confirmed no live caller, only the two artifact-01 doc references + README:478 tree line. The .gitignore scratch guard (apply_vl*.py / vl*_commit_msg.txt / vl*_ledger_entry.md) and the G1 README test-count source-of-truth were both found ALREADY present on disk (the .gitignore guard since VL-037 f/u + VL-042; README lines 318-322 already pin STATE.md as the count source and do not hardcode) - opener-vs-disk divergence, the Lesson 3 source-first precondition catching the opener's predictions, the third/fourth instance of the already-present family. G8/G9 evidence-doc notes: stability_proof_001.md already carries a NON-CURRENT header citing G9 truncation, and G8's narrated proofs are superseded by the executable runners (g3 / g5 / root_recovery / signing) - verified near-closed, recorded in the ledger, no proof rewrite. Single prose commit + this STATE + ledger append (Decision 1 / Decision 4 single VL-050 entry); this commit does not cite its own hash (VL-012). pytest 201 + 0 xfailed unchanged.)
