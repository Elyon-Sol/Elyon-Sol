...
Last updated: 2026-05-30 (VL-038 T-G4-enforce: the gate now enforces. `pep.py` pushes the envelope on the ELIGIBLE forward as the `X-Elyon-Sol-Envelope` header (body unchanged); an enforcing target verifies a delivered envelope against the committed published record `EVIDENCE/published_hashes.json` (Decision C: the published hashes, not the target's local disk), reusing `verify_envelope()` as-is. Refused-bypass evidence at `EVIDENCE/proofs/g4_refused_bypass_001.{log,md}`: 1 routed call honored and acted, 5 refused (403, not acted; A1 direct, A2 forged, A3 replay/target_url, and published-record-mismatch). New `TESTS/adversarial/test_enforcement.py` (7); `TESTS/test_pep.py` migrated to the push wire shape (still 5); `EVIDENCE/published_hashes.json` + generator added; pytest 119 -> 126 passed + 0 xfailed. G4 defensibly non-bypassable for routed-and-attested traffic; NOT blanket RESOLVED  -  A1 (target-policy) and cross-host TRANSPORT of the published record (= the G5 hardening) remain, named not built. Prior substantive entry VL-037 at `a959680`, VL-037 follow-up at `251b44b`; HEAD `15c53cb` (post-VL-038 audit disposition: `test_findings_001.py` (5 characterization tests pinning F1-F5) + audit methodology + the VL-039 G5 opener; 126 -> 131; capability unchanged from VL-038); next trajectory action: G5 cross-host transport and/or A1 target-side policy)
