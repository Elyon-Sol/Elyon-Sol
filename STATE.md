...
Last updated: 2026-06-09 (VL-079 C3 (artifact 13, Phase C): attack harness + falsifiable claim sheet (the gate-2 deliverable of external_verification_readiness.md). New `EVIDENCE/proofs/attack_harness.py`: a pluggable `Surface` (admit a decision; attempt a tool call -> honored/reason), an `InProcessSurface` (over the VL-078 ExecutorGate), an `HttpSurface` (over a real reference-target URL - the AUTHOR adapter for a real cross-host surface) + a `RequestsClient`, and `run_suite` running 8 gate-2 break-it challenges (unattested/A1, forged-signature, replay, rebind-tool, rebind-args, target_url-swap, drifted-state, stale) plus a positive control; an attack PASSES when DEFEATED with the expected REF_* reason. Runner `EVIDENCE/proofs/attack_suite_001_runner.py` (+ committed `attack_suite_001.log`): 8/8 attacks defeated + positive control honored on the in-process surface, exit 0. Claim sheet `docs/methodology/falsifiable_claim_sheet.md`: every deposit-audit (VL-059) Section-A/B claim restated as a break-it challenge mapped to its attack id + defender + honest status (DEFENDED-in-process; the key-record/root attacks pointed at their existing defenders; the Section-C named-open boundaries - A1, G5, A3b, root-compromise, rebuild-cost - listed as limits, not defended). Spec `docs/restructure/19_attack_harness_and_claim_sheet_spec.md`. HONEST CEILING (gate 2): a green in-process run proves the attacks are well-formed and the gate refuses them; it is NOT external validation - referent-incomplete until gate 1 (real cross-host transport, C1/C2); the HttpSurface adapter is shape-tested against a real reference target (HTTP) and is ready to run the SAME suite over real transport. Acceptance: TESTS/adversarial/test_attack_harness.py (2) - the full suite defeated in-process + the HttpSurface adapter driving a real reference target. pytest 284 -> 286 + 0 xfailed (live in-sandbox, Python 3.10). Build-then-wire: new files only; IMPLEMENTATION/* and published_hashes byte-unchanged -> no evaluator_sha256 roll. Next: the remaining Phase-C work is AUTHOR-locus (C1/C2 real hosts, then C3 live-run, then C4). Does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-09 (VL-078 B5 (artifact 13, Phase B - FINAL): latency budget + executor SDK. New `IMPLEMENTATION/executor_sdk.py`: a thin `ExecutorGate` that factors the executor sequence (load + anchor-verify record -> `verify_envelope` -> VL-076 replay de-dup) into one component returning `Decision(honored, reason)`, so an integrator wires admission in a few lines (construct the gate once with pinned keys + target_id + the published record; call `.check(envelope, interaction)` per request; act only if honored). No gate logic re-implemented; fail-closed on every undecidable path; second consumer pattern for the VL-076 seam. Spec `docs/restructure/18_latency_budget_and_sdk_spec.md`. Harness `EVIDENCE/proofs/latency_budget_001_runner.py` (+ committed `latency_budget_001.log`) measures p50/p95/p99 + throughput of the admit and verify paths; INDICATIVE sandbox figures: VERIFY (executor added per-call cost) p50 ~0.14 ms / p99 ~0.29 ms / ~6800 calls/s; ADMIT measured through the TestClient HTTP boundary (harness-inflated). Locus: AUTHOR re-runs on representative hardware for the budget of record; network/TLS cost NOT included (Phase C). Acceptance: TESTS/adversarial/test_executor_sdk.py (10) reproduces the executor refusal matrix via the SDK + the shared-replay-cache cross-instance case; the harness runs exit 0 (loose p50-verify<50ms sanity bound). pytest 274 -> 284 + 0 xfailed (live in-sandbox, Python 3.10). Build-then-wire: NO default-path caller; evaluator.py / pep.py / verifier.py / envelope.py / reference_target.py / mcp_server.py / replay_cache.py / published_hashes.json byte-unchanged -> no evaluator_sha256 roll. **Phase B (harden the wedge) is now fully walked B1-B5.** Next open action advances to Phase C - C1 (deploy packaging; locus AUTHOR). Does not cite its own hash (VL-012).)
