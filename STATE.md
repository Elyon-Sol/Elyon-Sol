...
Last updated: 2026-06-09 (VL-082 C2 (artifact 13, Phase C): real TLS/cert + trust bootstrap artifacts authored (locus AUTHOR for a real cross-host TLS run). New `deploy/tls/gen_certs.py` generates a private dev CA + per-service leaf certs (gate/target/publisher, EC P-256, SANs for the compose names + localhost + any real hostnames) using the `cryptography` lib only (no openssl-binary dependency). `deploy/docker-compose.tls.yml` is a compose OVERLAY that serves each service under `uvicorn --ssl-*`, flips the URLs to https, and points the gate/target clients at the CA bundle via the existing `transport.py` `ELYON_TLS_CA_BUNDLE` / `ELYON_TLS_CLIENT_CERT` hooks (no code change). `deploy/tls/trust_bootstrap.md` documents the out-of-band distribution (CA bundle + pinned anchor + gate public key on channels SEPARATE from the record) and the dev-CA vs real/Let's-Encrypt paths. Spec `docs/restructure/21_real_tls_and_trust_bootstrap_spec.md`. HONEST LOCUS SPLIT: the container/TLS stand-up is UNVALIDATED (no docker), but the cert MATERIAL is validated in-sandbox by a REAL TLS handshake - `TESTS/deploy/test_tls_certs.py` (6) drives a server (leaf) + a CA-trusting client to a completed, verified handshake over `ssl.MemoryBIO`, and a wrong-CA client is REFUSED (SSLCertVerificationError, fail-closed); plus chain validation (CA self-signed + CA:TRUE; leaf CA-signed, in-window, SAN) and the overlay structural check (dependency-free, no pyyaml). `deploy/tls/certs/` is git-ignored. pytest 290 -> 296 + 0 xfailed. Build-then-wire: new deploy/tls/ + overlay + test only; IMPLEMENTATION / canon / MANIFEST / published_hashes byte-unchanged -> no evaluator_sha256 roll. Next: C3's LIVE run + C4 (AUTHOR - real hosts). Does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-09 (VL-081 C1 (artifact 13, Phase C): deploy packaging artifacts authored (locus AUTHOR for the stand-up; the build sandbox has no docker). New `deploy/`: a single `Dockerfile` (one image, three uvicorn entrypoints by command override), `docker-compose.yml` wiring `publisher` (:9100) / `target` (:9000) / `gate` (:8000) as networked services via the existing `ELYON_*` env contract (no code change), `bootstrap_config.py` (generates a gate Ed25519 keypair + computes the pinned-root anchor from the committed record, writes a git-ignored `deploy/.env`), and `runbook.md` (single-box / two-real-host / cloud stand-up + a C2 real-TLS preview naming the transport.py hooks). Spec `docs/restructure/20_deploy_packaging_spec.md`. HONEST LOCUS SPLIT: the container layer (Dockerfile + compose) is UNVALIDATED in-sandbox (no docker) - its stand-up is the author's; what IS validated in-sandbox is the CONFIG bootstrap - `TESTS/deploy/test_bootstrap_config.py` (4) proves the generated config is internally consistent AND round-trips (an envelope signed with the bootstrap key, for the bootstrap target_url, is HONORED by verify_envelope against the bootstrap pinned key + the committed record; tampered args refused) - C1's only sandbox-green referent. Compose check is dependency-free (no pyyaml in CI deps). pytest 286 -> 290 + 0 xfailed. Build-then-wire: new deploy/ + TESTS/deploy/ only; IMPLEMENTATION / canon / MANIFEST / published_hashes byte-unchanged -> no evaluator_sha256 roll. Next: C2 (real TLS/cert + trust bootstrap; artifacts authorable in-sandbox, validation AUTHOR). Does not cite its own hash (VL-012).)
