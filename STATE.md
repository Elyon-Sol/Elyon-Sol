...
**(1) POST THE LIVE-SITE CORRECTIONS — the only item with an outward-facing consequence today.** `site/index.html` is corrected and pushed (53f35fc) but it is REPO-CANONICAL; the live elyon-sol.io is a separate artifact (VL-131) and still tells every visitor that Rev 6 is the current Zenodo record, while Rev 7 and Rev 8 are published. Apply in its editor: the concept DOI `10.5281/zenodo.19367848` in three link slots (hero, "Open the record", footer) — this is the fix that stops the class recurring, since a version-specific DOI restales on every publication; "Current (Rev 8)" `10.5281/zenodo.21364720`, published 14 Jul; the version chain through Rev 8; `553` -> `616` in two places; the changelog footnote ("all eight closed", not "two remain tracked"), three added rows (SES-5, SES-6, SES-8), and the new SES-9a subsection. This retires the old item (5) ("elyon-sol.io says 512 tests"), whose own numbers were stale twice over. RE-VERIFIED 2026-07-16 (post-SES-9b): the LIVE elyon-sol.io is UNCHANGED — still Rev 6 / 553 / the false "two items remain tracked" footnote — so the WordPress edit is still pending and pulling FROM the live site would REGRESS the repo. TEST-COUNT CONVENTION adopted this session and documented in an `site/index.html` comment: the page's `tests green` figure tracks the CURRENT PUBLISHED Zenodo revision (Rev 8 = 616), NOT repo HEAD (630 at VL-148) — so every number stays citable to a DOI and the count advances per-publication, not per-commit; do NOT bump it every increment. It moves to the repo-HEAD figure only when a new Enforcement-Evidence Addendum publishes. (Same class-fix as the concept DOI.)
