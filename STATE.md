...
**(3) The GR-6 named-open items, in priority order.** (a) The **GR-5 clause-1 heading break** is the one with teeth: entries VL-135 onward use `## VL-N` while clause 1 specifies volumes are "cut at a primary `### VL-N` header", so the NEXT ledger archiving event would mis-cut. It needs a clause-1 amendment BEFORE that split; normalising the headings in place is forbidden by GR-4 clause 2. Latent, not urgent. (b) **vol_002**: `## Current verified state` is 165,108 bytes and is structurally history (107 bullets, 100 naming a VL, VL-008..VL-131), stale since VL-131; archiving it needs author judgment on which bullets remain TRUE — not a mechanical cut. (c) **Ledger index gap**: VL-132..VL-146 unsummarised, gap marker recorded. (a)-(c) plus Current-verified-state's staleness all begin at 2026-07-03: the close protocol's additive-readability bookkeeping stopped being run. Worth understanding as a class — a protocol step too expensive to execute silently stops being executed, which is exactly the failure GR-6 repaired at the entry point.
