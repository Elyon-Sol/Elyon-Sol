...
Last updated: 2026-06-08 (VL-066 T-G5-continuity + wedge: replay / exactly-once closed, and the wedge property demonstrated end-to-end on an agent tool-call surface. The gate stamps a SIGNED, hash-excluded `decision_id` (uuid) on every default ELIGIBLE forward (`envelope.sign_envelope` + `pep.py`); a new executor-layer `REF_VERIFY_REPLAY` code; and the reference enforcing target gains a TTL-bounded seen-set keyed on `decision_id` (pruned by `not_after`) that refuses an already-honored admission - `verify_envelope` stays pure (anti-replay is the acting party's stateful concern). `decision_id` is in the signed region (tamper-proof) and in `_HASH_EXCLUDED_KEYS` (wire decision hash unchanged). New `EVIDENCE/proofs/wedge_agent_toolcall_001_runner.py` demonstrates, on an MCP-shaped tool-call surface, that a side-effecting tool fires ONLY on a valid admission and is refused (tool does not fire) on replay / un-attested / rebind / drift / stale - 7/7, fired exactly once. Tests: `test_reference_target.py` replay + decision_id-signed (2); `test_pep.py` key-set +`decision_id`. Suite 216 -> 218. Honest open: the seen-set is PER-INSTANCE (multi-instance executors need a SHARED cache, e.g. Redis TTL=max-age, or replay crosses instances); record freshness (A3b sub-case b) still deferred; the demo is in-process (real MCP server / latency / external attacker unproven). No new canon invariant; no hashed-file change (envelope/pep/verifier/reference_target not hashed). Classification: trajectory / continuity + wedge demonstration per VL-017a. Does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-08 (VL-065 T-G5-continuity: decision freshness - A3b sub-case (a) closed. The default ELIGIBLE forward (`IMPLEMENTATION/pep.py`) now stamps a SIGNED `not_after` (decision max-age; `ELYON_DECISION_MAX_AGE_SECONDS`, default 300s), and `verify_envelope` refuses a captured, validly-signed decision presented beyond it (step 1.5b - the proven key-expiry primitive applied to the decision). Canon-safe: verification-layer policy, no new CCS invariant, no `reassert()` change; `not_after` is inside the signature (tamper-proof) and excluded from `decision_sha256`, so the wire decision hash is unchanged and `pep.py` is not a hashed file (no `evaluator_sha256` roll, no published-record regen). New `TESTS/adversarial/test_decision_freshness.py` (2: fresh honored, stale refused); `TESTS/test_pep.py` envelope-key set migrated (+`not_after`). Suite 214 -> 216. Cutover like VL-047 (default-path behavior change). Honest open: replay WITHIN the window (time-window != exactly-once; needs a nonce + stateful verifier), record freshness (A3b sub-case b, cross-host), and cross-host clock-skew tolerance remain. Classification: trajectory / continuity move per VL-017a. Does not cite its own hash (VL-012).)
...
**Replay / exactly-once is closed (VL-066) and the wedge property is demonstrated end-to-end on an agent tool-call surface.** The gate stamps a signed `decision_id`; the reference enforcing target refuses an already-honored admission (`REF_VERIFY_REPLAY`); `EVIDENCE/proofs/wedge_agent_toolcall_001_runner.py` shows a side-effecting tool fires only when admitted and is refused on replay / un-attested / rebind / drift / stale. The falsifiable wedge claim - admitted + exactly-once + bound + drift-invalidated + decision-fresh, executor-verified independently of the caller - now holds in-process end-to-end. Remaining toward a production, externally-validatable wedge: (1) a SHARED replay cache for a horizontally-scaled executor (the per-instance seen-set is single-instance only; a multi-instance deployment needs e.g. Redis TTL=max-age or replay crosses instances); (2) **record freshness** (A3b sub-case b) - the deferred B-prime-1 -> signed-record migration; (3) a REAL MCP-server integration (the demo is in-process) plus a latency/throughput budget and integration ergonomics; (4) the G5 real-transport finish line - artifact 12 **step 5** (attack harness mapped to `docs/methodology/external_verification_readiness.md` gate 2), the deploy-target artifacts (docker-compose, two-real-VM, real-CA), and finish line **(B)** - an EXTERNAL attacker on a real surface - which alone certifies G5 (GR-3 bounds in-loop attack to characterization). Standing, none blocking: deposit-readiness audit (VL-059, reserved); the now-safe `target.py` stub removal; A1 / caller-carry / proxy-removal (section-14 forks); T-bookkeeping (G1/G8/G9/G11/G14 + `server.py` retirement) and the T-prose-drift on the stale numbered list below.
