...
Last updated: 2026-06-09 (VL-074 B1 (artifact 13, Phase B): record freshness - the signed published-record reader. New `IMPLEMENTATION/published_record_source.py` (sibling of key_record_source.py) validates a publisher-signed published record and enforces freshness (now < not_after strict + monotonic serial), refusing a stale record with REF_VERIFY_PUBLISHED_RECORD_STALE and a bad/unknown/unsigned one with REF_VERIFY_PUBLISHED_RECORD_INVALID (new codes in verifier.py). Signer EVIDENCE/published_hashes_signed_gen.py wraps the LIVE published_hashes_gen pins under a stable pinned publisher key (runtime artifact, never committed). Spec docs/restructure/14_published_record_freshness_spec.md. Build-then-wire: the byte-anchor reader published_source.py + the committed published_hashes.json + reassert/verify_envelope/reference_target/pep.py are byte-unchanged (no evaluator_sha256 roll); wiring onto the default consult path is a later increment (parity with VL-039 seam -> VL-060 wire). Acceptance (artifact 13 B1): TESTS/adversarial/test_published_record_freshness.py (10) flips a stale record honored -> refused. pytest 218 -> 228 + 0 xfailed (live in-sandbox). Next open action advances to B2 (cross-host clock-skew tolerance; locus SANDBOX). Does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-09 (VL-059 A6 (artifact 13, Phase A): the deposit-readiness audit recorded. New file docs/methodology/deposit_readiness_audit.md is the GR-3-bound audit of what is deposit-ready vs bounded / named-open, executed at last into the long-reserved VL-059 slot (reserved at VL-058). It sorts every project claim into Section A deposit-ready (each tied to a named referent - the 218/0 suite confirmed live in-sandbox + CI-green at c519f34, the 12 hermetic runners, the 3 green deployment predicates, the resolved gaps G0/G1/G2/G3/G7/G11/G14), Section B bounded (admissible only with the bound in the same breath; "forgery-resistant" stays out of any deposit), Section C named-open (A1 declining-caller; the G5 real-transport floor + external attacker; A3b record freshness; G12/G13 canon halves; the G8 STATE.md auto-regen residual; the rebuild-cost ratio - no referent), Section D forbidden framings (the VL-057-demoted convergence verdicts; "whole canon realized"), closing with an operational deposit gate. A6 complete; Phase A "clean the base" (A1-A6) is now fully walked. No code/canon/SPEC/evaluator/MANIFEST/published_*/test change; suite 218 unchanged. Next open action advances to B1 (record freshness, A3b sub-case b; locus SANDBOX). Does not cite its own hash (VL-012).)
