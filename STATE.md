...
Last updated: 2026-06-09 (VL-085 deploy runbook (operational; no code/claim change): `deploy/host_setup_hyperv.md` - a copy-paste provisioning checklist for the AUTHOR's first cross-host TLS run on two local Hyper-V Linux VMs (VM-A = gate :8000; VM-B = target :9000 + publisher :9100). Covers: Hyper-V enable + Default-Switch NAT, New-VM PowerShell for both VMs, per-VM Docker/git/NTP base setup, bootstrap_config + gen_certs (SANs = both VM IPs), a small `docker-compose.hosts.yml` override (the committed compose is single-box; this points the gate forward + target identity at the real VM-B IP), the per-host `docker compose ... up` split, the `attack_suite_live_runner.py` invocation, the C4 flip-REAL_TRANSPORT-green step, a smoke test, and troubleshooting. HONEST SCOPE: two Hyper-V VMs green REAL_TRANSPORT (the cross-host referent) but are NOT the public-internet / external-attacker referent (one physical box, private virtual net, both ends author-controlled); a real external attacker remains the G5/GR-3 finish line. UNVALIDATED in-sandbox (no Hyper-V/docker); the author's stand-up. No code / canon / test change (suite 298). Does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-09 (VL-084 follow-up 2 - comprehensive read-every-file repo audit (mechanical checks + two subagents reading all of docs/restructure, docs/methodology, SPEC, README, zenodo, protocols). Mechanical pass clean: py_compile all tracked .py OK; suite 298/0; readiness validate 0 errors / 0 missing proofs; canon.lock == sha256(canon.md); all 16 IMPLEMENTATION modules import; no stale deleted-module refs in current source. Four HIGH findings found and FIXED: (1) `deposit_readiness_audit.md` Section A item 1 still said 'pytest suite (218/0)' - a self-contradiction the VL-084 reconciliation missed (it fixed the referent-class counts but not this body line); -> 298/0. (2) `04_current_vs_claimed.md` presented G2 as PARTIALLY-closed / 'proposed VL-017/018/019' in both the G2 section (no RESOLVED status) and priority-order item 4, contradicting STATE's 'G2 RESOLVED'; G2 marked RESOLVED (VL-014..VL-019) in both. (3) `docs/zenodo/enforcement_evidence_addendum_rev3.md` (an UNPUBLISHED draft, no DOI) ended mid-table at '| Supersede'; the Provenance 'Supersedes' row completed (Revision 2, DOI 10.5281/zenodo.20387278, snapshot 89ff2f9). (4) `01_repository_structure.md` is reconciled only to VL-010 (2db1807) and omits the entire Phase A/B/C tree; a STALE NOTICE banner added pointing to git ls-files / STATE / ledger. Verified NON-findings: `IMPLEMENTATION/target.py` is NOT in HEAD (the committed repo is clean; the on-disk copy is a sandbox mount ghost git cannot see); `EVIDENCE/published_keys.json` / `published_roots.json` are correctly runtime-generated / never committed; the 00_README / 12_g5 / README-line-566 staleness is in explicitly pinned snapshot docs (LOW, left as-is - editing a pinned snapshot would be the error). No code/canon/test change (suite 298). Does not cite its own hash (VL-012).)
