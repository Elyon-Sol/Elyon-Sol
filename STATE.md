...
Last updated: 2026-06-09 (VL-095 LIVE: both wired capabilities proven on real cross-host hardware - the shelf drawn down with real-execution proof, not just tests. On the author's VirtualBox deployment (gate .101; target instance A :9000 signed-mode, target2 instance B :9001, one shared Redis, publisher, dev-CA TLS): DEMO 1 (B3 cross-instance, VL-094) - the gate PUSHED a decision to instance A which honored + claimed the decision_id in the SHARED Redis (A acted=1); the SAME decision presented to instance B (a separate process) was refused (False, REF_VERIFY_REPLAY), B acted=0 - cross-instance exactly-once over real transport (a per-instance cache would have honored it twice). DEMO 2 (B1 freshness, VL-091/093 re-confirm) - a stale published record refused (False, REF_VERIFY_PUBLISHED_RECORD_STALE). Proof of record `EVIDENCE/proofs/real_transport_replay_crossinstance_001.log`. readiness.json `shared_replay_cache` exercised_e2e + transported flipped TRUE (naming the log) - B3 has now walked built (VL-076) -> wired (VL-094) -> exercised-e2e + transported (VL-095), matching B1. Readiness stays 4/4 green. Both default-path shelf capabilities (B1 freshness, B3 replay) are now built + wired + proven LIVE. Honest bound unchanged: two VMs / one host / private net / dev CA / author run; G5 (a real EXTERNAL attacker on a public surface) remains the only open item, and it is not code. Does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-09 (VL-094 WIRE B3 - the shared-replay-cache seam onto the reference target (the SECOND built-but-unwired capability drawn off the shelf, after B1/VL-091; the last shelf item with a default-path home). `IMPLEMENTATION/reference_target.py`'s inline per-process `app.state.seen` dict is REPLACED by the VL-076 `ReplayCache` seam (`app.state.replay_cache`): default `InMemoryReplayCache` (byte-behaviour-identical - prune-expired -> membership-refuse -> claim), or an injected SHARED cache for cross-instance exactly-once. `IMPLEMENTATION/replay_cache.py` gains a concrete `RedisReplayStore` (`claim` = Redis `SET key 1 NX EX <ttl>`; `redis` lazy-imported, not a hard dep) + `replay_cache_from_env()` (shared via `ELYON_REPLAY_REDIS_URL`, else InMemory); the module app builds from env. Spec `docs/restructure/24_shared_replay_cache_wiring_spec.md`. `deploy/Dockerfile` adds `redis`; a `deploy/host_setup_virtualbox.md` appendix gives the live cross-instance demo (Redis + a second target on :9001 sharing it; a decision_id honored on instance A refused REF_VERIFY_REPLAY on B). Acceptance: the wired target still refuses same-instance replay (test_reference_target 11/11 + a new cross-instance integration test: two target apps sharing one cache catch the cross-instance replay); RedisReplayStore over a fake client (honor-once / cross-instance / TTL-from-expiry / no-TTL / from_env-default). pytest 304 -> 310 + 0 xfailed. Byte-anchor / single-instance default unchanged (g4/g5 untouched). Honest bound: wired as a configurable shared-store mode; the bare default stays per-instance InMemory; a real 2-instance+Redis cross-host run is the author's; G5 unchanged. Does not cite its own hash (VL-012).)
