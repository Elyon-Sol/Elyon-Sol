...
**Decision freshness (A3b sub-case a) is closed (VL-065): the default ELIGIBLE forward stamps a signed `not_after` (decision max-age) and the verifier refuses a captured decision presented beyond it.** The active track is closing CONTINUITY against staleness/drift end to end - the property a downstream executor needs to trust an admitted action. Next: **record freshness** (A3b sub-case b) - a stale-but-anchor-matching published record can still be honored cross-host; give the published record(s) a signed freshness bound and enforce it on the fetch/verify path, mirroring the VL-065 decision-freshness cutover. Then, if pursued, **replay-within-window** (a captured decision is still honorable inside its max-age; closing it needs a nonce / exactly-once, which makes the verifier stateful - a deliberate, larger change) and **cross-host clock-skew tolerance** for the freshness checks. The prior G5 build is complete through step 4 - the reference enforcing target (VL-061) and the multi-process + real-TLS chain (VL-063, artifact 12 steps 2-3 in-env). Standing G5 transport items (unchanged): artifact 12 **step 5** (attack harness mapped to `docs/methodology/external_verification_readiness.md` gate 2), the deploy-target artifacts (docker-compose, two-real-VM, real-CA certs), and finish line **(B)** - an EXTERNAL attacker on a real surface - which is the author's to arrange and the only thing that certifies G5 (GR-3 bounds in-loop attack to characterization). Standing, none blocking: the deposit-readiness audit (VL-059, reserved) under GR-3; the now-safe `target.py` stub removal; A1 target-side policy and caller-carry / proxy-removal as the section-14 forks; T-bookkeeping (G1/G8/G9/G11/G14 + `server.py` retirement) and the T-prose-drift on the stale numbered list below.
