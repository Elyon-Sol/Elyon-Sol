...
Last updated: 2026-06-04 (VL-049 T-root-recovery-wire: the VL-044 planned-rotation primitive is consulted TARGET-SIDE over the VL-048 signed cross-host transport with no test-only shortcut; ROOT_RECOVERY flips RED->GREEN, the LAST of three deployment-readiness reds (3 of 3 green). A WIRING/trajectory move per VL-017a: reading (A) at Checkpoint A - the gate's default forward (pep.py) is UNCHANGED; rotation is target-side posture consulted on the live chain the forward drives, parallel to END_TO_END's currency-from-record. Proof of record EVIDENCE/proofs/root_recovery_cross_host_001_runner.py: a target pinning ONLY R1 (out-of-band, never re-pinned) on a genuinely byte-divergent disk fetches the published+root+key records over real loopback sockets via the production fetch path, builds the root-status view (R1 designates R2) and the key-record trust view (key record signed by the designated-active R2 vouching the gate's issuer key), and HONORS the gate-signed envelope - a planned in-band R1->R2 rotation with no re-pin - while refusing the keyless forge / revoked-root / retired-root / fetch-failure / stale-record cases (exit 0; KILLER PROPERTY holds: honored despite divergent disk while a local-disk verify would refuse). Checkpoint C decision (b): IMPLEMENTATION/key_record_source.py::fetch_key_record gains an additive root_status_view=None passthrough threaded to the unchanged load_key_record_from_bytes (default None = VL-042 byte-behavior) - the only seam by which the production transport wrapper validates a fetched key record against a root-status view; verify_envelope + the validation logic stay byte-unchanged (constraint g). IMPLEMENTATION/readiness.py: ROOT_RECOVERY_CAPABILITIES = (root_rotation, issuer_key_revocation) + a _consistency ROOT_RECOVERY clause (green requires those two exercised_e2e + transported, mirrors END_TO_END; does NOT require wired_to_default, honest since the gate forward is unchanged; load-bearing - a green-with-typed-false-dependency manifest is caught). EVIDENCE/readiness.json: root_rotation + issuer_key_revocation exercised_e2e/transported -> true (proof: the runner); wired_to_default stays false; ROOT_RECOVERY.green -> true with a scope note; validate_manifest honest; summary 3 of 3 green. TESTS/readiness/test_deployment_predicates.py::test_root_recovery_wired drops xfail and becomes the in-process regression gate (option alpha; production env-var key path, not the conftest fixture; the heavy no-shortcut transport proof is the runner). NO follow-up evaluate (wiring onto transport moves no trust boundary; the transitive-designation evaluate was SOUND 3-0 at VL-044 follow-up). pytest 200 + 1 xfailed -> 201 + 0 xfailed (real env: the ROOT_RECOVERY xfail becomes a real pass; the runner is not pytest-collected). Spec commit 52d3764 precedes build commit 7b0f258; this STATE+ledger commit does not cite its own hash (VL-012). Findings: wrong-repo-via-parent-.git (the spec apply-script run from ~/Downloads walked up to a stray ~/.git and edited nothing useful; corrective = anchor all git/grep at ~/Elyon-Sol, which the build apply-script's absolute REPO_ROOT then did); a mid-session laptop death + machine-image restore (the two spec files survived as unstaged edits, verified against HEAD 7a6cf01 by diff --stat, committed clean to 52d3764, no content lost); an apply-script idempotence bug (the append-style R1 edit had new_str containing old_str as a prefix, so the unique-anchor APPLY branch re-fired on re-run and would have doubled the block; caught by the fixture idempotent re-run; fixed by checking already-applied before the apply branch; VL-048 finding-4 family); an ASCII-sweep coverage gap (the standalone .md proof doc was authored separately from the apply-script's edits and not swept, shipped non-ASCII, caught by the real-env Checkpoint D sweep, fixed by --amend 1ee5674 -> 7b0f258; Lesson 7 stage-2 family, coverage refinement: sweep every deliverable). Honest frame: 3 of 3 green is the finite road the readiness gate named, walked - NOT "deployed" or "secure": out-of-band root/issuer COMPROMISE recovery (irreducibly out-of-band) and true multi-machine/TLS (the named G5 floor) remain; "forgery-resistant" stays BOUNDED (signed-path-under-uncompromised-root) and out of any deposit. Next: A1 target-side admission policy; caller-carry/proxy-removal; T-bookkeeping (G1/G8/G9/G11/G14 + server.py retirement); T-prose-drift (envelope.py module-docstring lines 42-43; verifier.py Step 1.5 comment; the stale trailing Next-open-action prose; backfill the drifted Current-verified-state bullets for VL-038/039/048/049). None blocking)
