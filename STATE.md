...
Last updated: 2026-06-03 (VL-047 T-default-secure mandatory signing cutover: the gate's default forward now SIGNS the envelope, so the secure path IS the only forward path; DEFAULT_SECURE flips RED->GREEN, the first of three deployment-readiness reds. A WIRING/trajectory move per VL-017a, not a new capability - signing was built+evaluated at VL-040/041/042; this puts it on pep.py's default path. pep.py gains a runtime signing-key provider (_get_signing_key; an injected key or the ELYON_SIGNING_KEY_HEX + ELYON_SIGNING_KEY_ID env pair, cryptography imported lazily, the private key never in the repo) and signs INSIDE the envelope-construction try/except, so a missing key FAILS CLOSED (REF_PEP_FAIL_CLOSED), never a downgrade to an unsigned forward (constraint i). The canary test_unsigned_path_unchanged_forge_still_accepted is RETIRED and renamed to test_verifier_unsigned_mode_accepts_forge_non_default (the verifier's unsigned mode legitimately remains for the enforcement/A1-bypass demos; it is just no longer the gate's default). New TESTS/conftest.py autouse fixture injects an ephemeral keypair into the provider (private in-process only); test_pep.py gains test_default_path_is_signed_and_forge_refused (canary replacement + wired_to_default proof) + test_default_forward_no_key_fails_closed; the DEFAULT_SECURE predicate drops xfail and wires ANCHOR 1; EVIDENCE/readiness.json issuer_signing.wired_to_default->true + DEFAULT_SECURE.green->true with a cross-host-excluded scope note (cross-host transport is END_TO_END_NO_SHORTCUT/G5, NOT asserted here); EVIDENCE/proofs/default_secure_cutover_001_runner.py is the single-process evidence (signs / target honors signed / target refuses forge / no-key fails closed; exit 0). NO follow-up evaluate (no new world-claim; signing's claim-track gate already ran at VL-040 follow-up 2). Honest frame: 1 of 3 green is the START of the road, not "secure"; the cutover closes forgery on what is now the only default path, but the decisive failure - root/issuer key compromise, recovery out-of-band - is unchanged, so "forgery-resistant" stays BOUNDED and out of any deposit. pytest 196+3 -> 199+2 xfailed (real env). Spec commit 82648dd precedes build commit 56fd4f1; this STATE+ledger commit does not cite its own hash (VL-012). Next: the remaining two reds - END_TO_END_NO_SHORTCUT (G5 cross-host transport), ROOT_RECOVERY (VL-044 rotation wired to the default path + transported); then A1 target-side policy; caller-carry/proxy-removal; T-bookkeeping (G1/G8/G9/G11/G14 + server.py retirement) and the remaining T-prose-drift (the stale trailing Next-open-action prose; the verifier.py Step 1.5 comment). None blocking)
