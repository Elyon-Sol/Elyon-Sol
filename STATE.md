...
Last updated: 2026-05-29 (commit: see `git log` for STATE.md; VL-036 T-G4-design: design artifact `docs/restructure/08_enforcement_design.md` drafted (threat model, G4 sub-questions, mechanism evaluation, section-14 compatibility, G4/G5 boundary at E1, recommended VL-037 increment); design-only per Decision A; no code/canon/manifest/test/spec change; G4 moves open-undesigned -> open-designed, build pending VL-037, NOT resolved; pre-draft cross-model verification of the threat model + mechanism-adequacy premise (T0/P1) convergent on P1, one exhaustiveness criterion-divergence finding resolved by derive-by-construction; P2/P3 source-first; pytest 106 passed + 0 xfailed unchanged; prior ledger entry VL-035 at commit `cdeeb25`; parent commit `cdeeb25`; next trajectory action: VL-037 G4-build)
