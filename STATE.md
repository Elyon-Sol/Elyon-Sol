...
Last updated: 2026-05-28 (commit: see `git log` for STATE.md; VL-034 T-G7-eval trajectory close: `TESTS/adversarial/test_evaluator_canonical.py` added with 22 canon-derived tests for the evaluator domain - 8 AC^3 (canon 11.7), 8 T^26 (canon 11.8), 6 manifest-integrity (canon 11.9 via artifact-05-layer per Decision C); augments (does not replace) the 23 code-derived tests in `TESTS/test_adversarial_evaluator.py` per Decision D; predicate-direct calls (ac3_valid / t26_valid / manifest_integrity_valid) mirror canon section 11's per-clause structure; manifest hashes derived live via manifest_sha256() per constraint (i), no hardcoded hash values; G11 documented in-comment and B-parked per Decision F; Checkpoint B surfaced no halt-class spec gap (all tests direct-canon or Decision-C artifact-05-layer acknowledgment); G7 transitions PARTIALLY ADDRESSED -> RESOLVED, both envelope and evaluator domains now canon-derived; artifact 04 G7 row + priority + 2 nested G0-section refs updated; artifact 06 rows 11.7/11.8/11.9 gain test cross-refs per maintenance rule 3 (no status change); 4 process findings (Lesson 7 stage 2 section-sign leak new character class user-caught; count-anchor-over-source drift Lesson 5 user-caught; source-first near-miss on opener identity Lesson 3 user-caught; verify-on-disk catching the hash-pinning error, positive); classification: trajectory move per VL-017a's distinction; no code/canon/manifest/spec change; pytest 84 -> 106 passed + 0 xfailed; prior ledger entry VL-033 at commit `5e2fab0`; parent commit `5e2fab0`; next trajectory action: T-bookkeeping or T-prose-drift or G4 build-outward)
