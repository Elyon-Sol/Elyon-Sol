...
Last updated: 2026-06-15 (VL-104: the OPA ext-authz admissibility SIDECAR is built and verified in-container - a deliberately-inserted in-house DERIVATIVE track per docs/design/opa_sidecar_kickoff.md, NOT a G5 road item. IMPLEMENTATION/authz_sidecar.py is a thin FastAPI ext-authz service ("elyon-authz") wrapping the SHIPPED ExecutorGate (VL-078): it answers ALLOW(200)/DENY(403 + x-elyon-reason: REF_*) over the X-Elyon-Sol-Envelope + X-Elyon-Sol-Interaction header contract - the reference_target consume-path (VL-061) refactored from "a target that ACTS" into "an authorizer that ANSWERS allow/deny" so it can sit as an Envoy ext_authz filter in front of ANY target. REUSE-only: no admissibility logic and no crypto re-implemented (imports verify_envelope + the VL-076 ReplayCache seam), no new refusal code (the two sidecar reasons are the existing REF_TARGET_*; every other DENY is the gate's REF_VERIFY_* surfaced unchanged), no new canonical invariant. TESTS/adversarial/test_authz_sidecar.py (15) covers allow + deny-on-each-REF_*-class + fail-closed-on-bad/absent-config + replay shared across two sidecar instances + the Envoy-forwarded suffix path; suite 372 -> 387. deploy/ carries the Mode A two-filter example (envoy.example.yaml - elyon-authz FIRST then OPA, failure_mode_allow FALSE on both - + docker-compose.authz.yml + opa-policy.example.rego + elyon-authz/README.md); the container stand-up + envoy --mode validate are UNVALIDATED in-sandbox (no docker/envoy) and are the author's locus, parity with C1/C2. Build-then-wire: git diff over tracked files empty, nothing imports the module. The "upstream of OPA" claim is now shipped, not just positioned. G5 remains the only open ROAD item. Does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-10 (VL-103: the EXTERNAL VALIDATION EXECUTION PLAN (G5) is drafted - docs/restructure/29_external_validation_execution_plan.md consolidates the previously-scattered external-readiness pieces (the 5-gate criterion in external_verification_readiness.md, the falsifiable_claim_sheet, the deploy/TLS runbooks, and the artifact-22 live attack runner) into one ordered procedure: Phase 1 public surface bring-up (two real hosts, real DNS, real CA - retiring the dev-CA bound that ceilings every prior live run; author self-test green before exposure), Phase 2 the decontaminated reviewer briefing pack (claim sheet + named floors + the inspector CLI as the shared adjudication tool; convergence verdicts incl. the VL-102 CONFIRMED, canon, ledger, and STATE all FORBIDDEN per ext-readiness gate 4), Phase 3 sourcing the blind attacker (gate 4) and the SEPARATE stake-free rebuild estimator (gate 3), Phase 4 rules of engagement (scope; success = target acts or gate admits against a DEFENDED claim; reaching a named-open boundary is not a finding; DoS conceded by the fail-closed design), Phase 5 intake -> ledger (a confirmed break is first-class progress per the VL-087..092 precedent and becomes a fix or a new named-open boundary; a clean run is recorded bounded by scope+window, never as 'unbreakable'). Operational plan, not code; introduces no new claim; SINGLE-SOURCE. G5 stays NOT-MET and the project NOT-READY for an external-validation claim until Phase 5 produces a real referent. This is the connective tissue between the finished in-house road and the only open item. Does not cite its own hash (VL-012).)
...
**DERIVATIVE TRACK (added VL-104) - OPA ext-authz sidecar: built + in-container green; next is AUTHOR validation.** The in-house sidecar (IMPLEMENTATION/authz_sidecar.py + TESTS/adversarial/test_authz_sidecar.py + the deploy/ Mode A example) is shipped build-then-wire (suite 372 -> 387; no existing default path changed). Its next steps, both AUTHOR-locus: (1) validate the container stand-up (`cd deploy && python bootstrap_config.py && docker compose -f docker-compose.yml -f docker-compose.authz.yml up --build`) + `envoy --mode validate -c deploy/envoy.example.yaml`; (2) bring it to a first OPA-shop design partner. The named next in-house sidecar increment is build-order step 4 (the declarative CUSTOM interaction-mapping format for gate-less deployments; OUT of VL-104 scope per the kickoff). This track is upstream-of-OPA integration BREADTH; it is NOT a G5 referent and does not change G5's status - the road item below is unchanged.
