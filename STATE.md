...
Last updated: 2026-06-08 (VL-061 T-G5-transport: artifact 12 step 4 done - the standalone, deployable reference enforcing target `IMPLEMENTATION/reference_target.py` lands, superseding the 8-line `IMPLEMENTATION/target.py` stub (the "single largest not-built item for (A)", artifact 12 section 3). It promotes the embedded scaffolding (the `TARGET_DRIVER` subprocess string in `g5_signed_cross_host_001_runner.py` and the in-process `_build_cross_host_target_app` / `build_enforcing_target_app` helpers in `test_cross_host.py` / `test_enforcement.py`) into one real service: it resolves out-of-band config from the environment (`ELYON_TARGET_URL` / `ELYON_PUBLISHER_URL` / `ELYON_PINNED_ROOT_SHA256` / `ELYON_GATE_KEY_ID` / `ELYON_GATE_PUBLIC_KEY_HEX`), reads the `X-Elyon-Sol-Envelope` header, fetches + anchor-verifies the published record over a real socket (`fetch_published_record`), and accepts iff `verify_envelope` accepts against the FETCHED record AND the pinned gate signature verifies - the reference policy is NOT authored-to-pass (verify_envelope is the sole acceptance authority). Fail-closed when unconfigured (`REF_TARGET_NOT_CONFIGURED`); A1 (un-attested caller) closed by the target's own policy (`REF_VERIFY_ENVELOPE_ABSENT`); record-anchor failure -> `REF_TARGET_ANCHOR_MISMATCH`. Build-then-wire: NO change to pep.py's default path or the target.py stub (no code importer of target.py). Verified referent-bound: new `TESTS/adversarial/test_reference_target.py` 8/8; full suite 203 -> 211; new `EVIDENCE/proofs/g5_reference_target_001_runner.py` (conftest-free, real publisher socket, real env-config + production signing path) ALL INVARIANTS HOLD (honor + forge + replay + target_url-swap + absent-envelope + record-mismatch + unconfigured; acted exactly once); the named G5 regression runners (`g5_signed_cross_host`, `root_recovery_cross_host`, `g5_transport_seam`, `g5_cross_host`) all exit 0 - no regression. G5 stays OPEN: single-host loopback, the gate->target hop is still modeled (capture/redeliver), real cross-host TLS is steps 2-3, and finish line (B)'s external attacker is author-arranged (GR-3 bounds in-loop attack to characterization). No canon/MANIFEST/SPEC/published_*/readiness change; no evaluator_sha256 roll; no new invariant (canon section 14: the target verifies and acts/refuses; transport is verification I/O); NO follow-up evaluate (GR-3). Cites the artifact-12 design (37f9ab7, VL-058) and names its three new build files; does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-08 (VL-060 T-G5-transport-wire: step 1b done - the VL-058 transport seam (`IMPLEMENTATION/transport.py` `post_to_target` / `get_published`) is now WIRED into `IMPLEMENTATION/pep.py` (the ELIGIBLE upstream forward) and `IMPLEMENTATION/published_source.py` (the published-record fetch), each called with DEFAULT args so the on-the-wire request is BYTE-IDENTICAL to the prior direct `requests.post` / `requests.get` (the seam resolves verify=True, cert=None, timeout=10 by default). The build-then-wire seam (VL-058, no caller) now has its two callers; `pep.py` KEEPS `import requests` (the test/runner monkeypatch surface `pep.requests.post`), `published_source.py` drops its now-unused `import requests`. 17 `fake_post(url, json, timeout, headers=None)` stubs across TESTS/ + EVIDENCE/proofs/ migrated to add `verify=None, cert=None` (the VL-038 `headers`-kwarg-migration precedent); the lone `_fake_post(*args, **kwargs)` needed none. Byte-identity proven by no-change: pytest 203 + 0 xfailed UNCHANGED (no test added/removed), and the two named regression runners (`g5_signed_cross_host_001_runner`, `root_recovery_cross_host_001_runner`) plus `g5_transport_seam_001_runner` exit 0, killer property holds - nothing moved. G5 stays OPEN (single-host loopback/TLS only; true cross-host + external attacker is finish line (B), artifact 12 steps 2-5, author-arranged). No canon/MANIFEST/SPEC/published_*/readiness change; no evaluator_sha256 roll; no new invariant (canon section 14: transport is verification I/O); NO follow-up evaluate (GR-3). Cites build b814ca0; does not cite its own hash (VL-012).)
...
**Step 4 (the reference enforcing target) is DONE (VL-061).** The standalone, deployable target `IMPLEMENTATION/reference_target.py` now supersedes the `target.py` stub: it honors a valid gate-signed routed call and refuses the forge / replay / target_url-swap / absent-envelope / record-mismatch / unconfigured set, configured entirely out-of-band, with the published-record hop over a real socket and `verify_envelope` as the sole acceptance authority (NOT authored-to-pass). The next G5 step is `docs/restructure/12_g5_transport_design.md` **step 2** - run the chain as separate OS processes on separate endpoints over real TLS in-env (gate node A -> the reference target node B -> publisher), with the gate->target hop a REAL socket rather than the currently-modeled capture/redeliver; docker-compose is a deploy-target artifact, not greened in-env (docker is absent from the build sandbox), and a two-real-VM promotion note is part of the deliverable - then **step 3** (TLS/cert + the out-of-band trust bootstrap) and **step 5** (the attack harness mapped to `docs/methodology/external_verification_readiness.md` gate 2, including the honestly-open A3b stale-record attack). Finish line **(B)** - an EXTERNAL attacker on a real surface - closes G5 and is the author's to arrange; GR-3 bounds any in-loop attack to characterization, never certification. Standing, none blocking: the deposit-readiness audit (VL-059, reserved) under GR-3; A1 target-side policy and caller-carry / proxy-removal as the section-14 forks; T-bookkeeping (G1/G8/G9/G11/G14 + `server.py` retirement, and the `target.py` stub removal that step 2's wiring makes safe) and the T-prose-drift on the stale numbered list below.

The historical build-track log below (G0/G2 schema work through the G4/G5
increments) is retained for provenance; its early items are long since done:
...
25. **`docs/restructure/07_continuity_recursion.md`
    drafted and landed (post-G0-build reading-aid artifact).** Done
    (VL-031, this commit). Per VL-023's PARTIAL HOLDS verdict +
    VL-024's STRENGTHENS-bounded-to-layers-B-and-C refinement +
    VL-025 follow-up's convergent confirmation + post-VL-029
    implementation evidence: artifact names the recursive-continuity
    discipline pattern visible at five layers (decision, manifest,
    methodology, session, evaluator-versioning), the request-layer
    non-fit, and the layer A/B/C bounding per VL-024. First pre-draft
    cross-model verification in project history (Grok and OpenAI,
    bundle of 7 files, four questions; both procedurally clean within-
    body per Lesson 6; substantive convergence on all four questions
    including the load-bearing Q4 evaluator-versioning fail-closed
    dissolution per post-VL-029 envelope.py Row 3 explicit fail-close).
    Artifact stats: 19349 bytes, 381 lines, ASCII-clean, md5
    `0ea94e694dfe3725776aaef12a9be412`. Pattern-first structure (s2)
    + inline citations (beta1). Introduces no new invariant, claim,
    or vocabulary; reading-aid track only. Classification: trajectory
    move per VL-017a's distinction. T-07 closes completely.
26. **T-methodology trajectory close: methodology backlog absorbed.**
    Done (VL-032, this commit). Five sub-edits across three methodology
    files: `docs/methodology/apply_script_template.py` synthetic-fixture
    verification docstring section (+2717 bytes); `docs/methodology/session_mechanics_lessons.md`
    Lesson 5 refinement plus new Lessons 7 and 8 (+12064 bytes; 6 to 8
    Lesson count); `docs/methodology/cross_model_evaluate_template.md`
    three revisions: single-instance language removed, Match-criterion
    clarification section added, co-upload format note section added
    (+3703 bytes). 15-plus accumulated process findings from VL-025
    through VL-031 absorbed into durable methodology. Sub-edit-4
    decision: Option B (Lesson 5 refinement) over Option A (new
    Lesson 9) per the file's own "How this file evolves" clause.
    Classification: efficiency move per VL-017a's distinction. The
    next reader of these methodology files sees Lesson 7's two-stage
    typographic-drift discipline, Lesson 8's pre-draft vs post-draft
    cross-model verification distinction, Lesson 5's opener-packaged-
    prediction refinement, the synthetic-fixture verification step's
    load-bearing `cat -A` refinement, the cross_model_evaluate_template's
    Match-criterion discipline, and the co-upload format note - all
    available as discoverable methodology rather than scattered ledger
    findings.
27. **T-citation-currency audit: SESSION_PROTOCOL.md citations
    annotated; STATE.md known-items pruned.** Done (VL-033, parent
    commit `fc15d1b`; ledger + STATE.md follow-up this commit).
    Four-category audit per VL-033 opener. Category A: 6 bracket
    annotations on stale SESSION_PROTOCOL.md citations in VL-023
    + VL-024 (Option B preserves history; +512 bytes ledger; VL-031
    gap candidate 6 closes). Category D: 9 closed items removed
    from STATE.md "Known items open" subsection (T-cite-C
    conservative discipline; -3832 bytes; subsection compacts
    19 -> 10 items; VL-029 gap candidate 2 closes). Categories B
    and C close empty (structural drift-resistance of STATE.md
    item-N citations + ledger append-only line stability).
    Step 0 Decision T-cite-E classified VL-029 gap candidate 1 as
    Type 2 (content drift, not citation drift); new T-prose-drift
    candidate created. Classification: efficiency move per VL-017a's
    distinction. The next reader sees an annotated ledger preserving
    VL-023's historical citations with current-line positions visible
    inline, plus a STATE.md known-items subsection compacted to only
    items lacking citable closure events or explicitly no-action
    records.
28. **T-G7-eval trajectory close: canon-derived tests for the
    evaluator domain; G7 closes completely.** Done (VL-034, this
    commit). `TESTS/adversarial/test_evaluator_canonical.py` adds
    22 canon-derived tests (8 AC^3 / canon 11.7, 8 T^26 / canon
    11.8, 6 manifest-integrity / canon 11.9 via artifact-05-layer
    per Decision C), augmenting the 23 code-derived tests per
    Decision D, predicate-direct to mirror canon section 11's
    per-clause structure. Checkpoint B surfaced no halt-class spec
    gap. G7 transitions PARTIALLY ADDRESSED (VL-028 + VL-029) ->
    RESOLVED (VL-028 + VL-029 + VL-034); both envelope and evaluator
    domains now canon-derived. Artifact 04 G7 row + priority + 2
    nested G0-section refs updated; artifact 06 rows 11.7/11.8/11.9
    gain test cross-refs per maintenance rule 3 (no status change).
    Pytest 84 -> 106 passed + 0 xfailed. Classification: trajectory
    move per VL-017a's distinction. Next: T-bookkeeping /
    T-prose-drift / G4 build-outward, no priority blocker.
29. **T-methodology: source-first precondition landed into Lessons 2
    and 3.** Done (VL-035, this commit).
    `docs/methodology/session_mechanics_lessons.md` refined (not
    expanded): Lesson 2 gains the VL-034 fragile-anchor third surface
    event + the cat-A/disk-bytes anchor corrective (line-wrap variant);
    Lesson 3 gains three surface events (VL-033 F3/F5, VL-034 F3), the
    identity-not-only-format failure-mode sharpening, the
    uploaded/governing-doc corrective bullet, the
    precondition-not-disposition corrective (binary precondition +
    `[unread]` flag + checklist-as-hard-gate), and a self-check. Lesson
    count unchanged at 8; Refined per the evolves-clause. Efficiency
    move per VL-017a. Next: T-G4-design (renumbered VL-036) per
    `vl036_session_opener.md`; T-bookkeeping and T-prose-drift remain
    open.
30. **T-G4-design: non-bypassable enforcement designed; build deferred to VL-037.** Done (VL-036, this commit). `docs/restructure/08_enforcement_design.md` drafted per artifact 05 build-order step 6 and Decision F: threat model (bypass definition grounded in pep.py's no-envelope forward; adversary classes A1-A5 derived by construction over the participant set; non-bypassability canon-permitted-not-mandated per artifact 04 G4), G4 sub-questions Q1-Q8, mechanism evaluation (artifact-05 open question 3; push vs caller-carry vs target-pull), section-14 compatibility (no new invariant; tension pre-exists in pep.py), G4/G5 boundary at E1 (G5 named as the durable-published-hash-source precondition; design stateable without it), and a recommended VL-037 increment (delivery-agnostic target-side verifier reusing `reassert()` + a `request_context` binding check; A1 named as the gate-unreachable floor). Pre-draft cross-model verification (T0/P1) convergent on P1; T0 exhaustiveness upgraded asserted->derived from the cross-model under-determined finding; P2/P3 source-first. G4 moves open-undesigned -> open-designed; build pending VL-037; NOT resolved. Design-only per Decision A; no code/canon/manifest/test/spec change; pytest 106 passed + 0 xfailed unchanged. Next: VL-037 G4-build (implement the recommended increment); T-bookkeeping (G1/G8/G9/G11/G14) and T-prose-drift remain open; G5 is the named G4 dependency and may merge with or precede the G4 build.
31. **T-G4-build: target-side envelope verifier built; delivery wiring deferred to VL-038.** Done (VL-037, this commit). `IMPLEMENTATION/verifier.py` `verify_envelope()` reuses `reassert()` (canon section 13 revalidation) and adds a symmetric `request_context`/`target_url` binding check (canon section 11.1; closes replay A3 per artifact 08 section 7). Canon-derived tests at `TESTS/adversarial/test_verifier.py` (11) plus honest A1-bypass at `TESTS/adversarial/test_bypass.py` (2) per Decision E. First G4 build increment per artifact 08 section 8 step 1; build-then-wire per VL-025 to VL-029. No pep.py/canon/manifest/spec change (Decision A). G5 named as the deployment precondition, A1 named as the gate-unreachable floor (artifact 08 sections 6/4.4). G4 NOT resolved (verifier has no caller; enforcement unchanged). Pytest 106 -> 119 passed + 0 xfailed. Next: VL-038 G4-delivery (decide push vs caller-carry vs target-pull per artifact 08 sections 4.3/9, wire into `pep.py`, migrate `TESTS/test_pep.py` to the delivered wire shape, connect the verifier as the target-side check); G5 (durable verification) is the named cross-host precondition and may merge with or precede VL-038; T-bookkeeping (G1/G8/G9/G11/G14) and T-prose-drift remain open with no priority blocker.
32. **VL-037 follow-up: working scratch files removed; STATE/ledger bookkeeping.** Done (removal at commit `251b44b`; ledger plus STATE in this bookkeeping commit). `git add -A` at the VL-037 close swept `apply_vl037_docs.py`, `vl037_commit_msg.txt`, and `vl037_ledger_entry.md` (the last a duplicate of the appended ledger entry) into `a959680`; `251b44b` removed them via `git rm`, no repo content lost, pytest 119 passed + 0 xfailed unchanged. Recovery via follow-up commit (VL-020/VL-021 precedent; no history rewrite). New session-mechanics finding (scratch-in-repo via `git add -A`); `.gitignore` guard deferred pending source-first read. Next: VL-038 G4-delivery; G5 the named precondition; T-bookkeeping (G1/G8/G9/G11/G14) and T-prose-drift open with no blocker.
33. **T-G4-enforce: defensible refused bypass; push delivery + enforcing target verifying against the published record.** Done (VL-038, this commit). `pep.py` pushes the envelope as the `X-Elyon-Sol-Envelope` header (body unchanged); an enforcing target (`TESTS/adversarial/test_enforcement.py`, 7 tests) verifies it against the committed `EVIDENCE/published_hashes.json` (Decision C) reusing `verify_envelope()` as-is (Decision D). `TESTS/test_pep.py` migrated to the push wire shape (still 5). Refused-bypass evidence at `EVIDENCE/proofs/g4_refused_bypass_001.{log,md}`: 1 honored+acted, 5 refused (403, not acted). No `SPEC`/canon/`verifier.py`/`envelope.py`/`evaluator.py` change (Checkpoint B: GR-2 not triggered; Decision A/D). G4 defensibly non-bypassable for routed-and-attested traffic; NOT blanket RESOLVED. Pytest 119 -> 126 passed + 0 xfailed. Next: G5 cross-host TRANSPORT of the published record (the real remaining hardening); A1 target-side admission policy; caller-carry/proxy-removal (section-14-faithful); T-bookkeeping (G1/G8/G9/G11/G14) and T-prose-drift remain open with no priority blocker.
34. **T-signing: issuer signing built (opt-in); forgery finding closed on the signed path.** Done (VL-040; spec `b9ca90a` + this build commit). `sign_envelope()` (Ed25519, duck-typed) + `verify_envelope(..., pinned_public_keys=...)` with `REF_VERIFY_SIGNATURE_INVALID` / `REF_VERIFY_SIGNATURE_UNKNOWN_KEY` (fail-closed, signature checked before `reassert()`); issuer fields excluded from `decision_sha256`/`reassert()` Row 2 (no-op unsigned). The VL-039 follow-up 2 three-model forge is refused on the signed path (`TESTS/adversarial/test_signing.py` (10); `EVIDENCE/proofs/signing_forgery_defeated_001.{runner.py,log,md}`, exit 0). Docs: artifact 05 (spec commit), artifact 08 section 4.2 A2 tamper-vs-forgery correction, artifact 04 G4, artifact 06 section 8.2 UNIMPLEMENTED -> PARTIAL. Canon 8.2/11.9; no new invariant; no new reassertion row; section 14 holds. New dep `cryptography==44.0.0` (duck-typed). pytest 139 -> 149 + 0 xfailed. Opt-in: `pep.py` default unsigned; forgery closed only where signing is required. **Next: record signing and/or an envelope issuer-key revocation/expiry mechanism (B-prime-2; revocation/compromise-recovery is the evaluate-confirmed load-bearing floor per VL-040 follow-up 2, under the bounded forgery-resistance claim), then the mandatory signing cutover (signature required on `pep.py`'s default forward; the test that flips is `test_unsigned_path_unchanged_forge_still_accepted`); A1 target-side policy; caller-carry/proxy-removal; T-bookkeeping (G1/G8/G9/G11/G14) and T-prose-drift remain open.** (Carry-forward: STATE never received numbered items / current-state bullets for the post-VL-038 audit or VL-039; their narrative lives in the Last-updated line and the ledger; a STATE prose-refresh of the stale trailing 'Next open action' guidance and the 'What is locked vs. open' head is a T-prose-drift candidate.)
35. **T-key-lifecycle (expiry): issuer-key validity window built (opt-in).** Done (VL-041; spec `abdb9e0` + build `807ccfe`). `sign_envelope(..., not_after=...)` stamps a signed-region `not_after` (tamper-proof; excluded from `decision_sha256` per the Checkpoint B decision, so the VL-040 no-op properties hold); `verify_envelope(..., now=None)` enforces `now < not_after` at Step 1.5b, fail-closed to `REF_VERIFY_SIGNATURE_EXPIRED` (canon section 9). `TESTS/adversarial/test_signing_expiry.py` (11) + `EVIDENCE/proofs/signing_expiry_001_runner.py` (exit 0); pytest 149 -> 160 + 0 xfailed (real env). No `pep.py` change (build-then-wire; emitting expiring envelopes is the integrator-posture cutover). Time-bounds an UNDETECTED key compromise (the VL-040 follow-up 2 decisive failure) without depending on detecting the leak. **Next: the published signed key record (B-prime-2) carrying revocation + rotation (the detected-compromise instant-kill case; introduces a publisher/root key that owes its own framework-level evaluate before 'forgery-resistant' moves off its bound), then the mandatory signing cutover (posture knob; the test that flips is `test_unsigned_path_unchanged_forge_still_accepted`); A1 target-side policy; caller-carry/proxy-removal; T-bookkeeping (G1/G8/G9/G11/G14 + `server.py` retirement + the now-earned `.gitignore` guard for `apply_vl*.py`) and T-prose-drift remain open.**
36. **T-key-record (B-prime-2): published signed key record; revocation built (opt-in); the new publisher/root trust floor.** Done (VL-042; spec `c323b65` + build `5e9fbf6`). `EVIDENCE/published_keys_gen.py` (live signer) + `IMPLEMENTATION/key_record_source.py` (new reader; mirrors `published_source.py`: pinned-root signature -> freshness (`not_after` + opt-in `serial`) -> per-key trust view; `REF_VERIFY_KEY_RECORD_INVALID` / `REF_VERIFY_KEY_RECORD_STALE`) + `IMPLEMENTATION/verifier.py::verify_envelope(..., key_record_view=...)` record-exclusive four-way lookup (`REF_VERIFY_KEY_UNKNOWN` / `REF_VERIFY_KEY_REVOKED` / `REF_VERIFY_KEY_OUT_OF_WINDOW`) before the unchanged signature + VL-041 expiry checks. `TESTS/adversarial/test_key_record.py` (15) + `EVIDENCE/proofs/key_record_001_runner.py` (exit 0); pytest 160 -> 175 + 0 xfailed (real env). No `pep.py` change (build-then-wire; consulting the record is target-side posture). Canon 8.2/9/11.9/13; no new invariant; section 14 holds under the narrowed reading (the root is THE trusted identity). Closes the DETECTED-compromise instant-kill case (revocation) VL-041 expiry could not reach. **Next: run the new-trust-root cross-model evaluate (the claim-track gate; `~/elyon-sol-offrecord/new_trust_root_evaluate_prompt.md`, three labs, off-record) and fold it (VL-042 follow-up) - only then may 'forgery-resistant' move, and only as far as the bound permits; the publisher/ROOT key is a new singular trust floor (root compromise is total). Then root rotation process; cross-host transport (G5); the mandatory signing cutover (posture; the test that flips is `test_unsigned_path_unchanged_forge_still_accepted`); A1 target-side policy; caller-carry/proxy-removal; T-bookkeeping (G1/G8/G9/G11/G14 + `server.py` retirement) and T-prose-drift (incl. the verifier.py Step 1.5 'signed-path only' comment now stale) remain open.**
37. **T-readiness (GR-track): the WIRING-track drift gate; machine-checked deployment-readiness, fail-closed on dishonesty.** Done (VL-043; spec `efeb8ba` + build `753e978`). Adds the third axis beyond CAPABILITY and CLAIM: `EVIDENCE/readiness.json` (single source of readiness truth; per-capability `built/wired_to_default/exercised_e2e/transported`, test-backed-or-false-with-reason) + `IMPLEMENTATION/readiness.py` (engine) + `TESTS/readiness/test_readiness.py` (fails the build on a dishonest manifest; demonstrated catching flag-true-without-proof / green-while-unwired / missing-proof-file) + `TESTS/readiness/test_deployment_predicates.py` (DEFAULT_SECURE + END_TO_END_NO_SHORTCUT as declared xfail). pytest 175 -> 178 + 2 xfailed (real env). No admission-path change; no new invariant; NO follow-up evaluate (makes no claim about the world; GR-rule candidate). Honest initial state 0 of 3 green (see `EVIDENCE/readiness.json`) - red by design. **Next: turn the reds green, in order - DEFAULT_SECURE the day of the mandatory signing cutover (the canary `test_unsigned_path_unchanged_forge_still_accepted` flips; the next real deployment progress); END_TO_END_NO_SHORTCUT at real cross-host transport (G5); ROOT_RECOVERY at VL-044 (planned root rotation + per-root status, under this gate's watch). Wire the two predicate ANCHORs to `pep.py`'s default forward + real transport when the cutover/transport work is touched (needs a source read; scaffold-red until then). A1 target-side policy; caller-carry / proxy-removal; T-bookkeeping (G1/G8/G9/G11/G14 + `server.py` retirement + 00_README now owes artifacts 08/09/10) and T-prose-drift remain open.**
38. **T-root-recovery: planned root rotation + per-root status built; ROOT_RECOVERY stays RED by design.** Done (VL-044; spec `7cfc699` + conservative-frame `9e5181b` + build `aec58ee`). Per artifact 11 (B-prime-3): a trusted root signs its successor's designation (in-band rotation, conservative single hop) + per-root status (active/retired/revoked) honored at the record-validation layer. `EVIDENCE/published_roots_gen.py` + `IMPLEMENTATION/root_record_source.py` (sibling reader -> status view) + a `root_status_view` gate on `key_record_source.py` (`REF_VERIFY_ROOT_REVOKED` / `REF_VERIFY_ROOT_RETIRED` via issued_at<retired_at; None = VL-042 byte-behavior) + four `REF_VERIFY_ROOT_*` constants in verifier.py (no `verify_envelope` logic change). Tests `TESTS/adversarial/test_root_record.py` (18) + proof `EVIDENCE/proofs/root_record_001_runner.{py,log}` (live R1->R2 rotation, exit 0). `root_rotation` capability built-but-unwired; `ROOT_RECOVERY` RED with named anchor + narrowed blocked_by (Decision F + G); gate 0 of 3 green. Canon 8.2/9/11.9/13/14; no new invariant; section 14 narrowed reading one layer up. NO `pep.py` change (build-then-wire). pytest 178 -> 196 + 3 xfailed. **Next: run the transitive-designation cross-model evaluate (Decision H (ii); prompt `~/elyon-sol-offrecord/vl044_root_designation_evaluate_prompt.md`, three labs, blind, off-record) and fold the verdict (VL-044 follow-up) - only then may `forgery-resistant` move, and only as far as the bound permits; then the doc-freshness commit (artifact 04 VL-041..044, 00_README owes 08/09/10/11); then the reds in order - DEFAULT_SECURE (signing cutover; canary `test_unsigned_path_unchanged_forge_still_accepted` flips), END_TO_END_NO_SHORTCUT (G5 transport), ROOT_RECOVERY (rotation wired+transported); A1 target-side policy; caller-carry / proxy-removal; T-bookkeeping (G1/G8/G9/G11/G14 + `server.py` retirement) and T-prose-drift remain open.**
39. **T-root-recovery follow-up: transitive-designation cross-model evaluate folded; SOUND, bound unmoved.** Done (VL-044 follow-up, this commit). Decision-H (ii) evaluate run off-framework (Grok/OpenAI/Gemini, blind, off-record, `vl044_root_designation_evaluate_prompt.md`); verdict-of-record only (raw responses off-repo, VL-008). 3-0 convergent: transitive root designation is SOUND as built and adds NO adversary-reach beyond the VL-042-follow-up root-compromise-is-total bound; `forgery-resistant` UNMOVED (signed-path-under-uncompromised-root, out of any deposit). One Match-criterion divergence on the stated `overlap-conflict fail-closed` phrasing (OpenAI OVERSTATED, Grok/Gemini ACCURATE-by-construction; second instance of the VL-042-follow-up pattern; code + spec already correct). Two deferred gap candidates (retirement clock-skew - load-bearing, the artifact-09 clock-note sibling; consumer-layer window enforcement - minor). No code/canon/spec/manifest change. **Next: the doc-freshness commit (artifact 04 owes VL-041/042/043/044; 00_README owes artifacts 08/09/10/11), folding the two gap candidates + the carried artifact-09-section-5 clock note into the spec pass; then the readiness reds in order (DEFAULT_SECURE signing cutover, END_TO_END_NO_SHORTCUT G5 transport, ROOT_RECOVERY rotation wired+transported); A1 target-side policy; caller-carry / proxy-removal; T-bookkeeping (G1/G8/G9/G11/G14 + `server.py` retirement) and T-prose-drift. None blocking.**
40. **T-prose-drift + T-bookkeeping: VL-041..044 doc-freshness, the three spec-gap folds, and two methodology promotions.** Done (VL-045; content commits `3555dc1` doc-freshness + `919bc40` spec-clarification + `00f5709` methodology; STATE + ledger this commit). artifact 04 folds VL-041/042/043/044 (G4 cluster + G5 note, no status transitions); 00_README lists 08-11. artifact 11 section 6.1 clock-and-window note (retirement-gate clock-skew, VL-044 follow-up finding 1, + consumer-layer window); artifact 09 section 5 freshness-clock mirror (long-carried item closed); 3.3 overlap-conflict verified clean. Lesson 9 (run-cwd/scratch, three instances); cross_model_evaluate_template requester-discipline note (two instances). .gitignore guard found already present (VL-037 f/u + VL-042), not edited. Process/record hardening per VL-017a; no follow-up evaluate; board unchanged at 0 of 3 wired. **Next: the readiness reds in order - DEFAULT_SECURE (the mandatory signing cutover; the canary `test_unsigned_path_unchanged_forge_still_accepted` flips), END_TO_END_NO_SHORTCUT (G5 cross-host transport), ROOT_RECOVERY (VL-044 rotation wired to pep.py's default path + transported); A1 target-side policy; caller-carry / proxy-removal; the VL-042 ledger de-dup micro-commit; T-bookkeeping (G1/G8/G9/G11/G14 + `server.py` retirement) and the remaining T-prose-drift (the stale trailing Next-open-action prose below item 40; the verifier.py Step 1.5 comment). None blocking.**
41. **T-bookkeeping: ledger integrity - the duplicate VL-042 build entry removed.** Done (VL-046; ledger de-dup + VL-046 record, STATE this commit). The VL-042 build entry appeared twice as adjacent byte-identical blocks (254 lines / 16502 bytes each, verified identical by the apply-script before removal, VL-045 finding 1); the second copy removed, the first retained verbatim, the VL-042 follow-up untouched. The rare justified deletion of a verified exact duplicate (changes multiplicity, not recorded content), logged in its own appended VL-046 entry so the deletion is itself on the append-only record. No code/test/canon/manifest/spec change; 196 passed + 3 xfailed unchanged. **Next: the readiness reds in order - DEFAULT_SECURE (signing cutover; the canary `test_unsigned_path_unchanged_forge_still_accepted` flips), END_TO_END_NO_SHORTCUT (G5 transport), ROOT_RECOVERY (VL-044 rotation wired+transported); A1 target-side policy; caller-carry / proxy-removal; T-bookkeeping (G1/G8/G9/G11/G14 + `server.py` retirement) and the remaining T-prose-drift (the stale trailing Next-open-action prose below item 41; the verifier.py Step 1.5 comment). None blocking.**
42. **T-default-secure: the mandatory signing cutover; DEFAULT_SECURE goes green (1 of 3).** Done (VL-047; spec `82648dd` + build `56fd4f1`). `pep.py`'s default forward now signs (`_get_signing_key()` provider -> `sign_envelope` inside the fail-closed try/except; no key -> `REF_PEP_FAIL_CLOSED`, never an unsigned downgrade). The canary `test_unsigned_path_unchanged_forge_still_accepted` retired -> `test_verifier_unsigned_mode_accepts_forge_non_default` (verifier unsigned mode kept for the enforcement / A1-bypass demos, no longer the default). New `TESTS/conftest.py` autouse ephemeral-key fixture; `test_pep.py` +`test_default_path_is_signed_and_forge_refused` (canary replacement + `wired_to_default` proof) +`test_default_forward_no_key_fails_closed`; the DEFAULT_SECURE predicate drops xfail and wires ANCHOR 1; `EVIDENCE/readiness.json` `issuer_signing.wired_to_default`->true + `DEFAULT_SECURE.green`->true (cross-host-excluded scope note; G5 not asserted); `EVIDENCE/proofs/default_secure_cutover_001_runner.py` (exit 0). NO follow-up evaluate (no new world-claim). pytest 196 + 3 xfailed -> 199 + 2 xfailed. **Honest frame: the first red going green is the start, not "secure"; "forgery-resistant" stays bounded (root/issuer compromise out-of-band) and out of any deposit.** **Next: the remaining two reds - END_TO_END_NO_SHORTCUT (G5 cross-host transport), ROOT_RECOVERY (VL-044 rotation wired to `pep.py`'s default path + transported); then A1 target-side policy; caller-carry / proxy-removal; T-bookkeeping (G1/G8/G9/G11/G14 + `server.py` retirement) and the remaining T-prose-drift (the stale trailing Next-open-action prose below item 42; the `verifier.py` Step 1.5 comment). None blocking.**
43. **T-end-to-end: the signed cross-host chain; END_TO_END_NO_SHORTCUT goes green (2 of 3).** Done (VL-048; spec `2b48336` + build `a89c9b1`). The proof of record is `EVIDENCE/proofs/g5_signed_cross_host_001_runner.py`: a real two-process, real-socket, genuinely-divergent-disk run composing the VL-047 signing cutover with the VL-039 transport - the gate signs on its default path via the production env-var key path (NOT the conftest in-process fixture), pushes, and a target subprocess with a byte-divergent `evaluator.py` fetches the published record over a real `http.server` socket (production `fetch_published_record`) and verifies signature (out-of-band-pinned key) + currency-from-record + binding; honors signed-valid despite divergent disk, refuses keyless forge / tamper / anchor-fail / absent (exit 0). `IMPLEMENTATION/readiness.py::_consistency` narrowed to `END_TO_END_CAPABILITIES = (issuer_signing, enforcement_push)` (the enumerated dependency set; `validate_manifest` honesty loop preserved over all capabilities). `TESTS/readiness/test_deployment_predicates.py::test_end_to_end_no_shortcut` drops xfail, wires ANCHOR 2 as the lighter in-process signed-chain regression gate (production env-var key path, not the fixture). `EVIDENCE/readiness.json`: `issuer_signing` + `enforcement_push` `exercised_e2e`/`transported` -> true (proof: the runner); `END_TO_END_NO_SHORTCUT.green` -> true with the A3b-freshness + cross-host-floor scope note; expiry/revocation/rotation stay false. GR-2 formalized in `docs/MAINTENANCE_PROTOCOL.md`. Spec edits: `10_readiness_spec.md` section 4.2 (signed chain + enumerated set explicit) + section 8 + section 9 (GR-2); `08_enforcement_design.md` section 6 (VL-048 update). Canon 8.2/9/11.9/13/14; no new invariant; `verify_envelope` logic unchanged (the fetch is real, the verify is as-is); section 14 holds. NO follow-up evaluate (transport moves no trust boundary; the bounds were evaluate-set at VL-040 f/u 2 + VL-039 f/u). pytest 199 + 2 xfailed -> 200 + 1 xfailed. **Honest frame: 2 of 3 green; green means the full signed chain runs over real transport with no shortcut, NOT "deployed" (true multi-machine/TLS is the named G5 floor) and NOT A3b-freshness-closed. "forgery-resistant" stays bounded and out of any deposit.** **Next: the last red - ROOT_RECOVERY (the VL-044 rotation primitive wired to `pep.py`'s default path AND run over this session's transport with no shortcut; likely reuses this runner's transport); then A1 target-side policy; caller-carry / proxy-removal; T-bookkeeping (G1/G8/G9/G11/G14 + `server.py` retirement) and the remaining T-prose-drift (the `envelope.py` module-docstring lines 42-43 stale post-VL-029/047; the `verifier.py` Step 1.5 comment; the stale trailing Next-open-action prose below item 43). None blocking.**
44. **T-root-recovery-wire: planned root rotation consulted target-side over the signed cross-host chain; ROOT_RECOVERY goes green (3 of 3).** Done (VL-049; spec `52d3764` + build `7b0f258`). The VL-044 rotation primitive is wired onto the VL-048 transport: reading (A) - the gate default forward is unchanged; rotation is consulted target-side on the live chain. Proof of record `EVIDENCE/proofs/root_recovery_cross_host_001_runner.py` (a target pinning only R1 on a byte-divergent disk fetches published+root+key records over real sockets via the production fetch path, honors a gate-signed envelope vouched by the designated-active R2 with no re-pin, refuses keyless-forge / revoked / retired / fetch-failure / stale; exit 0; KILLER PROPERTY holds). Checkpoint C (b): `key_record_source.py::fetch_key_record` gains an additive `root_status_view=None` passthrough to the unchanged `load_key_record_from_bytes` (default None = VL-042 byte-behavior); `verify_envelope` + validation logic byte-unchanged (constraint g). `readiness.py` `ROOT_RECOVERY_CAPABILITIES = (root_rotation, issuer_key_revocation)` + a `_consistency` ROOT_RECOVERY clause (green = those two `exercised_e2e` + `transported`, mirrors END_TO_END; NOT `wired_to_default`, honest; load-bearing). `EVIDENCE/readiness.json` flips both capabilities' e2e/transported -> true (proof: the runner), `ROOT_RECOVERY.green` -> true + scope note; 3 of 3 green. `test_root_recovery_wired` drops xfail -> in-process regression gate (option alpha; the runner owns the no-shortcut transport). NO follow-up evaluate. pytest 200 + 1 xfailed -> 201 + 0 xfailed. **3 of 3 deployment predicates green - the finite road the readiness gate named is walked; NOT "deployed" or "secure" (out-of-band root/issuer COMPROMISE recovery + true multi-machine/TLS (G5) remain named floors; "forgery-resistant" stays bounded and out of any deposit). Next: A1 target-side admission policy; caller-carry / proxy-removal; T-bookkeeping (G1/G8/G9/G11/G14); T-prose-drift (the `envelope.py` module-docstring lines 42-43; the `verifier.py` Step 1.5 comment; the stale trailing Next-open-action prose below item 44; backfill the drifted Current-verified-state bullets for VL-038/039/048/049). None blocking.**
45. **T-G5-transport: G5 real-transport design (artifact 12) + step-1 transport seam built-then-wire.** Done (VL-058; design `37f9ab7` + build `7894c5d`). `docs/restructure/12_g5_transport_design.md` is the G5 build plan (the (A) buildable / (B) external-attacker split; build order steps 1-5; verified env-capability note - real TLS between distinct OS processes, docker NOT available in the sandbox). `IMPLEMENTATION/transport.py` adds the TLS-config seam (`post_to_target` / `get_published`; verify + optional client cert from args or the `ELYON_TLS_*` env, fail-closed; default request BYTE-IDENTICAL to the current direct `requests.post` / `requests.get`), proven by `EVIDENCE/proofs/g5_transport_seam_001_runner.py` over real TLS between OS processes (7/7, exit 0). Build-then-wire: NO callers; `pep.py` / `published_source.py` byte-unchanged; pytest 203 + 0 xfailed unchanged; no readiness/canon/spec/published_* change; no new invariant (canon section 14); GR-3 (no evaluate). G5 stays OPEN (capability unchanged until wired). **Next: wire the seam into `pep.py` + `published_source.py` with default args (byte-identical; prove the suite + the existing cross-host runners still pass) = step 1b; then the reference enforcing target (step 4; `target.py` is an 8-line stub today); then the multi-process + TLS harness (step 2, in-env; docker-compose is deploy-target only); then the TLS/cert bootstrap (step 3); then the attack harness mapped to the claim sheet (step 5). The external attacker (finish line B) closes G5 and is the author's to arrange. The reserved deposit-readiness audit renumbers VL-058 -> VL-059. A1 target-side policy + caller-carry / proxy-removal remain the section-14 forks; T-bookkeeping (G1/G8/G9/G11/G14 + `server.py` retirement) and T-prose-drift (the stale numbered Next-open-action list, items 30-44) remain open. None blocking.**
...
- **G14** - unknown-key refusal code under-determination inside
  ...
  inside `interaction`" rejected shape; build `5df3918` flips
  request_validator.py step 4d to emit it (the step-5
  `REF_SCHEMA_TYPE_MISMATCH` returns and the `REF_SCHEMA_PARSE_ERROR`
  boundary are byte-unchanged) and adds the spec-derived reject
  case. See `docs/restructure/04_current_vs_claimed.md`.
