...
Last updated: 2026-06-10 (VL-101: TOOLING INVENTORY - docs/TOOLING.md, the one-page index of every functional tool with one-line purpose + invocation + spec/ledger reference, in seven sections: the gate chain (pep/evaluator/request_validator/envelope/verifier/transport), target-side executors (reference_target/executor_sdk/replay_cache/mcp_server), trust records (publisher + the four readers + the four generators + readiness), the VL-097..099 audit layer (envelope_inspector CLI + issuance_log), the adversarial harness + proof-runner family, the deploy stack, and governance scripts. No test counts (G1 discipline); a maintenance rule binds future tools to add their row in the landing increment. README Orientation gains the TOOLING pointer and sheds an incidental G1-family drift (a stale 'most-recent entry is VL-029' sentence). Doc-only. Specs 26/27/28 remain SINGLE-SOURCE pending the author's VL-100 verification runs. G5 remains the only open road item. Does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-10 (VL-100: cross-model verification of the audit layer STAGED - docs/methodology/verification_request_vl100.md instantiates the VL-008 verification-request template over 22 enumerated claims (C26-1..C28-7) of specs 26 (inspector/reconciler), 27 (semantic re-evaluation), and 28 (issuance log), bound to a 9-file primary-source bundle (the 3 specs + envelope_inspector.py + issuance_log.py + pep.py + upstream envelope.py/verifier.py/evaluator.py). Premise-classification shape (Supported / Contradicted / Under-specified / Reframing required) per the VL-015/VL-016 precedent; per-spec status transitions SINGLE-SOURCE -> CONFIRMED (all claims Supported by two verifier runs) / DISPUTED (any Contradicted) are defined in the request. Unlike VL-014/VL-016 (chat-only), the concrete request is COMMITTED for durability. EXECUTION IS THE AUTHOR'S: run Grok + OpenAI with the request + the 9 attached files, bring back both responses; adjudication lands at a future entry (proposed VL-102). No code change. G5 remains the only open road item. Does not cite its own hash (VL-012).)
...
**NEXT (updated at VL-101 close, 2026-06-10) - execute the staged VL-100 verification runs (AUTHOR).** The request is committed at docs/methodology/verification_request_vl100.md; run it against Grok and OpenAI with the 9 attached files (the 3 specs 26/27/28 + envelope_inspector.py + issuance_log.py + pep.py + envelope.py + verifier.py + evaluator.py), collect both full responses including the Scope check sections, and bring them back for adjudication (per-claim tally, procedure-adherence check, status transitions SINGLE-SOURCE -> CONFIRMED/DISPUTED per spec, gap-candidate promotion) as proposed VL-102. AUTHOR-OPTIONAL in parallel: the live audit-chain run (ELYON_ISSUANCE_LOG_PATH on the deployed gate, target-side action records, `envelope_inspector reconcile` over real history). In-house alternatives if the runs wait: none blocking - the backlog is the standing process-finding list below. G5 (a real EXTERNAL attacker on a real PUBLIC surface) remains the only open road item; that track is deployment-and-people and is the author's.
