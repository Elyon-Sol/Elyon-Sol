...
Last updated: 2026-06-02 (VL-042 T-key-record (B-prime-2): published signed KEY RECORD built, opt-in - `EVIDENCE/published_keys_gen.py` signs a live `EVIDENCE/published_keys.json` of currently-valid issuer keys (per-key window + explicit `revoked` flag) under a publisher/ROOT key, with the record's own freshness bound; `IMPLEMENTATION/key_record_source.py` (new reader, mirrors `published_source.py`) verifies the record against a pinned ROOT public key, checks freshness (`not_after` + opt-in `serial`), and builds a per-key trust view; `IMPLEMENTATION/verifier.py::verify_envelope(..., key_record_view=...)` consults it RECORD-EXCLUSIVELY (decision 3) with a four-way lookup (`REF_VERIFY_KEY_UNKNOWN` / `REF_VERIFY_KEY_REVOKED` / `REF_VERIFY_KEY_OUT_OF_WINDOW`) before the unchanged Step 1.5 signature + VL-041 Step 1.5b expiry checks; the reader emits `REF_VERIFY_KEY_RECORD_INVALID` / `REF_VERIFY_KEY_RECORD_STALE`. pytest 160 -> 175 + 0 xfailed (real env); spec `c323b65` (artifact 09, standalone, spec-defines-the-change) + build `5e9fbf6`. Closes the DETECTED-compromise instant-kill case (revocation) that VL-041 expiry could not reach - a deployment can now INSTANTLY refuse a revoked key and a stale key record; the residual cached-record window is the transport/G5 surface. Build-then-wire: NO `pep.py` change (consulting the record is target-side posture). First `IMPLEMENTATION/` module to import `cryptography` (envelope/verifier stay duck-typed; dep unchanged since VL-040). Canon 8.2/9/11.9/13; no new invariant; verifier-layer, orthogonal to `reassert()`/CCS; section 14 holds ONLY under the narrowed reading carried up one layer (the publisher/ROOT key is now THE trusted identity - 'identity-agnostic' = identity is not an admissibility SUBSTITUTE, not no-identity-trust-anywhere). CLAIM-TRACK GATE: the publisher/ROOT key is a NEW, SINGULAR, LOAD-BEARING trust floor (root compromise is total) that OWES its own framework-level cross-model evaluate (the new-trust-root evaluate, prompt at `~/elyon-sol-offrecord/new_trust_root_evaluate_prompt.md`, run off-framework after this commit); 'forgery-resistant' stays BOUNDED, not settled, and out of any Zenodo deposit until that verdict is folded (VL-042 follow-up). Prior substantive entry VL-041 at build `807ccfe`; the VL-040 follow-up 2 verdict at `00fa503`. Next trajectory action: run the new-trust-root evaluate and fold it (VL-042 follow-up), then root rotation process; cross-host transport (G5); the mandatory signing cutover; A1 target-side policy; caller-carry/proxy-removal; T-bookkeeping (G1/G8/G9/G11/G14 + `server.py` retirement) and T-prose-drift remain open)
