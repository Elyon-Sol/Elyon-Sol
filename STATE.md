...
**Step 1b (wire the G5 transport seam) is DONE (VL-060).** The seam
(`IMPLEMENTATION/transport.py`) now has its two callers on the default
path (`pep.py` push, `published_source.py` fetch), byte-identical by
default, with the full suite + the named regression runners green. The
next G5 step is `docs/restructure/12_g5_transport_design.md` **step 4** -
the reference enforcing target (a real downstream running `verify_envelope`
+ the published-record check), superseding the 8-line
`IMPLEMENTATION/target.py` stub - then **step 2** (the chain as separate OS
processes over real TLS in-env; docker-compose is a deploy-target artifact,
not greened in-env - docker is absent from the build sandbox), **step 3**
(TLS/cert + trust bootstrap), and **step 5** (the attack harness mapped to
`docs/methodology/external_verification_readiness.md` gate 2, including the
honestly-open A3b stale-record attack). Finish line **(B)** - an EXTERNAL
attacker on a real surface - closes G5 and is the author's to arrange; GR-3
bounds any in-loop attack to characterization, never certification.
Standing, none blocking: the deposit-readiness audit (VL-059, reserved)
under GR-3; A1 target-side policy and caller-carry / proxy-removal as the
section-14 forks; T-bookkeeping (G1/G8/G9/G11/G14 + `server.py` retirement)
and the T-prose-drift on the stale numbered list below.
