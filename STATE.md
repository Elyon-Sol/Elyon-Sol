...
Last updated: 2026-06-09 (VL-078 B5 (artifact 13, Phase B - FINAL): latency budget + executor SDK. New `IMPLEMENTATION/executor_sdk.py`: a thin `ExecutorGate` that factors the executor sequence (load + anchor-verify record -> `verify_envelope` -> VL-076 replay de-dup) into one component returning `Decision(honored, reason)`, so an integrator wires admission in a few lines (construct the gate once with pinned keys + target_id + the published record; call `.check(envelope, interaction)` per request; act only if honored). No gate logic re-implemented; fail-closed on every undecidable path; second consumer pattern for the VL-076 seam. Spec `docs/restructure/18_latency_budget_and_sdk_spec.md`. Harness `EVIDENCE/proofs/latency_budget_001_runner.py` (+ committed `latency_budget_001.log`) measures p50/p95/p99 + throughput of the admit and verify paths; INDICATIVE sandbox figures: VERIFY (executor added per-call cost) p50 ~0.14 ms / p99 ~0.29 ms / ~6800 calls/s; ADMIT measured through the TestClient HTTP boundary (harness-inflated). Locus: AUTHOR re-runs on representative hardware for the budget of record; network/TLS cost NOT included (Phase C). Acceptance: TESTS/adversarial/test_executor_sdk.py (10) reproduces the executor refusal matrix via the SDK + the shared-replay-cache cross-instance case; the harness runs exit 0 (loose p50-verify<50ms sanity bound). pytest 274 -> 284 + 0 xfailed (live in-sandbox, Python 3.10). Build-then-wire: NO default-path caller; evaluator.py / pep.py / verifier.py / envelope.py / reference_target.py / mcp_server.py / replay_cache.py / published_hashes.json byte-unchanged -> no evaluator_sha256 roll. **Phase B (harden the wedge) is now fully walked B1-B5.** Next open action advances to Phase C - C1 (deploy packaging; locus AUTHOR). Does not cite its own hash (VL-012).)

PREVIOUS: 2026-06-09 (VL-077 B4 (artifact 13, Phase B): real MCP server - the in-process wedge demo (VL-066) is promoted to a runnable MCP server. New `IMPLEMENTATION/mcp_server.py`: a JSON-RPC 2.0 server over stdio speaking the MCP `initialize` handshake + `notifications/initialized` + `tools/list` + `tools/call`, with the production admissibility gate on tool execution - it reconstructs the expected interaction from the ACTUAL call and runs the production `verify_envelope` (signature -> reassert/currency -> binding -> freshness) then the VL-076 `ReplayCache` seam (its first real consumer). The envelope rides in the tools/call params `_meta` under `elyon-sol/envelope`. The side-effecting tool fires exactly once (the admitted call); un-attested (`REF_VERIFY_ENVELOPE_ABSENT`), rebound tool/args (`REF_VERIFY_BINDING_MISMATCH`), drifted (`REF_VERIFY_REASSERT_RE_EVALUATE_REQUIRED`), stale (`REF_VERIFY_SIGNATURE_EXPIRED`), and replayed (`REF_VERIFY_REPLAY`) calls are refused unfired; a refusal is an isError tool result, not a protocol error; pre-handshake tools/call is JSON-RPC -32002. Spec `docs/restructure/17_mcp_server_spec.md`. Closes the wedge demo's stated fidelity gap (no handshake / no stdio transport). Acceptance: TESTS/adversarial/test_mcp_server.py (14) covers the handshake + full refusal matrix + exactly-once; EVIDENCE/proofs/mcp_server_001_runner.py spawns the server as a REAL SUBPROCESS and drives JSON-RPC over real stdin/stdout (11/11, exit 0), confirming exactly-once across the process boundary via the read-only `elyon-sol/executed_count`. pytest 260 -> 274 + 0 xfailed (live in-sandbox, Python 3.10). Build-then-wire: NO caller on the default pep.py path; evaluator.py / pep.py / verifier.py / envelope.py / reference_target.py / replay_cache.py / published_hashes.json byte-unchanged -> no evaluator_sha256 roll. Honest ceiling: stdio is a real transport + process boundary but LOCAL; cross-host TLS + a real attacker remain the G5 / Phase-C finish line. Next open action advances to B5 (latency budget + SDK). Does not cite its own hash (VL-012).)
