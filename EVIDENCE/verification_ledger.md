...
## VL-146 — SES-9a ENABLE: signed key-record mode ON, live target + sidecar (2026-07-14)

**Claim.** SES-9a (VL-145, commit 1d6e777) built the record-exclusive gate-key trust path default-off. This session turns it ON on the two live enforce surfaces: the K-01 gate-key revocation floor is moved from OUT-OF-BAND re-pin to IN-BAND signed-record flip. A publisher-served ROOT-signed key record carries the live gate entry (gate-deploy-001); the target fetches it per request (ELYON_KEY_RECORD_URL), the sidecar reads a timer-synced local copy (ELYON_KEY_RECORD_PATH); both pin the ROOT public out-of-band and enforce the entry's revoked flag + validity window, superseding the static ELYON_GATE_* pin (retained as the rollback path).

**Construct.** (a) LIVE-OPS, config-only on the hosts: the ELYON_KEY_RECORD_* trios on target + sidecar, an in-process ROOT keypair on pub (private in a 0600 EnvironmentFile, public pinned OOB), and elyon-keyrecord-sync.timer (120s, inside the 300s not_after) syncing the record to the sidecar's local file. (b) ONE code increment — the kickoff assumed a serving endpoint that did NOT exist: added IMPLEMENTATION/publisher.py::published_keys_signed() (GET /published_keys_signed.json, per-request ROOT-signed, mirroring published_hashes_signed(); publisher-side env ELYON_KEY_RECORD_SIGNING_KEY_HEX/_ROOT_ID/_GATE_KEY_ID/_GATE_PUBLIC_KEY_HEX/_GATE_REVOKED), serial = int(now.timestamp()) so a restore always outranks a revoke. Commit 4a40bc9. Pre-req: all three nodes were 14 commits behind on 7dac7ad (1d6e777 absent) — git pull --ff-only to 012834b (then 4a40bc9 on pub), restart, running-commit re-verify, before any trio was set.

**Referent.** Live, over real transport. Target (attack_suite_live_runner + a pull-mode probe): positive control HONORED via the record path, 6/6 adversarial defeated (exit 0); fail-closed on a 404 key-record URL -> REF_VERIFY_KEY_RECORD_INVALID (no static-pin downgrade); revocation drill -> probe honored=False reason=REF_VERIFY_KEY_REVOKED, restore at higher serial -> HONORED; rollback rehearsal -> trio unset HONORS via static pin, trio restored -> key-record mode. Sidecar (/authz decision probe, mint at gate /governed-call): valid -> ALLOW/REASSERTED_AND_BOUND; revoke drill -> DENY/REF_VERIFY_KEY_REVOKED then ALLOW; fail-closed (garbage local file) -> DENY/REF_VERIFY_KEY_RECORD_INVALID then ALLOW. Lower-serial rollback -> REF_VERIFY_KEY_RECORD_STALE is NOT live-reproducible with the per-request endpoint (monotonic serial); unit-proven, TESTS/adversarial/test_fix_serial_rollback.py. Endpoint tests: test_publisher_key_record_endpoint.py 5/5; full adversarial suite 538 green on the author machine.

**Status.** RECORDED — live-ops enable + one publisher code increment. SINGLE-SOURCE, white-box, author-side. Committed natively (this block + STATE; 4a40bc9 carries the endpoint + tests).

**Honest scope.** Changed DEPLOYMENT POSTURE plus one serving endpoint; the verifier/canon trust primitives are unchanged (record-exclusive path is VL-042, wired VL-145). NEW availability dependency, named: key-record freshness is on the critical path — a pub outage or stalled sidecar sync ages the record past not_after and the surface fails CLOSED (REF_VERIFY_KEY_RECORD_STALE), an invalid/absent one is refused (_INVALID) — correct, but a coupling to monitor (pub uptime + the 120s sync inside 300s). D3 (root-status chain) deliberately NOT stood up (LATER; anti-drift). NOT G5; the external-attacker floor is unmoved and remains NOT-MET.

**Incident (carry it intact).** During live-ops the publisher SIGNING private key was printed to the working chat (a broad env-grep on the pub host), and the key-record ROOT private once (same mode). Remediation: publisher signing key ROTATED -> publisher-2026-07-14 (public bb704cc4...), re-pinned on target + sidecar, signed-hash path re-verified green; ROOT REGENERATED -> public bc12137d... before any consumer pinned the exposed one (zero blast radius). This CLOSES VL-108 open-item (1) but REPEATS that exposure class — standing lesson: presence-only checks (grep -q, masked awk) on any host holding a private key, never a value-printing grep.

#### Citation discipline (VL-012)
Prior substantive entry: VL-145 (SES-9a build, 1d6e777). Cites VL-042 (record-exclusive path), VL-108 (open-item (1), the prior publisher-key exposure, closed here), VL-122/VL-128 (the live-ops re-sign/serve pattern), VL-091 (the signed-hash-record parity mirrored by the new endpoint). Does not cite its own hashes.

#### Environment note (Cowork sandbox)
Endpoint + tests authored and suite-run via the VL-108/VL-126 mount discipline (the mount served a STALE verification_ledger.md this session — ended at VL-144, missing VL-145 — so this block was appended natively by the AUTHOR at the true file end). All host commands were run by the author; the assistant generated syntax, wrote the endpoint/tests, and adjudicated outputs. GR-4: appended, not edited.

## VL-147 — PUBLIC-SURFACE CURRENCY: the site's Zenodo DOI was two revisions stale; STATE.md's repo-is-private premise is false (2026-07-16)

**Claim.** A consistency check of the public site (elyon-sol.io) against the repository and against primary sources found four drifts on the site and one in STATE.md. Unlike VL-129 — where the live WordPress had drifted from the repo — the live page and `site/index.html` were IN SYNC with each other and both drifted from reality. The site's outward-facing citation target was the most consequential: it advertised Rev 6 as current while two later revisions were published.

**Construct.** (a) Fetched the live page and diffed its assertions against `site/index.html` at HEAD; identical. (b) Resolved each Zenodo DOI. (c) Queried the GitHub API for repository visibility. (d) Re-ran the suite at HEAD. Corrections landed in `site/index.html` (commit 53f35fc), the REPO-CANONICAL source; the LIVE site is a separate artifact updated in its own editor (VL-131), so this commit does not change what is public.

**Referent.** Primary sources, each checked directly:
- **DOI staleness.** Site asserted "Latest (Rev 6) 10.5281/zenodo.21107731" in four link targets + the version chain. `https://doi.org/10.5281/zenodo.21364720` resolves 200 to `zenodo.org/records/21364720` — Revision 8, published 2026-07-14, v27, superseding Revision 7 (`21313671`, 2026-07-11), which supersedes Rev 6. The site sent every visitor and citer to a twice-superseded record. **CORRECTED.**
- **Recurrence, not just the instance.** The staleness is structural: the site hardcoded a VERSION-SPECIFIC DOI, which restales on every publication (Rev 6 → 7 → 8 already). Zenodo publishes a CONCEPT DOI — `10.5281/zenodo.19367848`, "represents all versions, and will always resolve to the latest one" — which the site did not use. Three of the four links now point at it; the version-specific DOI appears once, as the labelled current revision. **CORRECTED at the mechanism.**
- **Test count.** Site asserted 553 (hero badge + assurance chip). 553 was correct at Rev 7's snapshot (26fcac2e, 2026-07-09). Suite re-run at HEAD: **616 passed**. Rev 8's own abstract states 616 — the site was stale against its own latest publication. Consistent with the G1 count of record (VL-145: 593 → 611; VL-146's endpoint +5). **CORRECTED.**
- **False changelog footnote.** Site asserted "Two lower-severity hardening items remain tracked (a canonicalization-consistency cleanup; a safer default for one inline sidecar mode)." Those are SES-5 and SES-6, both RESOLVED 2026-07-11 (VL-143, VL-144); Rev 8 lists both as fixed. The site UNDERSTATED its own state — the opposite direction from VL-125's overclaim, and harmless to credibility, but false. **CORRECTED**; rows added for SES-5, SES-6, and SES-8 (serial rollback), which the table omitted entirely.
- **SES-9a absent.** Signed key-record trust — built VL-145, ENABLED LIVE VL-146, the headline advance in Rev 8 — was not on the challenge page. **CORRECTED**: new subsection carrying the live drill result, the author-side white-box scope line, and the availability dependency VL-146 named (key-record freshness on the critical path, fails CLOSED on staleness).
- **STATE.md premise false.** `## Next open action` item (0) reads "HISTORY-SCRUB + OPEN THE CORE REPO. The core is AGPL-3.0 open-core but the GitHub repo is private." The GitHub API reports `private: False, visibility: public`, and Rev 8 states "The source repository is public and inspectable." The item's premise is FALSE and its second half is DONE. **DISPUTED, not yet corrected in STATE.md** — see Honest scope.

**Status.** RECORDED — currency verification + correction of the repo-canonical site. SINGLE-SOURCE, white-box, author-side. Site corrections committed 53f35fc; structural check (VL-131 style) parser residual 0, nothing unclosed, closes `</html>`, sections 11/11 unchanged.

**Honest scope.** Author-side currency work: no code/canon/manifest/test change, suite unaffected, no pin moved. It corrects what the project SAYS about itself, not what it does — the site's honest-scope language is retained verbatim throughout, and G5 is NOT-MET and unmoved. Two items are RECORDED-OPEN rather than closed here: (1) the LIVE site still carries all four drifts until the author applies them in its editor — this entry does not claim the public page is corrected; (2) STATE.md item (0) is disputed but not rewritten — retiring it is author judgment about the ordered action list, and it bundles a second half whose status this entry does NOT establish: the item made a history-scrub for the VL-108-exposed publisher key a HARD security gate ("dead AND unreachable in history") to be completed BEFORE flipping visibility. The repository is now public. The key is DEAD (rotated VL-122, and again VL-146), so the residual exposure is low. Whether the UNREACHABLE half was ever executed is not recorded in STATE.md or this ledger and was NOT established here — it is a named-open question, not a finding either way.

#### Citation discipline (VL-012)
Prior substantive entry: VL-146 (SES-9a enable, live). Cites VL-129 (the prior site-drift correction and the precedent that this class is a verification event), VL-131 (site/index.html is repo-canonical, the live site is separate, and the parser-residual validation style reused here), VL-125 (the overclaim direction this entry inverts), VL-143/VL-144 (SES-5/SES-6, which the footnote wrongly called open), VL-145/VL-146 (SES-9a, absent from the changelog), VL-108/VL-122 (the publisher-key exposure underlying item (0)'s scrub gate). Does not cite its own hashes.

#### Environment note
Native Windows checkout; no mount artifact (the VL-108/VL-126 hazard is sandbox-only and did not apply). The assistant fetched the live page, resolved the DOIs, queried the GitHub API, re-ran the suite, made the `site/index.html` edits, and authored this block; the author directed the sweep and applies the live-site edits. Landed in the same session as the GR-6 STATE.md archive split (4890d85, a4b7add) — that work is reorganization and correctly takes NO VL entry per GR-4 clause 1, as GR-5 did. GR-4: appended, not edited.

#### Ledger heading-format drift (recorded, not corrected)
Entries VL-001..VL-133 use `### VL-N - <date> - <title>`; VL-135 onward use `## VL-N — <title> (<date>)` — a heading-DEPTH and dash change at the VL-134 boundary. This entry follows the current (`##`) form for consistency with its neighbours. Flagged because GR-5 clause 1 specifies archive volumes are "cut at a primary `### VL-N` header": at the next ledger archiving event that cut rule will not match any entry from VL-135 on. A GR-5 clause-1 amendment (or a heading normalisation, which clause 2 forbids as an in-place edit) is required before the next volume split. Recorded here; not acted on.

## VL-147 follow-up — the item-(0) history-scrub HARD GATE is MET: no secret material has ever been reachable in git history (2026-07-16)

**Claim.** VL-147 left the "Next open action" item (0) DISPUTED-not-corrected because its second half — a HARD SECURITY GATE requiring the VL-108-exposed publisher signing key to be "dead AND unreachable in history" BEFORE the repository went public — had a status this ledger did not record, and the repository is already public. This entry establishes it against a primary source rather than leaving it open. The gate is MET; item (0) is DONE in both halves.

**Construct.** A presence-only sweep of the FULL object store — every blob that has ever existed on any ref, not just HEAD or the reachable tree. `git cat-file --batch-all-objects` enumerated 1055 blobs; each was matched against (a) a 64-hex value assigned to any `*SIGNING_KEY_HEX` / `*PRIVATE_KEY_HEX` / `*_PRIVATE` / `*SECRET_KEY` variable — the exact shape of an Ed25519 seed, which is what the exposed publisher and root keys are — and (b) PEM private-key blocks. Obvious placeholders (`0000...`, `deadbeef`, `<...>`, `$VAR`) were excluded from the count. Discipline per the VL-108/VL-146 standing lesson: the scan reports blob id, path, and VARIABLE NAME only — no matched value is printed, and none was.

**Referent.** 1055 blobs swept. **64-hex secret assignments: 0. PEM private-key blocks: 0.** Corroborating, from the same sweep: no `.env` file has ever been added on any ref (the nine candidate paths are generators, tests, specs, `governance.env.example`, and the rotation runbook — no key material); `ELYON_KEY_RECORD_SIGNING_KEY_HEX=` and `ELYON_ROOT_SIGNING_KEY_HEX=` appear in ZERO commits; the `ELYON_PUBLISHER_SIGNING_KEY_HEX=` / `ELYON_SIGNING_KEY_HEX=` occurrences are documentation and example assignments in runbooks/specs, and the blob sweep confirms none carries a 64-hex value. This matches the record: VL-108 and VL-146 both state the keys were "printed to the working chat", and deploy `.env` files live only on the hosts and are gitignored — the exposure channel was the CHAT, never the repository.

**Status.** RECORDED — the gate is satisfied VACUOUSLY: there is nothing to scrub, because the material was never committed. Combined with DEAD (publisher rotated VL-122, rotated again VL-146; root regenerated VL-146 before any consumer pinned it — zero blast radius), item (0)'s criterion "dead AND unreachable in history" is **MET**, and its first half (open the core repo) is already done — the GitHub API reports `visibility: public` (VL-147). Item (0) is retired in STATE.md in the same commit.

**Honest scope.** This is a TARGETED sweep for the class item (0) actually named — an Ed25519 seed assigned to a signing/private variable, plus PEM blocks — over the full object store. It is NOT a general gitleaks/trufflehog run and does not claim the repository is free of every secret class (an API token, a password, a high-entropy string in an unusual shape). Item (0) named a specific artifact and a specific criterion; that criterion is met. A broader credential sweep is a separate, lower-priority question and is NOT closed by this entry — the material at issue is dead regardless, so the residual risk it would address is not the VL-108 key. No code/canon/manifest/test change; suite unaffected. NOT G5.

#### Citation discipline (VL-012)
Prior substantive entry: VL-147 (public-surface currency), which named this question open. Cites VL-108 (the original publisher-key exposure and the pre-exposure checklist that became item (0)), VL-122 (publisher key rotation — the DEAD half), VL-146 (the repeat exposure, the second rotation, and the root regeneration), and the item-(0) text in STATE.md's "Next open action". Does not cite its own hashes.

#### Environment note
Native Windows checkout. Presence-only scan authored and run by the assistant; no value was printed to the session, in keeping with the VL-146 standing lesson that produced the rule. A first pass of the scan checked only HEAD due to an error in the assistant's own script (an empty rev slice silently narrowed the sweep); it was corrected to the full 1055-blob object-store walk before any verdict was drawn — the HEAD-only result would have been a weaker claim presented as a stronger one. GR-4: appended, not edited.

## VL-148 — SES-9b (GL-01-refine): approver trust resolved from the signed chain IN-PROCESS; the wiring guard gates G-01 on provenance, not injectedness (2026-07-16)

**Claim.** The last deferred governance-review item (VL-124's OpenAI sharpening, scheduled at VL-145) is resolved. VL-123's high-impact startup guard required the approver map to be INJECTED (not the bare static pin) but checked injected-NESS, not signed-chain PROVENANCE — so a gate-controlled map injected under a different `key_id` passed both the guard and `verify_grant`. Injectedness is not provenance: whatever process can set the injection seam can supply any keys. This increment moves approver trust onto the signed key-record chain the gate cannot forge, resolved IN-PROCESS by pep — the analog, for the APPROVER key, of what SES-9a/K-01 (VL-145/VL-146) did for the ISSUER key.

**Construct.** (a) `IMPLEMENTATION/pep.py`: `_resolve_signed_chain_approver_keys()` reads the `ELYON_APPROVER_KEY_RECORD_PATH` + `ELYON_PINNED_ROOT_KEY_ID` + `ELYON_PINNED_ROOT_PUBKEY_B64` trio (with `ELYON_SIGNING_KEY_ID` excluded as the gate key, `ELYON_CLOCK_SKEW_SECONDS` for the window) and resolves the role-distinct approver map via the EXISTING `key_record_source.load_key_record_from_bytes` + `approver_trust.resolve_approver_keys` (VL-119, byte-unchanged). `_get_approver_keys_with_provenance()` labels the map with one of `APPROVER_PROV_{SIGNED_CHAIN,INJECTED,STATIC_PIN,NONE}`, resolution order seam > signed-chain > static pin > none; `_get_approver_keys()` returns the map unchanged for the request path. (b) `IMPLEMENTATION/governance_wiring.py`: the provenance vocabulary lives here (the guard is the authority on what is acceptable); `assert_high_impact_wiring`'s `approver_from_injected: bool` becomes `approver_provenance: str`, and G-01 now requires `SIGNED_CHAIN` — an `INJECTED` or `STATIC_PIN` map is refused under a high-impact manifest. (c) `deploy/governance/approver_trust_bootstrap.py`: RETIRES the injection step (resolve-then-inject was exactly what made provenance shim-supplied rather than intrinsic) and re-exports `pep.app`; pep resolves the same env trio natively. The env contract and run command are unchanged, so `docker-compose.governance.yml` and `GOVERNANCE_DEPLOYMENT.md` keep working — the finding's "simplify deployment" half is realised (the shim is now a thin alias).

**Referent.** Suite 616 → 630. `TESTS/adversarial/test_approver_signed_chain.py` (+12) drives pep's OWN resolver over a REAL publisher-signed key record on disk (built with the production `EVIDENCE.published_keys_gen` + the signed reader + the role-distinct resolver): an `approver`-role key resolves with `SIGNED_CHAIN` provenance and the resolved public key verifies that approver's signature; a no-approver-role / wrong-pinned-root / tampered-signed-field / missing record all fail closed to an EMPTY map that is still labelled `SIGNED_CHAIN` (so G-06 fires — not a silent downgrade to the static pin); provenance ordering; and the REAL pep startup hook STARTS a high-impact gate wired via the signed chain but FAILS CLOSED (`RuntimeError`, "G-01") on the bare static pin. `test_governance_wiring.py` migrates the param and adds the load-bearing catcher `test_G01_injected_refused_GL01_refine` — an INJECTED map is refused at G-01, the exact residual. REVERT-PROVEN: reverting G-01 to accept any non-static provenance turns the injected-refused catcher, the none-refused catcher, and the e2e signed-vs-injected test RED; restore → GREEN (native, Python 3.13.14 / pytest).

**Status.** RESOLVED — the last scheduled governance-review item; SES-9b is done. SINGLE-SOURCE, white-box, author-side. Committed `ef91842` (build + tests) + this ledger block + STATE.

**Honest scope.** DEPLOYMENT-POSTURE hardening, NOT a live break: on the live surface (`HIGH_IMPACT: []`, single worker) the guard is a NO-OP and the residual was never exploitable — the increment is inert there and the per-request path is byte-behavior-unchanged (trio unset → early return; the static-pin path is identical). Canon / evaluator / manifest / published_hashes / impact / envelope / verifier / approval / approver_trust are byte-UNCHANGED — NO pin moved. Pure, ABOVE G(I). The CUSTODY half of [FIX H5] (a deployment proof the gate process cannot read the approver PRIVATE key) remains operator-locus, as VL-119 recorded; this closes the PROVENANCE-guard residual, not custody. NOT G5; the external-attacker floor is unmoved.

#### Citation discipline (VL-012)
Prior substantive entry: VL-147 follow-up. This block cites VL-124 (the GL-01-refine finding + its named-open schedule), VL-123 (the guard this tightens), VL-119 (`approver_trust.resolve_approver_keys` + the signed key-record trust view it reuses, byte-unchanged), VL-115 (the pep approval wiring + the `_INJECTED_APPROVER_KEYS` seam), VL-145/VL-146 (SES-9a/K-01 — the ISSUER-key signed-chain precedent this mirrors for the approver key), and VL-042 (the record-exclusive reader path). Does not cite its own hashes.

#### Environment note
Native Windows checkout; no mount artifact. The assistant designed the increment (after reading the VL-124 finding, the guard, `approver_trust`, the deploy shim, and the SES-9a precedent in `reference_target`), wrote the code and tests, ran the revert-proof, and authored this block; the author chose the trajectory item and the startup-resolution fork (over per-request). GR-4: appended, not edited.

## VL-149 — CORRECTION of VL-147: the "live site is stale" claim was a browser-cache artifact; the live elyon-sol.io was CURRENT, the REPO FILE was the stale one (2026-07-16)

**Claim.** VL-147 asserted the live elyon-sol.io was stale — "the live page and `site/index.html` were IN SYNC … both drifted from reality," advertising Rev 6 and sending "every visitor and citer to a twice-superseded record." That LIVE-SITE characterization is FALSE and is retracted here. The live site was CURRENT: its hero badge reads `616 tests green` (the Rev 8 figure), per a screenshot the author supplied, and per the author the live site matches Rev 8. The drifted artifact was the REPO FILE `site/index.html`, which lagged at 553 / Rev 6 until 53f35fc. VL-147 got the drift DIRECTION backwards — the repo was BEHIND the live site, not in sync with it.

**Construct.** Mechanism of the error: every read of elyon-sol.io this session went through the in-app browser, which served a CACHED page (Rev 6 / 553) — evidently carried from a prior session; no cache-busting reload was performed, and a second "re-fetch" merely re-served the same cache, which read as confirmation. VL-147's "fetched the live page … identical" and its live-site bullets rested entirely on that cached read. The author corrected it by supplying a screenshot of the live hero after this session twice reported the site as stale.

**Referent.** (a) Primary source for the correction: the author's screenshot of the live hero showing `616 tests green` — NOT the `553` the cache reported — plus the author's direct statement that the live site matches Rev 8. (b) What in VL-147 STANDS, because it never depended on the browser: the REPO FILE `site/index.html` was genuinely stale at 553 / Rev 6 (read DIRECTLY via the file tool) and its correction in 53f35fc was valid and necessary; Rev 8's DOI `21364720` resolved 200 via PowerShell `Invoke-WebRequest` (out-of-band, not the browser); the GitHub-visibility check; the suite count 616. (c) The STATE.md note in commit 2d9bc21 ("RE-VERIFIED … the LIVE elyon-sol.io is UNCHANGED — still Rev 6 / 553") is FALSE and is corrected in STATE.md in this same commit.

**Status.** CORRECTED / RETRACTED-IN-PART. Append-only per GR-4 clause 2: VL-147 is left BYTE-INTACT; this entry supersedes only its live-site claim and cites it, exactly as VL-129 corrected VL-125. The forward-pointer lives in `verification_ledger_index.md` (the additive-navigation layer, GR-4 clause 3), not inside VL-147. The author is authoritative on the live state; I cannot independently re-verify the full live site now because the in-app browser cache persists.

**Honest scope.** This NARROWS VL-147, it does not demolish it: the repo-file currency corrections were valid; only the claim about the LIVE site's state was wrong, and wrong because a cached browser read was trusted as a primary source. STANDING LESSON (promote): an in-app-browser read is NOT a primary source unless cache-busted; the file tool and out-of-band HTTP are reliable, the in-app browser cache is not — a re-fetch that "confirms" a prior read may simply re-serve the same cache. No code/canon/manifest/test change; suite unaffected; no pin moved. NOT G5; the external-attacker floor is unmoved.

#### Citation discipline (VL-012)
Prior substantive entry: VL-148 (SES-9b). Cites VL-147 (the entry corrected here), VL-129 / VL-125 (the correct-a-prior-entry precedent this follows), VL-131 (`site/index.html` is repo-canonical and the live site is a SEPARATE artifact — the distinction VL-147 blurred by calling them "in sync"). Does not cite its own hashes.

#### Environment note
Triggered by the author supplying a live-hero screenshot after this session reported the site as stale. All elyon-sol.io reads this session were via the in-app browser and are now known cache-stale; the reliable reads this session were the file tool (repo `site/index.html`) and PowerShell HTTP (DOI resolution, GitHub API). GR-4: appended, not edited.

## VL-150 — Refusal reason codes: evaluate() gains a disjoint G_ vocabulary via decide()/refusal_reason(); pep surfaces which condition refused (2026-07-16)

**Claim.** The reference evaluator returned a bare `"REFUSE"` for six structurally distinct failure conditions, and `pep.py`'s REFUSE branch documented that it withheld any evaluator-layer refusal code because "the evaluator-layer refusal vocabulary is not specified." This increment reinstates machine-readable refusal reasons — the canon-evolution audit's highest-ranked reinstatement candidate — as a closed set of six `G_` codes naming WHICH condition failed, surfaced by pep in the 403. IMPLEMENTATION-only: the CANON is UNTOUCHED (this is deliberately NOT a canon-version event) and `evaluate()`'s decision contract is byte-behavior-identical.

**Construct.** `IMPLEMENTATION/evaluator.py`: six module-level `G_` constants (G_MANIFEST_MALFORMED, G_REQUIRED_SETS_UNRESOLVED, G_AC3, G_T26, G_MANIFEST_INTEGRITY, G_INTERNAL), one per real refusal point in `evaluate()`'s short-circuit order, in a namespace DISJOINT from the boundary REF_* vocabulary (pep/verifier/authz_sidecar, which never call `evaluate()`). The decision is refactored into `decide(ctx, manifest) -> (state, reason)` as the single source of truth; `evaluate()` becomes a byte-behavior-identical back-compat projection `decide()[0]`; `refusal_reason() = decide()[1]`. Build-then-wire: `pep.governed_call` switches its evaluator call to `decide()` and surfaces `reason` in the 403 detail's `refusal_reason_code` (additive — prior callers saw `{terminal_state: REFUSE}` with no code; the REF_PEP_FAIL_CLOSED exception path is unchanged). No new invariant, no gate, no admissibility-decision change (canon §13/§14).

**Referent.** Test execution (native, Python 3.13 / pytest). `TESTS/adversarial/test_refusal_reason.py` (+15): each of the six codes fires for its condition; short-circuit order (AC3 precedes a simultaneous T26 failure); `evaluate()` proven byte-behavior-identical (bare `"ELIGIBLE"`/`"REFUSE"`, never a code or tuple); state/reason consistency across mutations; G_/REF_ namespace disjointness; and an END-TO-END pep test proving the 403 carries `refusal_reason_code == G_AC3` with the upstream never reached. `evaluator_sha256` moved e307fab2 -> ca7c922c; `EVIDENCE/published_hashes.json` regenerated via `published_hashes_gen.py` (VL-115 discipline) — `canon_sha256`, `evaluator_version` (0.9.8.4, tracks the canon version implemented, not the code revision), and `manifest_sha256` all UNCHANGED. Full suite 630 -> 645 green.

**Status.** RECORDED — IMPLEMENTATION increment, author-directed ("advance if a true advancement"). SINGLE-SOURCE, white-box. Committed 732f1d2 (code) + this ledger/STATE close. NOT deployed: the author deploys; a partial rollout self-refuses via `reassert()` Row 3 (a node on the old evaluator computes a mismatching `evaluator_sha256` → RE-EVALUATE-REQUIRED). Deliberately scoped as the CODE half, NOT the v0.9.8.5 canon increment — the canon-text pieces (the §6 `ccs_valid`→`manifest_integrity` naming note, the §8.2 PoE rationale) are not a compelling standalone canon increment and remain the author's to ratify and to author as the PDF source-of-record (GR-1).

**Honest scope.** Closes the auditability gap the audit ranked highest and pep itself documented as a known omission — for an auditability-first substrate, "which condition refused" is now readable off the 403 instead of re-derived. The admissibility decision is UNCHANGED for every input (same ELIGIBLE/REFUSE), so this adds observability, not behavior; the live surface is unaffected until the author redeploys. WHITE-BOX; NOT external validation; NOT G5.

#### Citation discipline (VL-012)
Prior substantive entry: VL-149. Cites VL-115 (the regenerate-via-the-generator re-pin discipline followed here), VL-132 (the prior `evaluator.py` edit that moved `evaluator_sha256`, and the "evaluator_version tracks the canon version implemented, not the code revision" convention this reuses), VL-019/VL-029 (the pep evaluation + envelope-on-ELIGIBLE path this wires into). Does not cite its own hashes.

#### Environment note
Native Windows checkout. The assistant chose the back-compat + additive shape (keep `evaluate()` byte-identical; add `decide`/`refusal_reason`; wire pep), wrote the code and the 15 tests, regenerated the pinned record, verified the surfacing end-to-end via a TestClient 403, and authored this block; the author directed the trajectory and the "code half only" scoping followed the honest assessment that the canon increment is not compelling and its PDF is authorial. GR-4: appended, not edited.
