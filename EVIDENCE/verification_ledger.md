...
T-G4-enforce, the G4 enforcement terminus: the first state in which Elyon-Sol is a gate (a refusal that stops the action, against a defensible source) rather than a validator whose verdict the target can ignore. Built per the VL-038 opener with pre-locked Decisions A-G and Checkpoints A-D.

Delivery (Decision B; push). `IMPLEMENTATION/pep.py` (2 edits): import `canonical_json`; attach `headers={"X-Elyon-Sol-Envelope": canonical_json(envelope)}` to the ELIGIBLE `requests.post` forward. The forwarded body is unchanged (`normalized_interaction`), so a routed call and a direct (un-routed) call differ only by the attestation header. Header chosen over body-merge so the body contract with the target is untouched and the direct-vs-routed difference is exactly the header (the property the enforcing target keys on). canonical_json is ASCII (ensure_ascii=True), so the header value is transport-safe. The rest of `pep.py` and all of `verifier.py`, `envelope.py`, `evaluator.py`, `request_validator.py`, `CANON/`, `MANIFEST/` are byte-unchanged (Decision A; constraint l; confirmed by diff at Checkpoint C).

Published record (Decision C anchor; constraint i). `EVIDENCE/published_hashes.json` (new) carries the canon, evaluator, and manifest versions plus sha256 hashes - the same three hashes `build_envelope()` pins - extending `CANON/canon.lock`'s discipline to the evaluator and manifest (the minimal G5 primitive named in artifact 08 sections 6/9). Home is `EVIDENCE/` not `CANON/` so it stays outside GR-1's version-increment-only scope (it legitimately changes whenever evaluator.py or manifest.json change). Derived live by `EVIDENCE/published_hashes_gen.py` (new), never hand-copied; the generator reads `_read_canon_lock()`, `_evaluator_sha256()`, and `manifest_sha256()` so the record matches an envelope's pins exactly. Values at this commit: canon d1c9d187..b4d7bd, evaluator cf311cb7..e512b, manifest a21dea8b..823b8.

Enforcing target and verification (Decision C/D). `TESTS/adversarial/test_enforcement.py` (new, 7 tests) holds a test-scope published-source reader (`load_published_hashes`, `verify_against_published_record`) and an enforcing target (`build_enforcing_target_app`) that honors a delivered call iff (1) the envelope's pinned hashes match the published record (the defensible currency anchor - the published hashes, not the target's local disk) AND (2) `verify_envelope()` accepts (decision_sha256 integrity closing A2, request_context/target_url binding closing A3, plus reassert local currency), reused as-is per Decision D with no new REF_VERIFY_ code. The only new reason is the harness-level `REF_TARGET_PUBLISHED_RECORD_MISMATCH`; a missing or unparseable header maps to `verify_envelope(None, ...)` reusing `REF_VERIFY_ENVELOPE_ABSENT`. The 7 tests: published-record-matches-live-envelope-pins (guards the derivation); routed-valid-honored-end-to-end; direct-no-envelope refused (A1); forged refused (A2); replay binding-mismatch refused (A3); target_url-mismatch refused; and envelope-not-matching-published-record refused - the Decision-C load-bearing case, staged by handing the target a published record whose canon hash differs from live, so the envelope passes local-disk reassert and binding yet is refused on the published record. This proves the published-record check is independent of the target's local disk: defensibility is anchored to the committed record, reproducible by a third party who clones the repository.

test_pep.py migration (the wire-shape change). `TESTS/test_pep.py` rewritten (full replacement, not str_replace, because the fake_post signature change recurs identically across all five tests and a non-unique str_replace anchor would abort): every fake_post stub accepts the new `headers` kwarg; the ELIGIBLE-forward test asserts the envelope rides in the X-Elyon-Sol-Envelope header with the body left bare (no envelope in the body); the response-envelope test asserts the pushed-header envelope equals the returned envelope via canonical_json. Test count unchanged (5).

Evidence (Decision E; g3 format). `EVIDENCE/proofs/g4_refused_bypass_001_runner.py` drives the full two-hop path through the real FastAPI request/response cycle; `g4_refused_bypass_001.log` (the run) and `g4_refused_bypass_001.md` (the proof) record: 1 routed call honored and acted (REASSERTED_AND_BOUND), 5 refused 403 and not acted (A1 REF_VERIFY_ENVELOPE_ABSENT; A2 REF_VERIFY_REASSERT_INVALIDATED; A3 replay and target_url REF_VERIFY_BINDING_MISMATCH; REF_TARGET_PUBLISHED_RECORD_MISMATCH). The runner asserts the invariants (1 honored+acted; 5 refused 403 with 0 target actions) and exits non-zero on failure. Differs from g3 (external webhook): g4's defensibility is the committed published record, reproducible offline; cross-host real-HTTP transport is named not built.

Checkpoint A (design, user-confirmed): published artifact shape (single EVIDENCE/published_hashes.json), reader-not-parameter (a thin published-source layer outside verify_envelope/reassert, which are reused as-is), header delivery, enforcing-target shape, the enumerated refusal cases and the test_pep.py assertions that change, and the evidence plan; Decision C confirmed satisfied via the published-record-mismatch case. Checkpoint B (spec-gap, resolved): no SPEC/request_schema.md edit (the forwarded shape is the unchanged body plus one out-of-band header carrying the already-artifact-05-specified envelope; request_schema.md scopes only the inbound shape and calls the forward a PEP-wire concern); published_hashes.json already named in artifact 08; candidate GR-2 not triggered (no schema edit); code and tests land together, evidence after green. Checkpoint C (implementation review): the pep.py diff is exactly the 2 intended edits; the five reused source files plus CANON/MANIFEST byte-unchanged; all new/changed files ASCII-clean; the generator reproduces published_hashes.json; the section 14 re-read confirms the gate still does not verify (it decides and delivers) - the target verifies - and push deepens the pre-existing section 14 tension without the gate executing, with caller-carry named as the section-14-faithful later architecture (no new invariant). Checkpoint D: this entry plus the structural-doc, STATE, and ledger edits.

Source-first (Lesson 3): the pre-session checklist completed (29 checklist files plus the opener read from disk this session; Lessons 2/3/5/7 bodies read in full). The two test files (test_pep.py, test_adversarial_evaluator.py) were uploaded mid-session and read from disk before the test_pep migration was drafted. The Checkpoint D targets (artifact 04 G4/G5, artifact 06 section 13/14, STATE.md last-updated/current-verified-state/next-open-action, the ledger VL-037-follow-up anchor) were each read from disk before editing.

Process findings:
1. Chat-prose section-sign (U+00A7) leak, two instances this session (Lesson 7 stage 2; user-caught both times) [VL-038 follow-up correction: one self-verified, one user-reported; the second is not self-verifiable, since the drafting-time byte-sweep does not cover chat prose (VL-033 annotate-not-overwrite discipline)]. The recurring typographic-drift family: Greek at VL-029/VL-031, em-dash at VL-032/VL-033, section-sign at VL-034/VL-036/VL-037, and now two more chat-prose section-sign instances at VL-038. No committed byte was affected (the LC_ALL=C ASCII sweep over every deliverable came back clean; U+00A7 is the two bytes 0xC2 0xA7, both caught by the sweep). The gap is that Lesson 7 stage 2 (the drafting-time byte-sweep) is applied to files and apply-script literals but not to chat prose, which is the unswept write path; the in-session corrective was to spell out "section N" in all prose. Candidate refinement: extend Lesson 7 stage 2 explicitly to chat prose that cites canon sections.
2. Published-source reader scoped to the test harness for VL-038 (not promoted to IMPLEMENTATION/), and cross-host TRANSPORT of the published record framed as the after-VL-038 hardening. The co-located demonstration's defensibility is the committed published record (third-party-reproducible), while verify_envelope's reassert still reads local disk; the published-record-mismatch test is what demonstrates the published check is load-bearing and independent of local disk.
3. test_pep.py migrated as a full-file replacement rather than via str_replace, because the fake_post signature change recurs identically across tests and a non-unique str_replace anchor would abort; recorded so that the apply-script (str_replace-shaped edits only) and the full-replacement test file are not conflated.
...
**Status:** RECORDED (ledger-integrity correction; no code/canon/manifest/spec/
test change, no STATE-claim change). NO follow-up evaluate.
**Author:** Claude (working session with the project author)
**Classification:** process/record hardening per VL-017a - neither a capability
nor a trajectory move. A single-purpose corrective: it removes an accidental
duplicate from this ledger and records the removal. Named in VL-045 finding 1;
executed here as its own commit because it touches the append-only ledger by
DELETION and so owes an explicit, self-documenting decision.

**The duplicate.** The VL-042 build entry (`### VL-042 - 2026-06-02 - T-key-record
(B-prime-2) ...`) appeared TWICE, as two adjacent byte-for-byte-identical blocks
(254 lines / 16502 bytes each; verified identical by the apply-script before
removal, not by eye). The most likely cause is a double paste at the VL-042
commit - the duplicate-append mirror of the chat-paste-eats-content family
(Lesson 1), a double rather than a skip. The `VL-042 follow-up` entry is distinct
and was untouched.

**What was done.** The SECOND of the two identical blocks was removed; the first
is retained verbatim in its original position. The apply-script located both
build-headers and the follow-up header at runtime, extracted the two blocks,
ASSERTED them byte-identical, and aborted-or-removed - so the removal could not
fire unless the duplicate was exact. Net change from the removal: -16502 bytes
(254 lines). One `### VL-042 - ... T-key-record` build header now remains; the
follow-up remains; the recorded content of VL-042 is unchanged (one copy of the
same bytes).

**Append-only discipline (the exception, made explicit).** The ledger is
append-only by rule; this is the rare justified deletion - removing a verified
exact DUPLICATE changes no recorded content, only its multiplicity. It is logged
HERE (this appended entry) so the deletion is itself on the append-only record.
No substantive entry was edited; no claim, verdict, hash, or finding was altered.

#### Files affected

- `EVIDENCE/verification_ledger.md` (duplicate block removed; this VL-046 entry
  appended)
- `STATE.md` (Last-updated; current-verified-state bullet; "Next open action"
  item 41)

#### Files NOT affected

- All code, all tests, canon, manifest, spec, and every `docs/*` artifact. No
  admission-path change; the test set is unchanged.

#### Citation discipline (VL-012)

This entry does not cite its own commit hash. Prior substantive entry: VL-045
(STATE + ledger `43f3c21`; content commits `3555dc1` + `919bc40` + `00f5709`).
Repo test set 196 passed + 3 xfailed, unchanged (no code touched).

#### Next trajectory action

Unchanged by this correction. The readiness gate's three reds remain the ordered
road: DEFAULT_SECURE (the mandatory signing cutover; the canary
`test_unsigned_path_unchanged_forge_still_accepted` flips), END_TO_END_NO_SHORTCUT
(G5 cross-host transport), ROOT_RECOVERY (the VL-044 rotation primitive wired to
`pep.py`'s default path + transported). Then A1 target-side policy; caller-carry /
proxy-removal; T-bookkeeping (G1/G8/G9/G11/G14 + `server.py` retirement); and the
remaining T-prose-drift (the stale trailing "Next open action" prose below STATE
...
**Status:** RECORDED (capability / build-then-wire, default-off). Referent-bound: the acceptance suite is executed (21/21), the full suite is re-run live in-sandbox (228 -> 249, 0 xfailed), and the default path is proven byte-unchanged by `git diff` (no hashed file touched). No canon / SPEC-of-record(canon) / evaluator / MANIFEST / published_hashes.json / published_source.py / pep.py / envelope.py change; no `evaluator_sha256` roll.
**Author:** Claude (working session with the project author).
**Classification:** capability / trajectory move per VL-017a. Second Phase-B item (B2) of the artifact-13 directive; advances Next-open-action to B3.

#### The gap closed (in the freshness checks)
Every consume-side freshness comparison added across VL-041 (decision `not_after`), VL-042 (issuer-key validity window), and VL-048/049/074 (the signed key / root / published record `not_after`) compares the consumer's wall clock against an issuer-stamped timestamp with a STRICT `now < not_after`, assuming the two clocks agree. On a real cross-host deployment NTP keeps independent hosts within a bounded error, not in lockstep, so a target whose clock runs slightly ahead of the issuer false-REFUSES a decision/record the issuer still considers fresh. B2 makes the window tolerant of a bounded, configurable skew.

#### What landed (spec then build, capability default-off)
- SPEC `docs/restructure/15_clock_skew_tolerance_spec.md`: the symmetric consume-side model (`[not_before, not_after)` interpreted as `[not_before - clock_skew, not_after + clock_skew)`), the four loci, the explicit NTP / max-skew assumption, the non-negative guard, the fail-closed-ordering-preserved note, the canon-section-14 (no-new-invariant) basis, and the honest ceiling (a record stays usable up to `clock_skew` past its true `not_after`; default 0 changes nothing on the live chain). Adjacent to `14_published_record_freshness_spec.md`.
- BUILD: a `clock_skew: timedelta = timedelta(0)` parameter, appended last (VL-042 positional-compat convention), on four loci. `IMPLEMENTATION/verifier.py::verify_envelope`: the decision `not_after` (step 1.5b) becomes `current >= not_after + clock_skew` -> `REF_VERIFY_SIGNATURE_EXPIRED`, and the issuer-key validity window (step 1.5) becomes `not_before - clock_skew <= current < not_after + clock_skew` -> `REF_VERIFY_KEY_OUT_OF_WINDOW` (the symmetric `not_before` case). `IMPLEMENTATION/published_record_source.py`, `key_record_source.py`, `root_record_source.py`: the record-level `now < not_after` becomes `now < not_after + clock_skew` -> the respective `*_RECORD_STALE`, threaded through each `fetch_*` wrapper. The per-key/per-root validity windows are parsed into the trust/status view unchanged and time-checked (skew-adjusted) once, at the verifier consume side. A negative `clock_skew` raises `ValueError` at every entry point (a config error, fail-loud, parity with `sign_envelope`'s naive-`not_after` refusal). The serial-monotonicity check is untouched (a serial is a counter, not a clock).
- TEST `TESTS/adversarial/test_clock_skew_tolerance.py` (21): decision in-window-by-skew accepted / beyond-skew EXPIRED / default-strict-boundary / explicit-zero-strict / widened-edge-strict; key-window symmetric (before-start within/beyond skew, after-end within/beyond skew, default strict); published / key / root record-level stale-but-within-skew accepted and beyond-skew STALE; published default-strict-boundary; and a negative-skew `ValueError` guard at all four entry points.

#### Build-then-wire scope (honest)
This is capability-present-and-default-off: with `clock_skew=0` (the default, and the only value any committed caller passes today) every check is byte-behavior-identical to its strict pre-VL-075 form, so the default path, the committed g4/g5 runners, and the existing freshness suites are unaffected. `git diff` is exactly the `clock_skew` additions (4 files, +69/-13); evaluator.py / published_source.py / published_hashes.json / pep.py / reference_target.py / envelope.py are byte-unchanged. Wiring a non-zero skew onto the live `pep.py` / `reference_target.py` chain is a deployment-configuration choice with its own VL if it lands as a default. The claim earned: "the freshness checks accept a configurable, symmetric, non-negative clock-skew window, default zero," NOT "the gate tolerates clock skew end-to-end on the default path."

#### Verification (referent-bound)
- In-sandbox (Python 3.10): `python -m pytest TESTS/adversarial/test_clock_skew_tolerance.py` -> 21/21; `python -m pytest TESTS/` -> 249 passed + 0 xfailed (was 228).
- `git diff --stat` over the four IMPLEMENTATION files: +69 / -13, all `clock_skew`-related; `git diff` over the hashed / default-path files (evaluator.py, published_source.py, published_hashes.json, pep.py, reference_target.py, envelope.py) is empty.
- All six touched/new files ASCII-only (VL-006) and LF-only (Lesson 11).

#### Sandbox note (mount-read truncation + recovery; VL-071/072 family)
During this session the Cowork mount served TRUNCATED reads (cut near ~line 200) of the host-tool-edited files to the sandbox's `python`/`cp`, and an in-sandbox read-then-write-back roundtrip (intended as a stat-cache bust) PERSISTED that truncation to the working-tree files on disk - the host file tools then showed the truncated tails too. The git OBJECT store was intact (HEAD blobs full: verifier 420 / published 203 / key 291 / root 286 lines), so recovery rebuilt each file by applying the exact edits to the HEAD original staged on tmpfs (`git show HEAD:f > /tmp/f.orig`) and writing the full result back through the sandbox (tmpfs read full -> host write full); a fresh-write read-back probe confirmed the mount serves freshly-written files correctly. The same tmpfs-from-HEAD pattern built this STATE/ledger edit. A stale `.git/index.lock` ghost (`rm` reports it removed while git reports "File exists" on lock creation - VL-069/protocol-section-6 rule 4) wedged the normal index; commits were built with plumbing through a tmpfs-local index per the protocol's "Committing around a wedged lock" recipe. Operational finding: do NOT round-trip host-edited files through the sandbox to bust the cache (it can persist a truncated read); restore from the HEAD blob on tmpfs instead. Prevention remains a Cowork restart to re-mount fresh (protocol section 6 rule 8).

#### Citation discipline (VL-012)
Prior substantive entry: VL-074 (the signed published-record reader). This entry cites VL-041 (decision `not_after`), VL-042 (the issuer-key validity window), and VL-048 / VL-074 (the root / published signed-record freshness) as the strict checks this widens; it does not cite its own (spec + build + STATE + ledger) hash.
...
**Status:** RECORDED (scaffolding / build-then-wire). Referent-bound: the attack suite runner is executed (8/8 attacks defeated + positive control honored on the in-process surface, exit 0), the acceptance suite is executed (2/2, incl. the HttpSurface adapter driving a real reference target over HTTP), the full suite is re-run live in-sandbox (284 -> 286, 0 xfailed), and the default path is proven byte-unchanged by `git diff`. No canon / SPEC-of-record(canon) / evaluator / MANIFEST / published_* / IMPLEMENTATION change; no `evaluator_sha256` roll.
**Author:** Claude (working session with the project author).
**Classification:** scaffolding / trajectory move per VL-017a. The first Phase-C item walked in-house (C3's SANDBOX half); C1/C2 and the live run are AUTHOR-locus.

#### What this records
C3 of `docs/restructure/13_road_to_external_readiness.md`: turn the wedge claim and `docs/methodology/external_verification_readiness.md` gate 2 into runnable attacks, each emitting pass/fail, plus the claim sheet pairing each bounded claim to its attack and honest status. Acceptance (artifact 13 C3): the harness + claim sheet committed; runs against the C1/C2 surface (SANDBOX scaffolding now; AUTHOR against the live surface).

#### What landed (spec then build, build-then-wire)
- SPEC `docs/restructure/19_attack_harness_and_claim_sheet_spec.md`: the pluggable `Surface` adapter, the attack suite (the gate-2 break-it challenges), the in-process/author-live split, the fail-closed/no-new-invariant basis, the build-then-wire boundary, and the honest ceiling (an in-process pass is not external validation; referent-incomplete until gate 1).
- BUILD: `EVIDENCE/proofs/attack_harness.py` (NEW): `Surface` adapters - `InProcessSurface` (admit via the production `pep`, attempt via the VL-078 `ExecutorGate`, one persistent replay cache so the replay attack is real), `HttpSurface` (admit via a gate URL, attempt via a reference-target URL over real HTTP - the AUTHOR adapter), and `RequestsClient` (a `.post` shim for a real host) - plus `run_suite` running 8 adversarial attacks + a positive control. An attack PASSES when DEFEATED with the expected production `REF_*` reason; the harness adds no decision and no reason code. `EVIDENCE/proofs/attack_suite_001_runner.py` (NEW, + committed `attack_suite_001.log`): runs the suite against the in-process surface - positive control honored, all 8 attacks defeated (unattested/A1 `REF_VERIFY_ENVELOPE_ABSENT`, forged-signature `REF_VERIFY_SIGNATURE_INVALID`, replay `REF_VERIFY_REPLAY`, rebind-tool / rebind-args / target_url-swap `REF_VERIFY_BINDING_MISMATCH`, drifted-state `REF_VERIFY_REASSERT_RE_EVALUATE_REQUIRED`, stale `REF_VERIFY_SIGNATURE_EXPIRED`), exit 0.
- CLAIM SHEET `docs/methodology/falsifiable_claim_sheet.md` (NEW): every deposit-readiness-audit (VL-059) Section-A/B claim restated as a break-it challenge mapped to its attack id, defender, and honest status (DEFENDED in-process); the key-record / root attacks (revoked key, retired root, stale published record) pointed at their existing defenders (`test_key_record` / `test_root_record` / `test_published_record_freshness`); and the Section-C named-open boundaries (A1 declining caller, G5 real-transport floor + external attacker, A3b default-path record freshness, root-compromise recovery, rebuild-cost) listed as limits stated to a reviewer, not defended claims. Carries the forbidden-framings list (no cross-model convergence verdict shown to a blind reviewer; no whole-canon / un-qualified non-bypassable claim).
- TEST `TESTS/adversarial/test_attack_harness.py` (2): the full suite defeated against `InProcessSurface` (positive control honored, 8 attacks defeated); the `HttpSurface` adapter driving a real reference-target TestClient over HTTP (positive control honored, un-attested + rebind refused) - proving the real-surface adapter shape, the seam C1/C2 plug into.

#### Build-then-wire / honest scope
New files only; `IMPLEMENTATION/*` and `EVIDENCE/published_hashes.json` are byte-unchanged (empty `git diff`), so no `evaluator_sha256` roll and the default path / existing runners are unaffected. The harness only OBSERVES the gate. Per gate 2, defeating the attacks against the IN-PROCESS surface proves they are well-formed and the gate refuses them locally; it is NOT external validation. The claim earned: "the gate-2 attacks are runnable and defeated in-process, and the claim sheet exists," NOT "the gate is externally attack-tested." The HttpSurface adapter is built and shape-tested but the real-transport run is the author's (gate 1 / C1+C2).

#### Verification (referent-bound)
- In-sandbox (Python 3.10): `attack_suite_001_runner.py` exit 0 (8/8 + positive control), captured to `attack_suite_001.log`; `TESTS/adversarial/test_attack_harness.py` 2/2; full suite `python -m pytest TESTS/` 286 passed + 0 xfailed (was 284).
- `git diff` over `IMPLEMENTATION/` + `published_hashes.json` is empty.
- All six new files ASCII-only (VL-006) and LF-only (Lesson 11).

#### Honest ceiling
The single binding NOT-READY reason is unchanged: the G5 real-transport floor and a real external attacker (external_verification_readiness gate 1, the load-bearing gate). Sections 1-2 of the claim sheet are green against the in-process surface - necessary scaffolding, referent-incomplete until gate 1 provides a real surface to attack. No result here moves the external-validation axis; the sheet's value is that it makes every bounded claim falsifiable and attack-paired BEFORE an external party arrives.

#### Citation discipline (VL-012)
Prior substantive entry: VL-078 (latency budget + executor SDK). This entry cites VL-059 (the deposit-readiness audit, the claim taxonomy), `docs/methodology/external_verification_readiness.md` (gate 2, the deliverable), and VL-061 / VL-066 / VL-078 (the reference target / wedge / ExecutorGate the surfaces drive); it does not cite its own (spec + build + STATE + ledger) hash.
