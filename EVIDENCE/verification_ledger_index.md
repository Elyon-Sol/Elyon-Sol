...
- VL-147 - 2026-07-16 - PUBLIC-SURFACE CURRENCY: the site advertised Rev 6 as the current Zenodo record while Rev 7 and Rev 8 were published (twice-superseded citation target); the version-specific DOI replaced by the CONCEPT DOI so the class cannot recur; suite 553 -> 616; changelog footnote falsely called SES-5/SES-6 open (fixed at VL-143/VL-144) and omitted SES-8 + SES-9a; STATE.md's "the GitHub repo is private" premise DISPUTED against the GitHub API (visibility: public). Live site NOT yet corrected - separate artifact (VL-131).
