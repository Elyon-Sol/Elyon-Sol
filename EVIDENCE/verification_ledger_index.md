...
- VL-132 - 2026-07-05 - T-governance typed-impact evaluator increment: per-interaction-type required-set selection (resolve_required_sets); impact lifted from all-or-nothing to per-type. EDITS evaluator.py -> evaluator_sha256 re-pinned; default-off (flat manifest byte-identical).
...
- VL-134 - RESERVED, never issued (held for the deferred typed-impact live migration; see STATE.md). No entry exists at this number.
- VL-135 - 2026-07-11 - SES-1: SSRF guard on the gate forward target (pep._target_url_allowed / REF_PEP_TARGET_URL_BLOCKED).
- VL-136 - 2026-07-11 - SES-2: interaction binding made exact (extra body fields refused; REF_VERIFY_BINDING_MISMATCH).
- VL-137 - 2026-07-11 - SES-3: skew-aware replay retention (replay window tracks the true validity horizon expiry + skew).
- VL-138 - 2026-07-11 - SES-4: decision_id replay backstop (enforcing target fails closed when decision_id is absent).
- VL-139 - 2026-07-11 - SES-7: interaction_type bound into the signed envelope (hashed/signed/verified target-side; flat path byte-identical).
- VL-140 - 2026-07-11 - SES-8: signed-record serial rollback defense wired into the enforcing target (monotonic serial high-water mark).
- VL-141 - 2026-07-11 - SES-5 / SES-6 dispositioned OPEN (tracked, not yet fixed at this point).
- VL-142 - 2026-07-11 - Live chain advanced to HEAD; the eight CVP fixes (SES-1..8) deployed across the four nodes; external validation (G5) still OPEN.
- VL-143 - 2026-07-11 - SES-5 resolved: one canonicalization repo-wide (receipt.py reuses envelope.canonical_json; divergence pin retired).
- VL-144 - 2026-07-11 - SES-6 resolved: sidecar inline posture declare-or-fail (ELYON_EXT_AUTHZ_INLINE refuses the header default; ELYON_INLINE_* mapping; standalone byte-unchanged).
- VL-145 - 2026-07-13 - SES-9a resolved: signed key-record trust wired into the enforce path (K-01; default-off / build-then-wire; commit 1d6e777).
- VL-146 - 2026-07-14 - SES-9a ENABLE: signed key-record mode ON live (target + sidecar); gate-key revocation moved in-band; publisher + root keys rotated after chat exposure.
- VL-147 - 2026-07-16 - PUBLIC-SURFACE CURRENCY: the REPO FILE site/index.html advertised Rev 6 while Rev 7 and Rev 8 were published; the version-specific DOI replaced by the CONCEPT DOI so the class cannot recur; suite 553 -> 616; changelog footnote falsely called SES-5/SES-6 open (fixed at VL-143/VL-144) and omitted SES-8 + SES-9a; STATE.md's "the GitHub repo is private" premise DISPUTED against the GitHub API (visibility: public). **[SUPERSEDED IN PART by VL-149: this entry ALSO claimed the LIVE site was stale/uncorrected — that was a browser-CACHE artifact. The live site was already CURRENT (Rev 8 / 616); the repo file was the stale artifact, corrected at 53f35fc. The repo-file + DOI + GitHub + suite facts STAND.]**
- VL-148 - 2026-07-16 - SES-9b (GL-01-refine, VL-124): approver trust resolved from the signed key-record chain IN-PROCESS; the wiring guard now gates G-01 on signed-chain PROVENANCE, not mere injectedness; the deploy shim's inject step retired; suite 616 -> 630. Governance-layer, default-off, not on the public surface; not a live break.
- VL-149 - 2026-07-16 - CORRECTION of VL-147: the "live site is stale" claim was an in-app-browser CACHE artifact; the live elyon-sol.io was CURRENT (Rev 8 / 616, per the author's screenshot), the REPO FILE was the stale one. Append-only; VL-147 left byte-intact; the repo-file / DOI / GitHub / suite facts stand. Standing lesson: a browser read is not a primary source unless cache-busted.
- VL-150 - 2026-07-16 - Refusal reason codes: evaluator gains six disjoint G_ codes via decide()/refusal_reason() (evaluate() byte-behavior-identical back-compat), pep surfaces refusal_reason_code in the 403. CANON UNTOUCHED (code enhancement, not the v0.9.8.5 canon increment); evaluator_sha256 re-pinned via the generator; suite 630 -> 645. Not deployed (author deploys). NOT G5.
